...
struct RawBind {
  public:
    ...
    static int new_Raw( lua_State *L, const TrackedDataSet &files,
                            const JDay &ot, 
...
class Raw : public Raw_interface {
  public:
    ...
    /*virtual*/ const MatrixPos &getDefaultGridsize() const { return def_gridsize; }

    // data members
  private:
    TrackedData * const td;
    CONST_IF_SERVER NA_Data * const qd;     // received from 'td->Acquire()' or explicitly given (and to be deleted)
    const NA_Level def_level;   // level requirement used in opening the raw (default for grids)
                                // NA_Level::NO_LEVEL if no level requirement used

    const MatrixPos def_gridsize;   // Default gridsize for grids made out of this Raw object
                                    // 'MatrixPos::ZERO' for no default
                                    // 'MatrixPos::DX' for use native gridsize as default (ignore 'gridsize' global)
