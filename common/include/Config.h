...
//
// Note that these two parameters actually carry different heights; #2 height from
// geo potential (the round-ish globe), #3 height from ground level (though this has
// not been fully confirmed - who knows exactly?). Maybe #3 should not ever be used,
// anyways? TBD
//
// 18-Oct-2011 PKi: The primary/secondary parameter is defined by SQD_PRIMARY_Z/SQD_SECONDARY_Z
//
...
//--- 18-Oct-2011 PKi
//
// Enable 'W' param (in SQD data) to first look for param #43 (kFmiVerticalVelocityMMS) and if
// not there, #39 (kFmiVelocityPotential).
//
...
//          <<
//
// Reason to always generate errors is easier coding (no need to test for missing data)
// and that no other part of q3 API uses the 'val, err' return method (otherwise commonly
// used idiom in Lua). 
//
// Reason to let the calling script handle errors is giving it more control. Being able
// to test if certain criteria is met and do something else if not. 
//
// Note: We could have a calling param (i.e. 'error=false') to allow also the calling
//      handling of errors (best of both worlds). Or the calling script can use
//      'pcall()' wrapping around the call to catch the errors (standard Lua construct).
//
...
//---
// Is flight level handling taken completely outside of the 'q3' API or do functions
// present 'flight=true' and 'flight=uint' API?
//
// Reason to have the handling outside is that flight levels are merely pressure levels
// presented with a function. That's it. 
//
...
//
// Note: DO NOT DISABLE THIS. Smartmet editor uses binary fetches and would seize to work
//      if this is disabled.
//
...
// METQU only:
//
// When Raw data is created from scratch, with standard param names (i.e. "T", "N"), shall
// the standard names be copied to the data (as native names, i.e. "T:4") or the native name
// parts left empty (i.e. ":4").
//
