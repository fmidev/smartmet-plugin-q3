...
*
*   - ApiMatrix is either a derivative of 'Matrix' (scalar matrix) or 'VectorMatrix'.
*
* Scripts deal with matrices either as 'Matrix' (scalars) or 'VectorMatrix'.
* Many operations work alike for the both, but they are distinguisable i.e. by their
* type names.
*
...
class MatrixSize : public LuaNew<MatrixSizeBind> {
  public:
    MatrixSize( const MatrixPos &top_, const MatrixPos &bottom_ )
        : top(top_), bottom(bottom_) { INVARIANT(); }

    MatrixSize( const MatrixPos &gridsize ) 
        : top( MatrixPos::ZERO ), bottom(gridsize-MatrixPos::DXDY) { INVARIANT(); }

    MatrixSize( const MatrixSize &o ) : LuaNew_base(), LuaNew<MatrixSizeBind>(), top(o.top), bottom(o.bottom) {}
    ...
    bool operator==( const MatrixSize &o ) const { return (top==o.top) && (bottom==o.bottom); }
...
*
* The iteration format is FIXED to be y in outer loop ([0] is northmost), x in inner
* ([0] is westmost). This order directly affects i.e. matrix output and must not be
* changed! To change it, make another class.
*/
class Matrix;
class MatrixIter;
...
/*---=== ApiMatrix ===---
*
* Both 'Matrix' and 'VectorMatrix' have the '.grid' property to climb back to their
* origin (and to sister matrices). This class encapsulates that behaviour.
*
* Also, they have the 'getProjection()' method for doing '[loc]' indexing. Note that
* projection can be supported for also calculated temporary matrices that would
* not have 'grid'.
*/
class ApiMatrix {
  protected:
    ApiMatrix() 
        : grid_key(0) { INVARIANT(); }
    virtual ~ApiMatrix();
  ...
  public:
    ...
    *       based on their internal state (location pointing to in another matrix).
    *       Such matrices will return an empty projection.
    */
  ...
  private:
    // 'grid_key' is not known at construction time; we need to be able to set it also for 'const'
    // matrices, thus it is 'mutable' (does not change the available matrix itself).
    //
    mutable unsigned grid_key;      // reference for getting to 'Grid' and sister parameters
...
/*---=== Matrix ===---
*
* Common Lua interface for 'MemMatrix', 'SQD_Matrix', 'MQD_Matrix', 'MaskMatrix'.
* Derived for 'SubMatrix'.
*/
class Matrix;
struct MatrixBind {
  public:
    ...
    // Keep public to allow 'SubMatrix::index()' to call it (we could also pick the '__index' 
    // value in 'SubMatrix::setup()' and remember it, but that gets kind of complex)
    //
  public:
    static int index( lua_State *L );
...
/*
* Note: Some matrices are read-only by nature (if memory mapped from disk). Others can
*       be made read-only by setting 'is_readonly' to true. READ ONLY IS NOT CHECKED
*       at every write operation; instead the upper script interfacing level is expected
*       to check it. This is to avoid numerous unnecessary checks at the low level.
*       In other words, write operations should never be attempted if 'is_readonly' is
*       true.
*/
class NFmiGrid;
class Matrix : public ApiMatrix, public LuaNew<MatrixBind> {
  ...
  public:
    ...
    MatrixPos getGridSize() const { return MatrixPos( size.getXS(), size.getYS() ); }
    ...
    */
    ...
    string_or_null getParamStr() const {
		if (param == 0 /*kFmiBadParam*/) return "";
  ...
  public:
    ...
    int offsetPosition(lua_State *L,const NFmiPoint &location,const MatrixPos &offset) const;
    int offsetPosition(lua_State *L,const Matrix &m,const MatrixPos &pos,const MatrixPos &offset) const;
    int offsetPosition(lua_State *L,const NFmiLocation &location,double xoffsetkm,double yoffsetkm) const;
    int offsetPosition(lua_State *L,const Matrix &m,const MatrixPos &pos,double xoffsetkm,double yoffsetkm) const;
  ...
  protected:
    ...
    void resize( const MatrixPos &gs ) { size= MatrixSize(gs); }    // used by 'MQD_Matrix' constructor
...
//  float reduce_min() const;
  ...
  protected:
    /*const*/ bool is_readonly;     // 'MemMatrix' can be made read-only after initialization
