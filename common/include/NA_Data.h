/*
* NA_DATA.H                            Copyright (c) 2009-10, Ilmatieteen laitos
*
...
//
// 22-Aug-2011 PKi: If USE_NEWBASE_PROJ is defined, using newbase interpolation, projection and scaling
...
/*
* Information about a 'NA_Data' derived data source (s.a. 'SQD_Data' or 'MQD_Data').
*
* Can be used also as-is, for carrying requirements for a to-be-created 'NA_Data'
* derivative.
*
...
*           "binary_offset=NNN"     start of binary block (16 byte aligned)     TBD: not sure if this is really needed
*/
class NA_Info {
  public:
    NA_Info( const time_t loadTime_,
			 const time_t modificationTime_,
			 const JDay &ot_,
			 const std::vector<JDay> &times_,
			 const std::string &nativeLevelType, const std::vector<NA_Level> &levels_,
			 const std::vector<NA_Param> &params_,
			 const Projection &projection_,
			 const std::string &wkt_ = "",
			 const uint timeStep_ = 0,
			 const double dx_ = 0,
			 const double dy_ = 0,
...
#ifdef USE_NEWBASE
    /*
    * Note: These are the currently known magic numbers (from 'NFmiProducerName.h'):
<<
...
<<
    */
    ...
    std::vector<NA_Param>::const_iterator getParamsEnd() const { return params.end(); }
  ...
  private:
    ...
    const std::vector<NA_Param> params;  // scalar params offered to API (host params not listed)
...
/*
* Abstract base class for 'SQD_Data' and 'MQD_Data' (and other possible data adapters)
*/
  ...
  public:
    ...
    */
    ...
    struct ProgressCallback {
        ...
        bool /*cancel*/ progress( unsigned long current, unsigned long total, const char *a=0, const char *b=0, const char *c=0 );
    ...
    //
    // Note: We cannot have the same function name for both a 'virtual' and a non-virtual (for METQU,
    //       providing 'const' behaviour).
    //
#ifdef METQU
    const Matrix *push_NativeMatrix_const( lua_State *L, 
                                const JDay &vt, 
                                const NA_Level &lev, 
                                const NA_Param &p
							    //
    ...
    // is not native)
    //
    // Note: This function is NOT a pure virtual. We offer a default implementation that does projection
    //      using proj4. However, a certain derived data provider may choose to override it, in case
    //      it thinks it can perform projections more effectively (i.e. only read part of the data in)
    //      or if it sports non-proj4 projection strings (s.a. Newbase for SQD data).
    //
    virtual const Matrix *push_Matrix( lua_State *L, 
                                const JDay &vt, 
                                const NA_Level &lev, 
                                const NA_Param &p, 
                                const Projection &proj,
    ...
    // data; pressure levels from hybrid data). Checks availablility of these.
    //
    virtual bool providesPressureLevelsFromHybrid() const = 0;

    /*
    * 22-Sep-2011 PKi: For fetching cross using newbase; overridden by SQD_Data, uses native projection
    *                  and gridsize
    */
    ...
    */
    virtual FmiParameterName mapParameter(const NA_Param &p,bool primary,bool *mapped) const {
        *mapped = false; return (FmiParameterName)0;
    }
