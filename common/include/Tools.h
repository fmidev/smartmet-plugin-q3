...
/*
* Returns the iterator of the (first) matching value, or 'vec.end()' if not there.
*
* Usage:
*   vector<JDay>::const_iterator it= find_it<JDay>( times, vt );
*/
template<typename T>
typename std::vector<T>::const_iterator find_it( const std::vector<T> &vec, const T &value ) {
...
/* 
* NOTE: With 'assert_invariant()' and failing 'pthread' calls, we DON'T want to continue
*       execution because the conditions are so severe. On the other hand, we don't want
*       to throw exception on these cases, because it would burden the C++ prototype
*       declarations unnecessarily. --AKa 4-Sep-2009
*/
#ifndef NDEBUG
...
#define PTHREAD_CALL( func, ... ) \
    { int rc= func( __VA_ARGS__ ); \
      if (rc) { \
        Logger::FATAL( __FILE__, __LINE__, 0, string_fmt("%s failed: %s", #func, pthread_rc2str(rc)) ); \
        abort(); \
    } }
...
class Mutex {
  ...
  private:
    void Lock() { assert(mm); PTHREAD_CALL( pthread_mutex_lock, mm ); }
    void Unlock() { assert(mm); PTHREAD_CALL( pthread_mutex_unlock, mm ); }
...
/*
* By making PThread locks C++ objects we gain automatic release i.e. if an exception
* is thrown within the lifespan of a lock.
*/
class ClaimMutex {
  public:
...
#ifdef UNIX
# include <glob.h>
  const char *glob_fn( const char *fn_mask, glob_t &gbuf, int &gbuf_i, bool is_dir=false );
#else
...
*
* Ref: <http://www.gnu.org/software/libc/manual/html_node/Aligned-Memory-Blocks.html#Aligned-Memory-Blocks>
*      'man posix_memalign'
*/
#ifdef __SSE__
inline float *sse_alloc( size_t bytes ) {
    void *p;
    int rc= posix_memalign( &p, 16, bytes );    // 16 "must be power of two and multiple of sizeof(void*)"
    if (rc!=0) {
        assert(rc==ENOMEM);
...
inline void sse_free( void *block ) {
    free(block);
}
#else
...
class Profiler_ms {
  ...
  public:
    ...
    Profiler_ms( const char *name_=0 ) : cumulative(0), last_start(0), name(name_ ? name_:"") {}
