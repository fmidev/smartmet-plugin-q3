/*
* CONTOUR.H                               Copyright (c) 2010, Ilmatieteen laitos
*/
#ifndef CONTOUR_H
#define CONTOUR_H
...
struct Point {
    ...
    bool operator==( const Point& o ) const { return (fabs(x-o.x)<EPSILON) && (fabs(y-o.y)<EPSILON); }

    double cross_z( const Point &o ) const {
        return x*o.y - y*o.x;
    }
    double dot( const Point &o ) const {
        return x*o.x + y*o.y;
    }
    double norm_pow2() const {
        return x*x + y*y;
    }
...
{
    // '.edge' provided publically; we're all family and only few places need this.
    //    
    bool edge;    // at material edge (or hole edge); do not stroke two such points

    EdgePoint() : Point(), edge(false) {}
    EdgePoint(double x_, double y_, bool edge_=false) : Point(x_,y_), edge(edge_) {}
...
class Contour : public LuaNew<ContourBind> {
  public:
    ...
    void add_point( const EdgePoint &p ) {
        points.push_back(p);
    }
    ...
    void set_offset();  // to be called when all points are there, before pushing the contour to Lua
  ...
  private:
    ...
    unsigned offset;        // offset of the first point ([1]) as seen [1] in Lua (after 'set_offset()' call). 
    ...
    void _INVARIANT( const char *file, unsigned line ) const {
        ...
        if (offset != ((unsigned)-1)) {
            // 'set_offset()' has been called ('points' is not being added to any more, and the contour
            ...
            // If point at 'offset' has 'edge'==false, it must be a closed contour (no edges, at all)
            //
            if (! points[offset].edge) {
                for( unsigned i=0; i<n; i++ ) {     // no need to use 'offset' for this check
                    assert_invariant( !points[i].edge );
                }

            } else {
                // If next point has 'edge'==false, the whole contour is edge only (may be useful
                // for fillings, at times).
                //
            #if 1
            ...
            #else                
                // The point following 'offset' must have 'edge'==false, making 'offset' the beginning
                // of a section.
                //
...
class ContourMatrix {
  public:
    ...
    // 08-Dec-2011 PKi: Return reference to data value; smoothening changes the data
    //
    float &getValue( const MatrixPos &pos ) /*const 08-Dec-2011 PKi*/ { return m[pos]; }
