...
*
* We have some other categories (BUG, TIMING, USAGE, LUA, STAT) that can be mapped on the 'standard'
...
class Logger {
  ...
  public:
    ...
    // A unique id for groupping separate log messages (i.e. all that have to do with particular script) together.
    //
    // This is unique within the one server (servers can be differentiated otherwise). If we need global
    // uniqueness, things like 'libuuid' can be used (giving 128-bit values).
    //
    // Current implementation simply gives the current time (in ms) and checks that the same time is not
    // given twice.
    //
    ...
    static void LOG( enum category cat, const char *file, unsigned line, const unique_t id_, const std::string &msg ) {
    ...
    //
  protected:
    virtual void log( enum category cat, const char *file, unsigned line, const unique_t id_, const char *msg, size_t msg_len ) = 0;

  public:
    static void DEBUG( const char *fn, unsigned line, const unique_t id, const std::string &msg ) { LOG( _DEBUG, fn, line, id, msg ); }
    static void OK( const char *fn, unsigned line, const unique_t id, const std::string &msg ) { LOG( _INFO, fn, line, id, msg ); }
    static void WARNING( const char *fn, unsigned line, const unique_t id, const std::string &msg ) { LOG( _WARNING, fn, line, id, msg ); }
    static void ERROR( const char *fn, unsigned line, const unique_t id, const std::string &msg ) { LOG( _ERROR, fn, line, id, msg ); }
    static void FATAL( const char *fn, unsigned line, const unique_t id, const std::string &msg ) { LOG( _FATAL, fn, line, id, msg ); }
    static void BUG( const char *fn, unsigned line, const unique_t id, const std::string &msg ) { LOG( _BUG, fn, line, id, msg ); }
    static void USAGE( const char *fn, unsigned line, const unique_t id, const std::string &msg ) { LOG( _USAGE, fn, line, id, msg ); }
...
#define LOG_OK( fmt, ... )      Logger::OK(     __FILE__, __LINE__, 0, string_fmt(fmt, __VA_ARGS__) )
...
#define LOG_TIMING( fmt, ... )  Logger::TIMING( __FILE__, __LINE__, 0, string_fmt(fmt, __VA_ARGS__) )
#define LOG_STAT( fmt, ... )    Logger::STAT( __FILE__, __LINE__, 0, string_fmt(fmt, __VA_ARGS__) )
...
class StderrLogger : public Logger {
  public:
    StderrLogger() : Logger() {}
    /*virtual*/ void log( category cat, const char *file, unsigned line, const unique_t id, const char *msg, size_t msg_len );
  private:
};
...
class E_ANY : public std::exception {
  public:
    E_ANY( Logger::category cat, const char *f, unsigned l, const std::string &m, const Logger::unique_t id= 0 )
    ...
    /*virtual*/ const char *what() const throw() {
        ...
        if (!file) {
            ...
            if (buf=="") {
                std::ostringstream o; o << file << ":" << line << ": " << msg;
                buf= o.str();
            }
            return buf.c_str();
        }
    ...
    //
    const char *what_nosource() const throw() {
        return msg.c_str();
    }
...
#define E_LOG_UNEXPECTED_DATA( fmt, ... ) E_UNEXPECTED_DATA( __FILE__, __LINE__, string_fmt(fmt,__VA_ARGS__) )
...
class E_BAD_FILE : public E_ANY {
  public: 
    E_BAD_FILE( const char *f, unsigned l, const std::string &fn, const std::string &what_ ) 
        : E_ANY( Logger::_WARNING, f,l, string_fmt("Unable to read (%s): %s", what_.c_str(), fn.c_str()) ) {}
...
class E_BAD_SIZE : public E_ANY {
  public:
    E_BAD_SIZE( const MatrixPos &a, const MatrixPos &b, const char *describe=0 )
        : E_ANY( string_fmt( "Differing grid sizes %s: %s != %s", describe?describe:"", 
...
class E_READONLY : public E_ANY {
  public: 
    E_READONLY() 
        : E_ANY( "Trying to write to a read only object" ) {}
};
...
class E_NO_MATCH : public E_ANY {
  public: 
    ...
    E_NO_MATCH() : E_ANY( "Param not in data" ) {}  /* used by 'SQD_Matrix' - does not need param identifiers */
};
