...
*
* The MQD format is a further development of FMI SQD data format. It has a human readable
* text mode header and allows 1-to-1 data transfer from SQD and back (on the parts that
* are used by q3, that is - SQD format may have features that q3 does not use, they may
...
*   - ability to have any set of levels (SQD allows one file to only contain one type of 
*       levels, either ground, hybrid or pressure)
*   - ability to store unit names for parameters
*   - ability to have dynamic grid size; each time/level/parameter combination ("tile")
*     may have its own grid resolution (SQD has a file-wide, fixed grid size).
*   - ability to have any parameter grid within the data be compressed, to save disk
*       space. Often used parameters (or levels, or times) may be left uncompressed 
*       while others are compressed. (SQD has no built-in support for compression.
*       Compressing whole SQD files makes accessing them tremendously slow.)
*   - uncompressed float data is stored 16-byte aligned, to allow fast SSE access
*       without need to copy to memory (using file memory mapping)
*   - ability to store data of various kinds (float, uint16, uint8, uint4, bool),
*       per parameter. This (together with compression) enables small file sizes
*       and takes away the need for complicated "merged parameters". (SQD files
*       employ merged parameters to keep file sizes small. This complicates code.) 
*
* TBD: Currently, all data is stored as float. Support for 'uint16', 'uint8', 'uint4'
*       and 'bool' storage types remains to be done.
*
* Revised: 22-Oct-2010
*/
#ifndef MQD_DATA_H
...
class MQD_Data : public NA_Data {
  public:
    ...
    */
    ...
    /*
    * Param tile for each time, level, param combo (stored in order at the beginning of the
    * binary block).
    */
    struct MQD_StoredTile {
      public:
        MQD_StoredTile( MQD_StorageType st, MQD_Method method_, const MatrixPos &gs_, size_t compressed_size_, size_t block_offset );
...
#ifdef METQU
        Tile( NA_Param::e_Datatype dt_, NA_Param::e_Interpolation method_, const Matrix &m,
