  ...
  public:
    ...
    */
    struct FontSpec {
        ...
        FontSpec( const std::string &label_, const Vectors::Vector &size_, const ContourInfo_Line::LabelParams *lp_ )
    ...
    */
    class Drawer {
      public:
        ...
        //
        virtual void mark_candidate( const Vectors::Point &p, double goodness, const char *str=0 ) {}
        virtual void mark_tr( const TiltedRect &tr, const char *str=0 ) {}
        virtual void mark_bb( const BoundingBox &bb, const char *str=0 ) {}
        virtual void mark_vector( const Vectors::PointAndVector &v, const char *str=0 ) {}
    };

    struct Config {
        double limit_along_line;    // min. distance between labels on the same curve (center to center)
        ...
        double dampen_corners_limit_deg;    // corners bigger than this will get dampened
        double dampen_corners;      // damping factor (<1.0) for such corners

        double boost_horizontal_limit_deg;    // lines less than this are taken as horizontal
        double boost_horizontal;        // boost factor (>1.0) for such (horizontal) points
        
        double curve_optimization;    // Optimization fine tuning; affects processing speed
                                    // How big areas of curves are stored as subcurves (chunks); in pixels
        ...
        //
        Config() : limit_along_line(0.0), limit_direct(0.0),
                    allow_overlapping_labels(false),
                    allow_overlapping_lines(false),
                    dampen_corners_limit_deg(8.0),
                    dampen_corners(0.8),
                    boost_horizontal_limit_deg(3.0),
                    boost_horizontal(1.5),
                    curve_optimization(0.0),     // 0 = not used (all one chunk; slooooow)
        			decimals(1)
                    {}
    };

    Labelizer( const Config &config_, Drawer &debug_drawer_, unsigned data_w, unsigned data_h ) 
        : config(config_), current_chain(0), chains(), last_point(), debug_drawer(debug_drawer_)
    ...
    static const Labelizer::Config &loadUserCfg(lua_State *L, int idx, Labelizer::Config &lCfg);
    static const Labelizer::Config &loadCfg(lua_State *L, int idx, Labelizer::Config &lCfg, bool uConfig);
    static int pushLabelizerCfg(lua_State *L);

    // 20-Dec-2011 PKi: Labelizer configuration field table. Must be in sync with Labelizer::Config
    //
    ...
    typedef struct {
        ...
        double *dp;			// NULL or &Labelizer::lblCfg.limit_along_line, ...
        bool *bp;			// NULL or &Labelizer::lblCfg.allow_overlapping_lines, ...
        int *ip;			// NULL or &Labelizer::lblCfg.decimals
    } _configFlds;
...
struct ChosenLabel {
  public:
    ...
    void RejectNearby( const ChainIterator &chosen, double limit_along_line, bool is_closed );

    bool LimitAlongTheLine( const ChainIterator &chosen, double limit, bool is_closed );
    bool LimitDirect( const Point &xy, const std::vector<TiltedRect> &done, double limit );
    bool LimitCrossingLines( const TiltedRect &, const std::vector<Chain*> &chains, const Chain *our_chain );
    bool LimitOverlappingLabels( const TiltedRect &, const std::vector<TiltedRect> &done );
