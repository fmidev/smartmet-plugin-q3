...
#ifndef __SSE__
# error "Not compiled with SSE support (use '-msse...' and/or target 64-bit Linux)"
#endif
...
/*
* These functions handle as much of the matrix they can (0..all), leaving the rest
* to the caller's trivial loop (if 'i'<'n').
*/
...
# define SSE_OP_mf( OP, C, A, SCALAR, I, N ) \
    { \
        ...
        if (a_data) { \
            ...
            \
            const __m128* va= (const __m128*)a_data; \
            const __m128 vv= _mm_set1_ps(SCALAR);  /* { SCALAR, SCALAR, SCALAR, SCALAR } */ \
            __m128* vc= (__m128*)c_data; \
            \
            while( (I)+(32-1) < (N) ) { /* bulk handled here */ \
                *(vc++)= OP( *va++, vv ); *(vc++)= OP( *va++, vv ); \
                *(vc++)= OP( *va++, vv ); *(vc++)= OP( *va++, vv ); \
                *(vc++)= OP( *va++, vv ); *(vc++)= OP( *va++, vv ); \
                *(vc++)= OP( *va++, vv ); *(vc++)= OP( *va++, vv ); \
                I += 32; \
...
*
*   '_mm_movemask_ps()' can be used to scale down the bits for comparison (it takes the MSB of
*   each four, resulting in 0..0x0f range).
*/
# define NO_NANS_8(vb) (0x0f== /*0..0x0f*/ _mm_movemask_ps( _mm_cmpord_ps(*(vb),*((vb)+1)) ))
...
*       reliably. For such pairs, the 'minmax_safe' (either 'max_safe' or 'min_safe')
*       function is used.
*
        ...
        for( ; (I)+(8-1) < (N); I += 8 ) { /* bulk handled here */ \
            if (NO_NANS_8(vb)) { \
                ...
                *vc= OP( *vc, *vb++ ); ++vc; \
                ...
                for( unsigned j=(I); j<(I)+8; j++ ) { C[j]= minmax_safe(C[j],(*B)[j]); } \
                vb += 2; vc += 2; \
            } \
        } \
...
*       parameter.
*
* Used in 'min()' or 'max()' between (one or more) matrices and (one or more) scalars.
* If all the matrices had a NAN (a hole) that hole REMAINS through this process
* (scalars only affect the non-NAN portion of the result).
*/
        ...
        for( ; (I)+(8-1) < (N); I+=8 ) { /* bulk handled here */ \
            if (NO_NANS_8(vc)) { \
                ...
                *vc= OP( *vc, vv ); ++vc; \
                ...
                for( unsigned j=(I); j<(I)+8; j++ ) { C[j]= minmax_keep_nans(C[j],SCALAR); } \
                vc += 2; \
            } \
        } \
    } /* last 0..7 values left for regular loop */
