...
class MemMatrix : public Matrix {
  public:
    ...
    // Non-virtual assignment operator: we use assignment to 'MemMatrix' rather much and this makes
    ...
    // Make a matrix read-only after its initial data has been set. Used i.e. when making
    // interpolations of SQD data; changing such data is conceptually wrong by apps.
    //
    void set_readonly() { is_readonly= true; }

    void scrap_projection() { proj= Projection::NONE; }

    // 10-Jan-2014 PKi: To get and store max data value; the max value is queried repeatedly when contouring.
    //
    float reduce_and_store_max() { if (!maxValue) { maxValue = reduce_max(); } return *maxValue; }

	void set_size(const MatrixPos &gs) { if ((gs.getX() * gs.getY()) > (int) getN()) throw std::runtime_error("Cannot enlarge matrix"); else resize(gs); }
  ...
  private:
    ...
    float * const data;     // read/write array (whose address does not change); SSE aligned (16 byte)
