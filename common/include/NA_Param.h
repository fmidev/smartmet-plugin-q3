...
/*
* Differentiates parameter name and id, plus handles parameter unit (and through it, 
* interpolation mode).
*
* "Native name" means the data about a parameter as presented in the data source
*       (i.e. "Lämpötila:4" for temperature in SQD files)
*
* "Standard name" means the standard name for a parameter. Excludes the id number
...
*   "Paine:1"                       "P"
*   ...
*   ""                              "T"         parameter created by the script (not stored)
*
* Note: The native id's are SPECIFIC TO EACH DATA ADAPTER. Some adapters may use them,
*       some not. For SQD files, i.e. 4 means temperature but for some other data storage
*       it may mean something else. This is why we need standard names in the first place.
*       --AKa 2-Dec-2010
*/
class NA_Param {
  public:
    ...
    enum e_Interpolation {
        ...
        INTERPOLATE_NEAREST,        // take nearest value (i.e. for discrete enum values, 'N','FOG', ...)
    ...
    class Unit {
      public:
        ...
        bool operator==( const Unit &o ) const {
            return (dt == o.dt) && (method == o.method) && (strcmp(unit,o.unit)==0);
        }
        bool operator!=( const Unit &o ) const {
            return ! (*this==o);
        }
      ...
      private:
        ...
        // Note: had segfaults with 'string_or_null' here (when initializing pre-main static
        //       variables).
        //
        /*const*/ char unit[16];
    };
    ...
    static const Unit UNIT_UINT16_NEAREST;   // same but range 0..65534 (65535 = missing value)

    static const Unit UNIT_BOOL;       // 0|1 (>= 2 is missing value)
                                        // no interpolation (value from nearest point)
                                        // data type: byte (or two bits)

    static const Unit UNIT_MAX254_ENUM;  // anything enumerated (with max. range 0..254; 255 = missing value)
                                        // no interpolation (value from nearest point)
                                        // data type: byte

    static const Unit UNIT_MAX14_ENUM;  // same but range 0..14 (15 = missing value)
    ...
    //
    // Note: There's multiple other "normal" units in this category as well (s.a. "m", "km/h")
    ...
    static const Unit UNIT_LAT;        // -90.0 .. 90.0 (units in degrees but no wrap-around)

    static const Unit UNIT_UNKNOWN_;    // no unit, no interpolation
    static const Unit UNIT_UNKNOWN_INTERPOLATABLE;      // result of mathematical operation, unit unknown (linear interpolation)

    NA_Param() : std_name(""), native_name(""), unit(UNIT_UNKNOWN_) { INVARIANT(); }
...
#ifdef USE_NEWBASE
    NA_Param( FmiParameterName e, const std::string & interpolation_name_ = "", const std::string & precision_ = "");
    NA_Param( FmiParameterName e, const std::string &native_name_, const Unit &unit_, const std::string & interpolation_name_ = "", const std::string & precision_ = "");
...
//#endif
  ...
  private:
    ...
    void _INVARIANT( const char *file, unsigned line ) const {
        ...
        //                          (Note: With 'USE_NEWBASE' defined, all params must have native name, at least ':NNN')
        // Just native name:    i.e. "MyCustom:12345" as a custom parameter (note: only SQD requires the id trailing)
...
#  endif
        ...
        if (native_name != "") {
            ...
            assert_invariant( colon && (atoi(colon+1) > 0) );     // always has an id part with integer tail
        }
# else
        (void)file; (void)line;
# endif
