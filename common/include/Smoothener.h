...
/* 
* Define for testing purposes to 'fake' a hole (missing values) within the material.
*/
...
class PointAndEdge : public Vectors::Point {
  private:
    ...
    bool edge;  // 'true' if the point is at edge of a fill path (not to be smoothened)
                // 'false' for all points in a curve path (from 'Tron::line()')
...
class SmoothPath {
  ...
  public:
    SmoothPath()
        : rim(),
        closed(false),  // only known at 'done()'
