...
/*---=== Array2D ===---
*
* Helper template for making a 2D array (of any type) that can be indexed using 'MatrixPos'
* (even negative indices on both X and Y).
*/
template <class T> class Array2D {
  public:
    ...
    //
    const T &operator[] ( const MatrixPos &pos ) const {
        return arr[ index(pos) ];
    }  
    T &operator[] ( const MatrixPos &pos ) {
        return arr[ index(pos) ];
    }  
...
*
* Note: We can rely on 'm' remaining non-GC'ed while we're alive ('new_SubMatrix'
*       takes care of this).
*
...
class SubMatrix : protected Matrix, public LuaNew<SubMatrixBind> {
  public:
    ...
    */
    ...
    //
    /*virtual*/ const Projection &getProjection() const { return Projection::NONE; }
