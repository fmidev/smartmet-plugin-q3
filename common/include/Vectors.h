...
*
*       Only available with Boost >= 1.37; Labelizer code not adopted to it, yet.
*
* Defines:
*       VECTORS_CACHE_NORM  Caches the norm calculations. Don't use this; 
*           tests indicate it actually slows down Q2 plugin.    --AKa 27-Jan-2009
*
    ...
    */
    class Vector {
      ...
      public:
        ...
        Vector( const Point &a, const Point &b ) : dx(b.x-a.x), dy(b.y-a.y), norm_cached(-1.0) {}
#else
    ...
    class BoundingBox {
      public:
        ...
        bool inside_or_at_edge( const BoundingBox &o ) const {
            return (o.lo.x >= lo.x) && (o.hi.x <= hi.x) &&
                   (o.lo.y >= lo.y) && (o.hi.y <= hi.y);
        }
