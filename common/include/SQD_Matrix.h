...
/*
* To ensure coherence between WD and WS (WD must be 0 if WS is missing; 0+..360 if WS is provided)
* and WVEC and WS,WD (WVEC= 100*round(WS)+(WD/10)), we need some extra administrational things.
*
* If this is defined, coherency is checked. If not defined, we keep the parameters unaware of
* each other.
*/
...
class SQD_Matrix : public Matrix {
  ...
  private:
    typedef float (*conv_f)( float );       // conversion function (for 'WD' range transforms)
