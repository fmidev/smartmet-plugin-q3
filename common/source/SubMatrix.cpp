...
SubMatrix::SubMatrix( Matrix& m_, const MatrixPos &ws ) 
    : Matrix( MatrixSize( -ws, ws ), m_.getUnit(), true /*read-only*/ )
    , m(m_)
    , m_pos(0,0)
    , r_km(NAN)
...
int SubMatrix::set_radius( lua_State *L ) {
    ...
    //
    if (isnanf(my.r_km)) {
        if (my.latlon) delete my.latlon;
        my.latlon= 0;
    } else {
        // We take the LONLAT matrix from Lua, since they're most likely having it ready.
        //
        ...
        my.latlon= a;  // storing 'a' in a const pointer (not to be changed any more)
    }
