/*
* VECTORMATRIX.CPP                          Copyright (c) 2009-10, Ilmatieteen laitos
* 
...
/*
* Push the X and Y components of 'b', which is either 'VectorMatrix' or 'Vector'.
* If some other type, return false.
*/
...
/*
* Create a new 'VectorMatrix' object of given size. Component matrices are given on the stack,
* and popped away from the stack. They are used AS-IS by 'VectorMatrix' and no other references
* to them MUST EXIST (the user is not allowed to pass them, or have a copy).
*
* This constructor MUST be called with 'new(L)' operator.
*/
VectorMatrix::VectorMatrix( lua_State *L, bool polar_ ) /*throw(E_BUG)*/
    : ApiMatrix(), LuaNew<VectorMatrixBind>(), m1(0), m2(0), m1_key(0), m2_key(0),
      polar(polar_) { 
...
*/
VectorMatrix::VectorMatrix( lua_State *L, const Matrix &a, const Matrix &b, bool polar_ ) /*throw(E_BUG)*/
    : ApiMatrix(), LuaNew<VectorMatrixBind>(), m1(0), m2(0), m1_key(0), m2_key(0), polar(polar_) { 

    const MatrixSize &size= a.getSize();
    if (size != b.getSize()) {
        throw E_LOG_BUG0( "Cannot construct 'VectorMatrix' from matrices of different size" );
    }
    ...
    // Push copies (not references) of the matrices, bind to them, and remove the first
    // references.
    //
        ...
        case LUA_TSTRING: {
            ...
            if (strcmp(s,"x")==0) {
                ...
                if (mm) {
                    ...
                    //
                    MemMatrix &m= *new(L) MemMatrix( me.getSize(), me.m1->getUnit(), me.getProjection() );
                    offset_t n= m.getN();
                    for( offset_t i=0; i<n; i++ ) { m[i]= me[i].getX(); }
                }
                return 1;
            }
                ...
                if (mm) {
                    ...
                    MemMatrix &m= *new(L) MemMatrix( me.getSize(), me.m1->getUnit(), me.getProjection() );
                    offset_t n= m.getN();
                    for( offset_t i=0; i<n; i++ ) { m[i]= me[i].getY(); }
                }
                return 1;
            }
                ...
                if (mm) {
                    ...
                    MemMatrix &m= *new(L) MemMatrix( me.getSize(), me.m1->getUnit(), me.getProjection() );
                    offset_t n= m.getN();
                    for( offset_t i=0; i<n; i++ ) { m[i]= me[i].getAbs(); }
                }
                return 1;
            }
                ...
                if (mm) {
                    ...
                    MemMatrix &m= *new(L) MemMatrix( me.getSize(), NA_Param::UNIT_DEG, me.getProjection() );
                    offset_t n= m.getN();
                    for( offset_t i=0; i<n; i++ ) { m[i]= me[i].getDeg(); }
                }
                return 1;
            }
...
* m_ud= __mul( any, m_ud )
*
*   VectorMatrix * Matrix:      multiply each vector with the scalar of that position
*   Matrix * VectorMatrix:      -''-
*   VectorMatrix * number:      multiply each vector with the global scalar
*   number * VectorMatrix:      -''-
*
...
*       get 'Matrix * VectorMatrix' and make it behave like 'VectorMatrix * Matrix'.
*/
...
/**
    if (push_cartesian( L, b_index )) {
        // Multiplying with vectors ('VectorMatrix' or 'Vector'), use cartesian coordinates
        //
...
**/
    {
        ...
        MemMatrix *m_v1= new(L) MemMatrix( gs, a->m1->getUnit(), a->getProjection() );
        MemMatrix *m_v2= new(L) MemMatrix( gs, polar ? a->m1->getUnit() : a->m2->getUnit(), a->getProjection() );
...
*
* Turn division into 'a * 1/b' (this is the simplest way of dealing with division,
* and gives the right results).
*
*   VectorMatrix / VectorMatrix|Vector:    divide each vector's components with each other (used by 'grad()')
*   VectorMatrix / Matrix:      divide each vector with the scalar of that position
*   VectorMatrix / number:      divide each vector with the global scalar
*
...
int VectorMatrixBind::tostring( lua_State *L ) {
    ...
    // TBD: If we need the stringification only here, use 'lua_pushfstring()' etc. for doing it.
    //
    stringstream ss;
    me.asString(ss, decs);
...
*/
void VectorMatrix::push_cartesian( lua_State *L, int b_index, bool choose_x ) const {
    L_ASSERT( b_index > 0 );
    ...
    if (!isPolar()) {
        ...
        //
        MemMatrix *m= new(L) MemMatrix( getGridSize(), m1->getUnit() /*abs unit*/, getProjection() );
...
void VectorMatrix::operator=( const VectorMatrix &o ) {
    ...
    // Setting a matrix of vectors with another (but they may be polar/cartesian different)
    //
