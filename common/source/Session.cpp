/*
 * SESSION.CPP                            Copyright (c) 2008-2010, Ilmatieteen laitos
 * 
...
static const string NO_DATA_WITH_MSG( "No data with given dataType, producerId and/or originTime." );
...
//
static /*const*/ void *REG_JSON_FUNC= (void*) &REG_JSON_FUNC;      // address of itself (unique)
...
// Registry keys to fetch prepared Lua side functions
//
/*const*/ void *REG_LATLON_FUNC= (void*) &REG_LATLON_FUNC;   // address of itself (unique)
...
/*
* matrix_ud | vectormatrix_ud = new_ApiMatrix( size_matrixpos_ud, [num|vector], [param_name_str], [projection_str] )
*
* Creates a new 'MemMatrix' or 'VectorMatrix' object.
*
* 'param_name' and value (if vector) decide whether the returned matrix is matrix of scalars ('MemMatrix') 
* or of vectors ('VectorMatrix'), and if vector whether it's stored as polar or cartesian.
*
* Initial value 'nan' guarantees initialization to NANs. With no initial value, the matrices
* may not be initialized (optimization in case the script will write over them).
*
* NOTE: All matrices produced are getting 'UNIT_UNKNOWN' which makes them unsuitable to be used 
*       in interpolations.
*/
static int new_ApiMatrix( lua_State *L ) throw() {
    ...
    if (param_is_2d || v2) {
        ...
        new(L) VectorMatrix( L, polar );    // eat components and take 'param' and 'proj' from them
        return 1;
...
* matrix_ud = new_ScalarMatrix( matrixpos_ud, [Matrix|num|nan], [projection_str], [setgrid] )
*
* Note: If 'num' is missing, the matrix is really NOT NEEDED TO BE INITIALIZED. 
*       The script (not the application script) will fill in so initializing to NAN
*       would be a waste.
*/
static int new_ScalarMatrix( lua_State *L ) {
    ...
    if (m) {
        ...
        if (m) {
            ...
            new(L) MemMatrix( gs, m->getLevel(), m->getParam(), m->getUnit(), proj );    // uninitialized

			if (lua_isboolean(L,4) && lua_toboolean(L,4)) {
	            // Take grid from given matrix. This is to enable "chaining" of PEEKXY's.
	            // Should there be any problems, disable this !
	            //
        ...
        if (!lua_isnumber(L,2)) {
            new(L) MemMatrix( gs, NA_Param::UNIT_UNKNOWN_INTERPOLATABLE, proj );    // uninitialized
        } else {
...
*
* The first upvalue decides if the matrix is made as cartesian ('false', used as 'new_VectorMatrix_xy')
* or polar ('true', used as 'new_VectorMatrix_polar').
*
* If submatrices are provided, their values are copied, so the returned matrix will be
* independent of the params given.
*/
...
*
* Creates a NAN-initialized matrix for storing values like the second parameter.
*
* Note: The sample value is NOT set anywhere in the matrix. It is just used to define, whether
*       the returned matrix is storing scalars, cartesian (xy) vectors or polar vectors.
*/
...
* Creates a new two-value vector (for placing in a matrix).
*
* Note: Values are adjusted to [0,inf) and [0,360) range, by 'Vector' constructor.
*/
...
*
* Note: 'require "fminames"' extends 'latlon' to take in city and geographic names (i.e. 'latlon("Helsinki")').
*/
...
static int distance_km( lua_State *L ) {
    ...
    // Note: Use 'init_from' instead of 'LatLon::instance()' to allow conversion from
...
void q3_bind( lua_State *L ) {
    ...
    // Derivation ambiguities leads us using such syntax (this is still essentially
...
static void INIT_ERROR( lua_State *L, int st, const char *file, unsigned line ) {
    ...
    lua_pushnil(L);     // trying to use as little memory as we can (in case of out-of-memory)
...
int Session::init( lua_State *L ) {
    ...
    */
    ...
    // Must also stamp this state with a unique id (keeps the various log messages together
    // in distributed logs)
    //
    {
#ifndef METQU
    ...
    lua_pushnumber( L, (double)id );       // 'double' has 56-bit accuracy (Lua integers normally 32, even in 64-bit systems)
    ...
    proto_init(L)
        ...
        .set( "jday", LuaNew<JDayBind>::is )    // making the constraint lower case (not "JDay")
    ...
    // [-1]: function returned by 'json.lua', to be used for filtering query return values
    // We'll store it in 'reg[REG_JSON_FUNC]', which is invisible for the Lua side.
    //
    L_ASSERT( lua_isfunction(L,-1) );

    // Also place the function in 'json' global, to be used by 'metqu' launching script.
    //
#ifdef METQU
    ...
    struct {
        const unsigned char *chunk; size_t bytes;
    } chunks[]= {
...
string_or_null Session::result_( lua_State *L, ostream &os, unsigned i ) {
    ...
    if (lua_getmetatable(L,i)) {
        ...
        if (s && (strcmp(s,"Cairo surface")==0)) {
            ...
            if (lua_type(L,-1) != LUA_TFUNCTION) {
                const char *msg= "Problem with Cairo binding: '<cairo surface>.contents' is not a function";
                LOG_BUG0( msg );
                luaL_error( L, msg );
            }
    ...
    switch( lua_type(L,i) ) {
        ...
        case LUA_TTABLE: {
            ...
            // There should not be errors in the JSON conversion. If there are, upper levels
            // will get them (we use 'lua_call()' for performance here, too).
            //
