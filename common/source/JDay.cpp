...
int JDayBind::index( lua_State *L ) {
    ...
    if (s) {
        ...
        if (strcmp(s,"isdst")==0) {
            lua_pushboolean( L, false );    // always UTC (for compatibility with 'os.date()')
            return 1;
        }
...
JDay::jd_T JDay::calc( time_t ticks ) {
    ...
    return calc( 1900+tmp.tm_year, 1+tmp.tm_mon, tmp.tm_mday, tmp.tm_hour, tmp.tm_min, tmp.tm_sec );
}
...
JDay::jd_T JDay::fromstring( const char *s ) {
    ...
    //
    int rc= sscanf( s, "%4d%2d%2d%2d%2d%2d%d", &yyyy, &mm, &dd, &hh, &min, &sec, &dummy );
        //
        // ...
        // 5: YYYYMMDDHHMM
        // 6: YYYYMMDDHHMMSS 
...
*      once per a query, to transform their initial values to 'JDay' for ease of 
*      calculations. However, this is being _also_ called for any subsequent read
*      of validtime/origintime parameters. This is so that changes to the values
*      can be made using the same format as in URL.
*
* Creates a non-valid JDay (test with 'operator bool()') if the entry is not appropriate.
*/
...
unsigned JDay::calc_jdn( int yyyy, int mm, int dd ) {
    ...
    // Julian Day number (integer) based on noon (calculation relies on integer division)
    //
...
unsigned JDay::hour() const {
    return (jd_plus_half / 3600) % 24;
}

unsigned JDay::min() const {
    return (jd_plus_half / 60) % 60;
}

unsigned JDay::sec() const {
    return jd_plus_half % 60;
}
...
unsigned JDay::wday() const {
    ...
    //
    // To give our kind of weekdays, we simply DO NOT do the subtraction of half (43200).
    //
    return (((jd_plus_half/86400)+1) % 7) +1;   // 1..7 (Sun..Sat); akin to Lua 'os.date()' :)
}
...
unsigned JDay::yday() const {
    ...
    //
    // Note: Likewise in 'wday()' we DO NOT subtract the hour/minute/sec correction; this
    //      compensates for our days starting at midnight and JD's at noon.
    //
    return (jd_plus_half/86400) - calc_jdn( yyyy, 1, 1 ) +1;     // 1..366 (365 on regular years)
}

/*
* Note: This only gets years >= AD 1 (but could be changed/tested for earlier years, if needed).
*
* Months are given as 1..12 (Jan..Dec).
* Days are given as 1..28|29|30|31.
*/
void JDay::gregorian( unsigned *year_ptr, unsigned *month_ptr, unsigned *day_ptr ) const {
    ...
    if (month_ptr || year_ptr) {
        ...
        if (month_ptr) *month_ptr= month_;
        if (year_ptr) *year_ptr= (month_ > 2) ? C-4716 : C-4715;
    }
}
...
*
* If covered (returns 'true'), 'exact' is set to tell whether the time was precisely there, or interpolated.
*/
bool JDay::covered_by( const vector<JDay> &times, bool &exact ) const {

    if (!*this) return false;   // time is unknown = not covered
...
//cerr << "!!" << a.wday() << " " << a.yday() << " " << string_fmt("%.6f",a._getJD_debug()) << endl;
...
//cerr << "!!" << aa.wday() << " " << aa.yday() << " " << string_fmt("%.6f",aa._getJD_debug()) << endl;
