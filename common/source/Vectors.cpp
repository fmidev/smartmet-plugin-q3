...
void Point::operator+=( const Vector &v ) { x+=v.get_dx(); y+=v.get_dy(); }
void Point::operator-=( const Vector &v ) { x-=v.get_dx(); y-=v.get_dy(); }

Point Point::operator+ ( const Vector &v ) const { return Point(x+v.get_dx(), y+v.get_dy()); }
Point Point::operator- ( const Vector &v ) const { return Point(x-v.get_dx(), y-v.get_dy()); }
...
static inline bool detached( double a, double da, double b, double db ) {
    ...
    if (da>=0.0) { a_min= a; a_max= a+da; }
    else { a_min= a+da; a_max= a; }

    return ( ((b<a_min) && (b+db<a_min)) || ((b>a_max) && (b+db>a_max)) );
}
...
* of its end points.
*
* This is used INTERNALLY ONLY in cases where the two lines (with their extensions)
* are known to be same.
*/
bool PointAndVector::judge_by_projection( const PointAndVector &other ) const {
    ...
    return ( ((dot1>=0.0) || (dot2>=0.0)) && ((dot1<=_norm) || (dot2<=_norm)) );
}
...
bool PointAndVector::intersects( const PointAndVector &other ) const {
    ...
    */
    ...
    /* 
    * Calculate crossing of 'other' (or its extension) on our line (or its extension)
    */
        ...
        if (fabs(o_dx) < Point::epsilon) {
            ...
            */
            if (within( p.x, other.p.x, o_dx )) {
                double y= other.p.y + (p.x - other.p.x) * (o_dy/o_dx);  // where 'other' crosses this line (p.x, y)
    ...
    */
    ...
    if ( ((z1<0.0) && (z2<0.0)) || ((z1>_norm) || (z2>_norm)) )
        ...
        double divident=
            (other.p.y - p.y) * dx * o_dx 
            + p.x * o_dx * dy - other.p.x * dx * o_dy;
...
*
* Use <http://www.math.jyu.fi/ylemat/opetusmateriaalia/havainnollistuksia/vektorilaskentaa/30_pistetulo.html>
* to play with vectors interactively.
*/
#ifdef SELFTEST_ENABLE
static void _assert_eps( double a, double b, unsigned line ) {
    if ( fabs(a-b) >= Point::epsilon ) {
        throw runtime_error( LOG_BUG( "Selftest failed on line %d (%lf != %lf)\n", line, a,b ) );
    }
}
