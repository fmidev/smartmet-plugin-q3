/*
* MQD_DATA.CPP                          Copyright (c) 2009-10, Ilmatieteen laitos
*
* Revised: 22-Oct-2010
*/
#ifdef MQD_ENABLED
...
*
* This is used for getting a full 16-byte aligned (4 floats) breadth for SSE operations.
* Data tiles are guaranteed to have the extra bytes (if needed) available after their
* actual data.
*/
#if 0   //def METQU
...
#endif
{
    ...
    if (len_actual < len_expected) {
        throw E_LOG_BAD_FILE( fn, string_fmt( "Partial file: length %ul < %u", len_actual, len_expected ));
    }
...
MQD_Data::MQD_Data( const NA_Info &info ) 
    : NA_Data(info), 
      mm(0),    // not associated to a file
      tiles( 0 /* TBD sse_alloc( getBinaryLength() ) */ ),
      is_readonly(false) {
...
/*
    // Get the right address from const 'getTile()' and then cast it to writable
    //
    return const_cast<const MQD_Data*>(this)->getTile( vt, lev, p );   // throws E_NO_MATCH
*/
}
#endif
...
void MQD_Data::output( ostream &out, ProgressCallback *cb ) const {
    ...
    out << "--MQD 1.0\n"    // MUST BE LIKE THIS (used for format detection)
        ...
        "--\n"
        "origintime= " << getOriginTime().toString() << "\n"
        "times= " << getTimes() << "\n"
        "levels= " << getLevels() << "\n"
        "params= " << params << "\n"
        "projection= " << getProjection() << "\n"
    "\n"
    "-- End of header portion. Rest of the file is binary and needs to remain properly aligned.\n";
    
    // Newline after '^D^Z' is important to let Lua 'f:lines()' read the end mark as a line
    // of its own.
    //
    // Writing is padded (with spaces) so that the location after '\n' is 16 byte aligned.
    //
    size_t pos= ((size_t)out.tellp()) + sizeof(CTRLD_CTRLZ) + 1;  // without padding
...
NA_Info MQD_Data::read_info( const char *fn ) { 
    ...
    //
    int st= lua_pcall( L, 0 /*args*/, 1 /*return values*/, 0 /*no error func needed*/ );
        //
        // 0 (ok)
    ...
    st= lua_pcall( L, 2 /*args*/, 2 /*return values*/, 0 /*no error func needed*/ );
        //
        // 0 (ok)
        // LUA_ERRRUN: runtime error
        // LUA_ERRMEM: no memory
    ...
    // Levels have been output using 'NA_Level' names, so we'll also read them in that way.
    ...
    while( lua_next(L,table_idx) ) {
        ...
        if (!key) {
            throw E_LOG_BAD_FILE( fn, string_fmt("Bad header: %s as key (expected string)", L_typename(-2) ) );
        }
...
//LOG_DEBUG( "%s: %s", key, s );
        ...
        try {
            if (strcmp(key,"version")==0) {
                ...
                ot= JDay(L,-1);
                if (!ot) {
                    // Don't use 'luaL_error'; we've crafted the state and it wouldn't lead anywhere (good)
                    //
                    throw E_LOG_BAD_FILE( fn, string_fmt("Bad origintime: %s", L_string_or_typename(-1)) );
                }
            }
            ...
            // Convert problems reported in individual header fields (E_USAGE) into E_BAD_FILE
            //
...
/*
* Returns the number of bytes the binary part is _supposed_ to be taking (it is not confirmed
* that the file is that long; maybe we should do that, and look for some 'completeness' tags
