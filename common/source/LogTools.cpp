...
/*
* Conversion functions required because we cannot read many headers in 'Q3Log.h'
* (it is one of the most elementary headers itself).
*/
string E_ANY::to_str( const MatrixSize &sz ) {
    return sz.asString();
}

string E_ANY::to_str( const MatrixPos &pos ) {
    return pos.asString();
}

string E_ANY::to_str( const JDay &vt ) {
    return vt.toString();
}

string E_ANY::to_str( const NA_Level &lev ) {
    return lev.toString();
}
...
void Logger::init( Logger *logger_ ) {
    ...
    // We don't allow changing a logger once it's set. If we do, we need locks in use of logger
...
*       be disabled.
*
* NOTE: Do NOT limit the length of the logged string in any way. DUMP() uses this
*       and can generate rather long string for debugging purposes. DO NOT pass
*       anything through 'string_fmt' that has an arbitrary (truncating) buffer 
*       limit.
*
* Upvalues: #1 if 'true', reports one level higher than normal (used by 'DUMP()')
*/
int Logger::LOG_( lua_State *L ) {
    ...
    unique_t id= (unique_t)lua_tonumber(L,-1);    // double has 56-bit accuracy (Lua integers are normally 32-bit only)

    if (!id) {
        // Do NOT bug about this. Can occur i.e. if using 'LOG' in 'proto.lua' initialization.
        //
    ...
    //---
    static const void *REG_LOG_timings= (void*)Logger::LOG_;     // Use the function pointer as register key
    ...
    if (lua_istable(L,1) && (lua_gettop(L)==1)) {
        ...
        if (!lua_isboolean(L,-1)) {
            luaL_error( L, "Bad LOG call, expected LOG{ timing=true/false } or LOG(...)" );
        }
    ...
    L_GROW( 1+lua_gettop(L) );      // we essentially double the entries, by adding '\t' in between
...
Logger::unique_t Logger::gen_unique() {
    ...
    { ClaimMutex lock(my_m);
        if (last>=ms) {
            ms= last+1;     // Taking up such time (it may be slightly in the future, no problem)
        }
        last= ms;
    }
...
void StderrLogger::log( enum category cat, const char *file, unsigned line, const unique_t id_, const char *msg, size_t msg_len ) {
    ...
    switch(cat) {
        ...
        case _ERROR:     ansi= _ANSI_RED("ERROR"); break;
        case _FATAL:     ansi= _ANSI_RED("FATAL"); break;
        case _DEBUG:     ansi= "DEBUG"; break;
        case _LUA:       ansi= _ANSI_YELLOW("LUA"); break;
        case _BUG:       ansi= _ANSI_RED("BUG"); break;
        case _USAGE:     ansi= _ANSI_YELLOW("USAGE"); break;
        case _TIMING:    ansi= _ANSI_YELLOW("TIMING"); break;
        case _STAT:      ansi= _ANSI_YELLOW("STAT"); break;
        case _MAINTENANCE: ansi= _ANSI_YELLOW("MAINTENANCE"); break;
    }

    // NOTE: Do NOT use 'string_fmt()' on 'msg' - it may be REALLY LONG (i.e. from script 'DUMP()') and
