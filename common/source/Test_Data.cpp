...
* Init 'my_info'
*/
static struct JustOnce_Test_Data {  // note: must have unique name (otherwise runtime problems, linker mixes the two structs)
  JustOnce_Test_Data() { 
    ...
    // Note: Cannot use 'Projection::NONE' here. Order of initialization of C++ globals is not defined; 'Projection::NONE'
    //       constructor may not have been run before us.
    //
    Projection pr_empty;      // empty projection

    my_info= new NA_Info( 0, 0, jd, times, nativeLevelType, levels, params, pr_empty );
  }
} just_for_init;

/*
*/
Test_Data::Test_Data() throw(E_BAD_FILE) /*not really, but prototype requires as to...*/ : NA_Data(*my_info) {
...
/*
*/
const Matrix *Test_Data::push_TestMatrix( lua_State *L, 
                                const JDay &vt, 
                                const NA_Level &lev, 
                                const NA_Param &p,
...
//LOG_DEBUG( "Lev: %d %lf", (int)lev.getType(), lev.getValue() );
    ...
    // Make factors for multiplication (faster than division; like speed would matter for test features, eh?)
    //
    ...
    MemMatrix *m= new(L) MemMatrix( gs, NA_Param::UNIT_UNKNOWN_INTERPOLATABLE, "" /*projection*/ );
