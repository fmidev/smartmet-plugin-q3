...
static int TestRising( NFmiFastQueryInfo &fi, FmiParameterName id, const char *fn_debug ) {
    ...
    // Loop through as much of the data until we get two values in the same time,location (on different levels).
    // There may be missing values between such two values; does not matter.
    //
            ...
            for( fi.ResetLevel(); fi.NextLevel(); ) {
                ...
                */                
                ...
                if (n >= HEIGHT_POLLING_GIVING_UP_LIMIT) {
                    LOG_WARNING( "GIVING UP parameter orientation detection. Over %d missing values for param %d.", n, (int)id );
                    return 0;
                }
#endif
#if 0
...
#else
                (void)lt; (void)lv; (void)n;
#endif
                if (a != kFloatMissing) {
                    v[++found]= a;
                    if (found==2) {
                        if (v[1]==v[0]) {   // heights should not be the same; if they are, continue looking
                            found--;
                        } else {
                            return (v[1]>v[0]) ? +1 : -1;
                        }
	...
	return 0;     // There were no two values in same time,location *anywhere* in the data - orientation does not matter
}
...
*/
void SQD_Data::setIter_exact( NFmiFastQueryInfo &fi, const JDay &vt, const NA_Level &lev, const NA_Param &p ) throw() {
...
/*
* Returns the levels available, in an order "from the ground up" (height param ascending, pressure descending)
*
* Throws E_BAD_FILE on file conformance issues.
*/
static vector<NA_Level> getLevels( const NFmiQueryInfo &info, const char *fn_debug, bool &revorder, string & nativeLevelType ) throw(E_BAD_FILE) {
    vector<NA_Level> vec;
    ...
    while( fi.NextLevel() ) {
        ...
        if ((lt!=kFmiGroundSurface) && (lt!=kFmiHybridLevel) && (lt!=kFmiPressureLevel) && (lt!=kFmiSoundingLevel)) {
    ...
    //---
    // Rule #1. If the file has pressure levels (then it ONLY has pressure levels in SQD), they should be given in
...
REVERSE:
			...
			//				    (otherwise data will be copied between unequal levels); inform about reversed order
...
static std::string interpolation_name( FmiInterpolationMethod method )
{
  switch(method)
	{
	case kNoneInterpolation: return "None";
	case kLinearly: return "Linear";
	case kNearestPoint: return "NearestPoint";
	case kByCombinedParam: return "ByCombinedParam";
	case kLinearlyFast: return "LinearlyFast";
	case kLagrange: return "Lagrange";
...
//                break;
        ...
        assert( e );
        NA_Param param( show_id ? e : (FmiParameterName)0, name.c_str(), SQD_Tools::unit_by_id(e), interpolation_name(id.GetParam()->InterpolationMethod()), id.GetParam()->Precision().CharPtr() );
...
#endif
#endif
    for (map<string,FmiParameterName>::const_iterator it = virtualParams.begin(); (it != virtualParams.end()); it++)
...
NFmiQueryData *SQD_Data::new_qd( const NA_Info &info, const MatrixPos &gs ) throw(E_USAGE) {
    ...
    for( vector<JDay>::const_iterator it= times.begin();
        it != times.end();
        ++it ) {
...
MatrixPos SQD_Data::getGridSize(size_t nPoints) const {
	...
	//					was given instead, the matrix will later be resized to the number of data points
	//					within the given area
	//
	return MatrixPos( qd->Info()->SizeLevels() * ((nPoints > 0) ? nPoints : qd->Info()->SizeLocations()), 1 );
}
...
) CONST_IF_SERVER throw(/*E_BUG*/) {
    CONST_IF_SERVER Matrix *m;

    //---
    // Special case: if looking for parameter 'Z' (with no id) look first at ':2' (kFmiGeopHeight) and then
    // ':3' (kFmiGeomHeight) transparently to the user. Newbase does similar.
    //
    // Note: Currently this processing is ONLY within the 'SQD_Data' adapter, thus not being applied to
    //      i.e. MQD data.
    //
    // Note: Vili was surprised to hear of such. It seems not to be a generally known approach, and is
    //      potentially risky. We'll place this in the configuration file for now (disabled by default).
    //
    // Note: Parameters ':2' and ':3' actually have different units ("gpm" and "m"). This further complicates
...
//#ifdef SQD_CASCADE_Z_ENABLED
//    if (p.toString() == "Z") {
//        m= push_NativeMatrix_e( L, vt, lev, kFmiGeopHeight, target_proj, target_gs, target_ready );     // id 2
//        if (!m) {
//            m= push_NativeMatrix_e( L, vt, lev, kFmiGeomHeight, target_proj, target_gs, target_ready );     // id 3
//        }
//    } else
//#endif
//    {
//        m= push_NativeMatrix_e( L, vt, lev, SQD_Tools::param_id(p), target_proj, target_gs, target_ready );
//    }

    bool mapped = false;

	// Try p or the primary parameter if available
    m= push_NativeMatrix_e( L, vt, lev, mapParameter(p,true,&mapped), target_proj, target_gs, target_ready, dataIds );

    if ((!m) && mapped)
    	// Try the secondary parameter
        m= push_NativeMatrix_e( L, vt, lev, mapParameter(p,false), target_proj, target_gs, target_ready, dataIds );

    return m;
}

/*
* Get data for given data point id's or data points within given area. The data is returned as [nPoints,1] matrix
*/
void pointValues(NFmiFastQueryInfo & info,const NFmiAreaFactory::return_type area,const DataIdList *dataIds,bool retMissing,MemMatrix &m,bool soundingData)
{
    MatrixPos::offset_t n = 0;
    ...
    bool getSoundingPressureValues = ((!retMissing) && soundingData && (queryParam.GetParamIdent() == kFmiPressure));
		...
		if (dataIds) {
			for (info.ResetLevel(); info.NextLevel(); ) {
				for (DataIdList::const_iterator it = dataIds->begin(); (it != dataIds->end()); it++) {
					...
					if ((!retMissing) && ((!soundingData) || (*itPressure != kFloatMissing)) && info.Location(*it))
						v = info.FloatValue();
...
/*
* TBD:  Current handling is inefficient. Scripts regularily ask for say 50,60 grid of interpolated values. 
*       We should calculate such right on, without pushing native resolution interpolation (s.a. 500,600). 
*/
CONST_IF_SERVER
Matrix *SQD_Data::push_NativeMatrix_e( lua_State *L, const JDay &vt, const NA_Level &lev, FmiParameterName e
  //
  // 22-Aug-2011 PKi: For fetching data using newbase interpolation, projection and scaling
  //
, const Projection *target_proj
...
, const DataIdList *dataIds_
    ...
    bool exact_level= ((lt != NA_Level::HEIGHT_LEVEL) && ((!lev) || lev.isGroundLevel() || lev.isSoundingLevel() ||
    ...
    // 16-Apr-2015 PKi: Store origintime of the first used querydata as a lua global; it will be returned in response headers
	//
    ...
    MemMatrix *m_ = retMissing
    				? L ? new(L) MemMatrix( native_gs, NAN, lev, e, unit, target_proj ? *target_proj : native_proj )
    					: ::new MemMatrix( native_gs, NAN, lev, e, unit, target_proj ? *target_proj : native_proj )
					: L ? new(L) MemMatrix( (!pointData) && target_gs ? *target_gs : native_gs, lev, e, unit, target_proj ? *target_proj : native_proj )
    ...
    if (! (same_proj && same_gs))
    {
    	...
    	//
    	// Set pr to projection to be used if area is created using NFmiAreaFactory (when nonnative projection
    	// or getting/cloning area should fail)
	...
	auto areaPtr = ((! (same_proj && same_gs)) && (! area)) ? NFmiAreaFactory::Create(pr->toString()) : (NFmiAreaFactory::return_type) area;

	if (pointData) {
	    // 07-Apr-2015 PKi: Return point data within the given area
		//
		pointValues(fi,areaPtr,dataIds,retMissing,*m_,levels_[0].getType() == NA_Level::SOUNDING_LEVEL);
	}
	else {
	// Attach grid; needed by PEEKXY
	//
	...
	std::shared_ptr<NFmiGrid> wantedGrid(new NFmiGrid(areaPtr.get(), m_->getSize().getXS(), m_->getSize().getYS()));
...
//			NFmiGrid wantedGrid(areaPtr.get(), m_->getSize().getXS(), m_->getSize().getYS());
        ...
        if(! (fi.PressureValueAvailable()           // Data has pressure parameter (useful for pressure level calculation)
              || fi.PressureLevelDataAvailable()    // Data has pressure levels as such (can be interpolated from)
            )) {
        ...
        for( fi.ResetLocation(); fi.NextLocation(); ) {
            ...
            // Note: Seems the 'fi.Time()' setting above does not cause time interpolation for '::PressureLevelValue()'.
            ...
            // There's a bug in Newbase '::PressureLevelValue()'. If it does not find the right pressure level,
            // it changes the current parameter to P (pressure). If we got NAN, we must reset the param to what
            // we wanted. (No way for us to know if the NAN was because of the "give up" condition or because 
            // of genuinly missing data only on that particular position).  --AKa 21-Dec-2010
            //
...
{
	// Times should be nonzero and in rising order. Use t1_ (the converted value of j2 in previous call)
	// if it is available
...
/*virtual*/ CONST_IF_SERVER Matrix *SQD_Data::push_NativeCross( lua_State *L,
								                              ...
								                              ) const
{
    ...
    bool setOk = fi.Param(e = mapParameter(p,true,&mapped));	// Try p or the primary parameter if available
    if (! setOk)
    {
    	if (mapped)
    ...
    // TimeCrossSectionValues or CrossSectionValues are used respectively instead of RouteCrossSectionValues

    NFmiDataMatrix<float> nm;

    checkedVector<float> pressures_or_heights;
    checkedVector<float> &pressures = pressures_or_heights,&heights = pressures_or_heights;
    checkedVector<NFmiLevel> hLevels;
    unsigned int nLevels = levelVec.size(),nLocs = locs.size(),nTimes = vtVec.size(),i;
    time_t t1 = 0,tDiff = 0;
    bool timeCross = ((!flightRoute) && (nLocs == 1) && (nTimes > 1) && timeDiff(vtVec[0],vtVec[1],tDiff,t1,NewBaseMinTimeStepInSecs));
    ...
    if (timeCross)
    {
        NFmiTimeBag validTimes(SQD_Tools::jd2mt(vtVec[0]),SQD_Tools::jd2mt(vtVec[nTimes-1]),tDiff / NewBaseMinTimeStepInSecs);

//    	if (ground)
//    		fi.TimeCrossSectionValuesGround( nm, NFmiPoint(locs[0].getLon(), locs[0].getLat()), validTimes );
//    	else
    	if (hybrid)
			fi.TimeCrossSectionValuesHybrid( nm, hLevels, NFmiPoint(locs[0].getLon(), locs[0].getLat()), validTimes );
    	else if (height)
			fi.TimeCrossSectionValues( nm, heights, NFmiPoint(locs[0].getLon(), locs[0].getLat()), validTimes );
    	else
    		fi.TimeCrossSectionValuesLogP( nm, pressures, NFmiPoint(locs[0].getLon(), locs[0].getLat()), validTimes );
    }
    else
    {
    	checkedVector<NFmiMetTime> validTimes;
    	checkedVector<NFmiPoint> locations;
	...
	//      	fi.RouteCrossSectionValuesGround( nm, locations, validTimes );
			...
			for (i = 0; (i < nTimes); i++) validTimes.push_back(SQD_Tools::jd2mt(vtVec[i]));
			for (i = 0; (i < nLocs); i++) locations.push_back(NFmiPoint(locs[i].getLon(),locs[i].getLat()));
			...
			if (nm.NX() == 0)
		        throw E_LOG_USAGE0( "FlightRouteValues() returned no data; must have equal number of locations, levels and times" );
...
/*virtual*/ void SQD_Data::getLocations(vector<LatLon> &locations,const Projection *target_proj) const
{
    NFmiFastQueryInfo fi(qd);
...
/*virtual*/ void SQD_Data::getDataIds(std::vector<unsigned long> &dataIds,const Projection *target_proj) const
{
    NFmiFastQueryInfo fi(qd);
...
/*virtual*/ void SQD_Data::getDataNames(vector<string> &dataNames,const Projection *target_proj) const
{
    NFmiFastQueryInfo fi(qd);
	...
	if (target_proj) {
		...
		for (fi.ResetLocation(); fi.NextLocation(); ) {
			...
			if (area->IsInside(p))
				dataNames.push_back(string(reinterpret_cast<const char*> (fi.Location()->GetName().GetCharPtr())));
		}
	}
    else
		for (fi.ResetLocation(); fi.NextLocation(); )
...
*
*                  Sets 'mapped' to true and returns the id of primary or secondary parameter as requested
*                  if the input parameter 'p' is one of the mapped parameters. Otherwise 'mapped' is set
...
void SQD_Data::mapParameter( const FmiParameterName id, std::map<std::string,FmiParameterName> &virtualParams )
{
