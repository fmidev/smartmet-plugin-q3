/*
 * SQD_Data.CPP                          Copyright (c) 2009-10, Ilmatieteen
 * laitos
 *
 * Revised:  22-Oct-2010
 */
...
/*
 * If we need to poll data for height (ids 2,3) or pressure (id 1) params, this
 * is how many NANs are tolerated before turning our back. Ideally, if there are
 * such params, they should have valid values as well.
 *
...
/*
 * Define if we know 1..n indexing of matrices follows the same rules (y
 * outermost, bottom to up, x innermost, left to right) as Newbase does.
 */
#define SQD_AND_Q3_INDEXING_ARE_THE_SAME

/*
 * Newbase 'NFmiFastQueryInfo::PressureLevelValue()' can change the current
 * parameter if requested pressure level is not available. This is a bug in
 * Newbase.
 */
 ...
 *       (tried earlier but never got them working fine - maybe another try
 * together with Marko?) 26-Oct-10 AKa
 */
static int TestRising(NFmiFastQueryInfo &fi, FmiParameterName id,
                      const char *fn_debug) {
  (void)fn_debug;
  ...
  // Loop through as much of the data until we get two values in the same
  // time,location (on different levels). There may be missing values between
  // such two values; does not matter.
  //
      ...
      */
      ...
      for (fi.ResetLevel(); fi.NextLevel();) {
...
#endif
        /* NOTE: Must be repeated, '.NextLevel()' may have changed the param
         * setting.
         */
        fi.Param(id);
        ...
        if (n >= HEIGHT_POLLING_GIVING_UP_LIMIT) {
          LOG_DEBUG("GIVING UP parameter orientation detection. Over %d "
                    "missing values for param %d.",
                    n, (int)id);
          return 0;
        }
#endif
  ...
  LOG_WARNING("Material with COMPLETELY MISSING param %d values. ('%s')",
              (int)id, fn_debug);

  return 0; // There were no two values in same time,location *anywhere* in the
            // data - orientation does not matter
}

/*
 * Is a parameter part of the 326 (WeatherAndCloudiness) or 19 (TotalWindMS)
 * combo.
 *
 ...
 *
 * NOTE: Expecting trouble with 'NFmiFastQueryInfo' copy constructor, so
 * changing 'fi' as reference.
 */
void SQD_Data::setIter(NFmiFastQueryInfo &fi, const JDay &vt,
  ...
  // This checks the validity of the level (that if ground level is required,
  // data only has one; that if interpolation is required the data actually can
  // provide such a level).
  //
  bool exact;
  ...
  // Note: This must be *after* setting the level (otherwise parameter may get
  // changed, i.e. to 'P'
...
void SQD_Data::setIter_exact(NFmiFastQueryInfo &fi, const JDay &vt,
                             const NA_Level &lev, const NA_Param &p) noexcept {
  ...
  try {
    ...
    throw E_LOG_BUG("Supposed to have had exact time, level and param: %s",
                    e.what());
  }
}
  ...
  while (info.NextTime()) {
    ...
    if (vt_last && (vt < vt_last)) {
      throw E_LOG_BUG("SQD data had times in non-ascending order: %s > %s",
                      vt_last.toString().c_str(), vt.toString().c_str());
    }
...
/*
 * Returns the levels available, in an order "from the ground up" (height param
 * ascending, pressure descending)
 *
...
static vector<NA_Level> getLevels(const NFmiQueryInfo &info,
  ...
  while (fi.NextLevel()) {
    ...
    FmiLevelType lt = nlev->LevelType();
    if (nativeLevelType.empty())
      nativeLevelType = boost::lexical_cast<string>((int)lt);
    long lv = (long)nlev->LevelValue(); // is 'long' for Newbase heritage

    any_level_found = true;

    if ((lt != kFmiGroundSurface) && (lt != kFmiHybridLevel) &&
        (lt != kFmiPressureLevel) && (lt != kFmiSoundingLevel)) {
  ...
  //---
  // Rule #0. Having just one level (MUST be ground level) - order does not
  // matter
  //
  // Order does not matter for sounding data either, we'll just return the data
  // as is
  //
  ...
  //---
  // Rule #1. If the file has pressure levels (then it ONLY has pressure levels
  // in SQD), they should be given in
  //          order from the highest to lowest (since pressure at ground level
  //          is highest).
  //
  if (vec[0].isPressureLevel()) {
    ...
    REVERSE:
      ...
      //
      // 31-Aug-2011 PKi: Must use original level order when creating a raw
      // object using template
      //				    (otherwise data will be copied
      //between unequal levels);
  ...
  if (p2_order * p3_order < 0) {
    throw E_LOG_BAD_FILE(fn_debug,
                         "Params 2 and 3 grow in different directions!");
  }
...
void addParam(vector<NA_Param> &vec, NFmiQueryInfo &info, FmiParameterName e) {
  ...
  if (info.Param(e)) {
    interpolation =
        interpolation_name(info.Param().GetParam()->InterpolationMethod());
...
uint getDataTimeStep(NFmiQueryInfo &info) {
  ...
  if (info.FirstTime()) {
    ...
    timeStep =
        (info.NextTime() ? info.ValidTime().DifferenceInMinutes(t1) : 60);
 ...
 *
 * Note: Only parameters that are actually accessible are listed, the "host"
 * params
 *       ":326" and ":19" are not. They are indicated by 'has_326' and 'has_19'
 * flags.
 *
 * Actual parameter names as stored in the SQD file are returned (i.e.
 * "Lämpötila:4"). For derived params, only the param names are returned (i.e.
 * "WD","WS,...).
 */
static vector<NA_Param> getParams(NFmiQueryInfo &info, bool &has_326,
                                  bool &has_19) {
  vector<NA_Param> vec;
...
//                  instead of coding parameterwise
   ...
   *
   * NOTE: SQD files are supposed to NOT have the same parameter as
   * 'stand-alone' (i.e. "WD:20") and combo (":19"), in the same file. We cannot
   * detect this situation by Newbase API, anyhow. Just trust it does not
   * happen, or that the behaviour is then undetermined (either the combo or the
   * stand-alone gets referred to, but we don't know, which one).
   */
  ...
  while (info.NextParam()) {
    ...
    case kFmiWindVectorMS:      // WVEC
      ...
      // Note: There's yet more disturbance by the subtle rules of "how Newbase
      // does it".
      //      Looking for param "Z" in Newbase matches either #2
      //      ('kFmiGeopHeight') or #3
      //      ('kFmiGeomHeight'), in this order.
      //
      //      If we find either #2 or #3 we also add "Z" (with no id mentioned)
      //      to the list of available params.
      //
      //      TBD: Should this be in addition to the #2 and/or #3. Maybe. But if
      //      there is
      //          a param "Z:2" (or "Z:anyid") the q3 rules will use that
      //          instead of the #2->#3 cascade chain. Confusing. Unnecessarily
      //          complex. Newbase is. Over and over...  *siiiigh**
      //
      //          Thing should be kept Simple. Newbase fails to do that. Always.
      //          And again.
      //
      // 18-Oct-2011 PKi: If the data has #43 (primary) or #39 (secondary)
      // adding "W:xxx"
      //                  (with generated id) to the list of available params.
      //                  Z has similar logic; "Z:xxx" is added if the data has
      //                  #2 (primary) or #3 (secondary).
      //
      // 31-Oct-2011 PKi: Parameter mapping for virtual parameters handled now
      // through virtual_params[]
      //                  instead of coding parameterwise
#if 0
#ifdef SQD_CASCADE_Z_ENABLED
//            case kFmiGeomHeight:
...
//                break;
    ...
    assert(e);
    NA_Param param(show_id ? e : (FmiParameterName)0, name.c_str(),
...
//
  ...
  for (map<string, FmiParameterName>::const_iterator it = virtualParams.begin();
       (it != virtualParams.end()); it++)
    vec.push_back(
        NA_Param(it->first.c_str(), NA_Param::UNIT_UNKNOWN_INTERPOLATABLE));
...
#ifdef METQU
SQD_Data::SQD_Data(const NA_Info &info) noexcept
    : NA_Data(info), qd(new_qd(info, info.getExtra_sqd_gridsize())),
...
NFmiQueryData *SQD_Data::new_qd(const NA_Info &info,
  ...
  for (vector<JDay>::const_iterator it = times.begin(); it != times.end();
       ++it) {
    NFmiMetTime mt = SQD_Tools::jd2mt(*it);

    // NOTE: '.Add()' insists in taking a pointer and it REALLY NEEDS TO BE A
    // NEW
    ...
    //
    tlist.Add(new NFmiMetTime(mt), false /*no duplicates*/,
              false /*insert in time order*/);
  }
  ...
  if (!levels.front().isGroundLevel()) {
    for (vector<NA_Level>::const_iterator it = levels.begin();
         it != levels.end(); ++it) {
  ...
  // Producer
  //
  // We don't use producer in the q3/metqu data model, but Newbase needs it for
  // creating an SQD file.
  //
#if 0
    if (!prod_num) {
  ...
  // Go through params and collect the ones really taking space on disk (i.e.
  // skip those derived from ':19' or ':326').
  //
  for (vector<NA_Param>::const_iterator it = info.getParamsBegin();
       it != info.getParamsEnd(); ++it) {
    ...
    if (!SQD_Tools::cut_at_colon(it->getNativeName_().c_str(), name_part_utf8,
                                 e)) {
      throw E_LOG_BUG0("Bad 'NA_Param'"); // shouldn't happen
    }

    string latin1_name =
        utf8_to_latin1(name_part_utf8.c_str()); // without the colon and id

    /*
     * Note: The interpolation method stored matters only for SQD format (and if
     * the file is used outside of q3). q3 now uses its own interpolation code
     * and is not dependent on these values.
     *
     * Note: INTERPOLATE_LINEAR_DEG params (s.a. 'WD') are stored with
     * 'kNearestPoint' since there is no interpolation method enum in Newbase
     * for degree values. This is a safe choise. Newbase may still do
     * interpolations differently for such entries - we don't know (or care).
     * -AKa 12-Oct-2010
     */
    ...
    pbag.Add(NFmiParam(e, latin1_name.c_str(),
                       kFloatMissing, // minvalue
                       kFloatMissing, // maxvalue
                       kFloatMissing, // scale
                       kFloatMissing, // base
                       "%.1f", // written to SQD header so we better go as
                               // default constructor does
                       method));
  }

  // Newbase requires this to be dynamically allocated (we think...)  AKa
  // 4-Oct-10
  //
  const NFmiProducer *nprod = new NFmiProducer(prod_num);
  assert(nprod);

  // Newbase has separate static functions for creating the 'kFmiTotalWindMS'
  // and 'kFmiWeatherAndCloudiness' mega parameters.
  //
  // Note: 'CreateParam()' should be a static function (but is not).
  //
  // Note: These two won't need to be given a name, since they are never
  // directly
  ...
  // Need to use 'CreateEmptyData()' to create (new) the object and also
  // initialize it (standard constructor does not initialize).      -- AKa
  // 9-Sep-2009 (based on email with Marko)
  //
  ...
  // Note: We're NOT expecting anything from the 'CreateEmptyData()' though in
  // practise it does
 ...
 */
#ifdef METQU
void SQD_Data::output(ostream &out,
                      ProgressCallback *cb) const /*throw(E_FATAL)*/
{
  assert(qd);
  ...
  try {
    ...
    throw E_LOG_FATAL("Writing SQD failed: %s",
                      what ? what : "(unknown C++ exception)");
  }
}
#endif
...
NA_Info SQD_Data::read_info(const char *fn) {
  ...
  // 'pr' initialized like this since we *know* the projection is
  // 'SQD_Projection' (avoids going through any other parsing)
  //
  // 22-Aug-2011 MPi/PKi: Use grid's area to create projection; if using
  // creation string the
  //						projection may not exactly be the same
  //(floating point accuracy problem)
  //

  const Projection pr(
      grid ? Projection(area->AreaStr().c_str(), SQD_Projection(area))
           : Projection(nullptr));
  ...
  NA_Info info2(
      time(nullptr), attrib.st_mtime, SQD_Tools::mt2jd(info.OriginTime()),
      times, nativeLevelType, levels, params, pr, area ? area->WKT() : "",
      getDataTimeStep(info),
      area ? area->WorldXYWidth() / (grid->XNumber() - 1) / 1000.0 : 0,
      area ? area->WorldXYHeight() / (grid->YNumber() - 1) / 1000.0 : 0,
      grid ? true : false);
  ...
  info2.setExtra_fn(fn)
      ...
      .setExtra_sqd_revlevorder(
          revlevorder); // 31-Aug-2011 PKi: Flag for original level order
...
/*virtual*/ bool SQD_Data::providesPressureLevelsFromHybrid() const {
  // Pressure levels available (calculated from hybrid levels) if data has param
  // 1 (kFmiPressure).
  //
...
MatrixPos SQD_Data::getGridSize(size_t nPoints) const {
  ...
  if (grid)
    return MatrixPos(grid->XNumber(), grid->YNumber());

  // 07-Apr-2015 PKi: Use number of given data points for point data. If
  // projection (bounding box)
  //					was given instead, the matrix will later be resized to
  //the number of data points 					within the given
  // area
  //
  return MatrixPos(qd->Info()->SizeLevels() *
                       ((nPoints > 0) ? nPoints : qd->Info()->SizeLocations()),
                   1);
}

/*
 * Push a certain 'Matrix' value on Lua stack (or use regular 'new' operator if
 * 'L'==0).
 *
 * Returns: pointer to the pushed object; tied to Lua GC if 'L'!=0. To be
 * deleted by the caller if 'L'==0. nullptr if the requested parameter is not
 * available
 */
/*virtual*/ CONST_IF_SERVER Matrix *
SQD_Data::push_NativeMatrix(lua_State *L, const JDay &vt, const NA_Level &lev,
                            const NA_Param &p
                            //
                            ...
                            const DataIdList *dataIds) CONST_IF_SERVER noexcept {
  CONST_IF_SERVER Matrix *m;

  //---
  // Special case: if looking for parameter 'Z' (with no id) look first at ':2'
  // (kFmiGeopHeight) and then
  // ':3' (kFmiGeomHeight) transparently to the user. Newbase does similar.
  //
  // Note: Currently this processing is ONLY within the 'SQD_Data' adapter, thus
  // not being applied to
  //      i.e. MQD data.
  //
  // Note: Vili was surprised to hear of such. It seems not to be a generally
  // known approach, and is
  //      potentially risky. We'll place this in the configuration file for now
  //      (disabled by default).
  //
  // Note: Parameters ':2' and ':3' actually have different units ("gpm" and
  // "m"). This further complicates
  //      things if we allow the silent cascade.
  //
  // 18-Oct-2011 PKi:
  //   Another special case: if looking for parameter 'W' (with no id) look
  //   first at
  //   ':43' (kFmiVerticalVelocityMMS) and then :39 (kFmiVelocityPotential).
  //
  //   The primary/secondary parameter selection for mapped parameters
  //   (currently Z and W) is now handled by mapParameter()
  //
  //#ifdef SQD_CASCADE_Z_ENABLED
  //    if (p.toString() == "Z") {
  //        m= push_NativeMatrix_e( L, vt, lev, kFmiGeopHeight, target_proj,
  //        target_gs, target_ready
  //        );     // id 2 if (!m) {
  //            m= push_NativeMatrix_e( L, vt, lev, kFmiGeomHeight, target_proj,
  //            target_gs, target_ready );     // id 3
  //        }
  //    } else
  //#endif
  //    {
  //        m= push_NativeMatrix_e( L, vt, lev, SQD_Tools::param_id(p),
  //        target_proj, target_gs, target_ready );
  //    }

  bool mapped = false;

  // Try p or the primary parameter if available
  m = push_NativeMatrix_e(L, vt, lev, mapParameter(p, true, &mapped),
                          target_proj, target_gs, target_ready, dataIds);

  if ((!m) && mapped)
    // Try the secondary parameter
    m = push_NativeMatrix_e(L, vt, lev, mapParameter(p, false), target_proj,
                            target_gs, target_ready, dataIds);

  return m;
}

/*
 * Get data for given data point id's or data points within given area. The data
...
void pointValues(NFmiFastQueryInfo &info,
  ...
  // With sounding data, return missing values for levels having missing
  // pressure value; read pressures first

  std::vector<float> pressures;
  auto queryParam = info.Param();
  bool getSoundingPressureValues =
      ((!retMissing) && soundingData &&
       (queryParam.GetParamIdent() == kFmiPressure));
  ...
  if ((!retMissing) && soundingData) {
    ...
    if (dataIds) {
      for (info.ResetLevel(); info.NextLevel();) {
        for (DataIdList::const_iterator it = dataIds->begin();
             (it != dataIds->end()); it++) {
    ...
    if (dataIds) {
      for (info.ResetLevel(); info.NextLevel();) {
        for (DataIdList::const_iterator it = dataIds->begin();
          ...
          if ((!retMissing) &&
              ((!soundingData) || (*itPressure != kFloatMissing)) &&
              info.Location(*it))
            v = info.FloatValue();
...
/*
 * TBD:  Current handling is inefficient. Scripts regularily ask for say 50,60
 * grid of interpolated values. We should calculate such right on, without
 * pushing native resolution interpolation (s.a. 500,600).
 */
CONST_IF_SERVER
Matrix *
SQD_Data::push_NativeMatrix_e(lua_State *L, const JDay &vt, const NA_Level &lev,
                              FmiParameterName e
                              //
                              // 22-Aug-2011 PKi: For fetching data using
                              // newbase interpolation, projection and scaling
                              //
                              ,
                              const Projection *target_proj,
                              const MatrixPos *target_gs, bool *target_ready,
                              const DataIdList *dataIds_) CONST_IF_SERVER noexcept {
  try {
    ...
    const DataIdList *dataIds =
        ((dataIds_ && (dataIds_->size() > 0)) ? dataIds_ : nullptr);

    const MatrixPos native_gs =
        getGridSize((pointData && dataIds) ? dataIds->size() : 0);
    ...
    bool exact_level =
        ((lt != NA_Level::HEIGHT_LEVEL) &&
         ((!lev) || lev.isGroundLevel() || lev.isSoundingLevel() ||
    ...
    // 16-Apr-2015 PKi: Store origintime of the first used querydata as a lua
    // global; it will be returned in response headers
    //
    ...
    //---
    // Direct connection to data - no interpolations needed (writable in command
    // line mode)
    //
    if (exact_time && exact_level && same_proj && same_gs &&
        ((!pointData) || (!dataIds))) {
      try {
        // Push a read-only proxy (for read matrices, caller does a copy most
        // likely anyways, to a MemMatrix if scaling and/or projection is
        // applied).
        //
        SQD_Matrix *m = L ? new (L) SQD_Matrix(this, vt, lev, e)
                          : ::new SQD_Matrix(this, vt, lev, e);

#ifdef USE_NEWBASE_PROJ
        // 22-Aug-2011 PKi: No further processing (interpolation, projection or
        // scaling) to be done
        ...
        //
        std::shared_ptr<NFmiGrid> wantedGrid(new NFmiGrid(
            qd->Info()->Area(), m->getSize().getXS(), m->getSize().getYS()));
        m->setGrid(wantedGrid);

        return m;
      } catch (const E_NO_MATCH &) {
        if (L) {
          // Didn't get created (but Lua did allocate the object AND attach its
          // GC to the C++ destructor. Nuke the object before it gets to GC.
          //
          LuaNew_base::nuke(
              L, -1); // removes the link from Lua GC to C++ destructor
        }
    ...
    NFmiFastQueryInfo fi(qd);
#if 1
    fi.First(); // Just in case (faint memory this would be needed?) -- AKa
                // 24-Mar-10
#endif

    // Set to the param
    //
    // Note: We may need to do this later on, within the loops, to keep the
    // right param selected
    //       but it's good to get out here early if the param won't even be
    //       available.
    //
    if (!fi.Param(e)) {
      return nullptr;
    ...
    // 22-Aug-2011 PKi: Fetch data using newbase interpolation, projection and
    // scaling if requested
    //
    // 07-Apr-2015 PKi: If interpolation would be needed for point data, return
    // missing values.
    //
    ...
    MemMatrix *m_ =
        retMissing
            ? L ? new (L) MemMatrix(native_gs, NAN, lev, e, unit,
                                    target_proj ? *target_proj : native_proj)
                : ::new MemMatrix(native_gs, NAN, lev, e, unit,
                                  target_proj ? *target_proj : native_proj)
        : L ? new (L)
                  MemMatrix((!pointData) && target_gs ? *target_gs : native_gs,
                            lev, e, unit,
                            target_proj ? *target_proj : native_proj)
            : ::new MemMatrix(
                  (!pointData) && target_gs ? *target_gs : native_gs, lev, e,
                  unit, target_proj ? *target_proj : native_proj);
    ...
    if (!(same_proj && same_gs)) {
      // Nonnative target projection and/or gridsize; if native projection,
      // clone grid's area.
      //
      // Set pr to projection to be used if area is created using
      // NFmiAreaFactory (when nonnative projection or getting/cloning area
      // should fail)
    ...
    auto areaPtr = ((!(same_proj && same_gs)) && (!area))
                       ? NFmiAreaFactory::Create(pr->toString())
                       : (NFmiAreaFactory::return_type)area;

    if (pointData) {
      // 07-Apr-2015 PKi: Return point data within the given area
      //
      pointValues(fi, areaPtr, dataIds, retMissing, *m_,
                  levels_[0].getType() == NA_Level::SOUNDING_LEVEL);
    } else {
      // Attach grid; needed by PEEKXY
      //
      if (!areaPtr.get())
        areaPtr = NFmiAreaFactory::Create(native_proj.toString());

      std::shared_ptr<NFmiGrid> wantedGrid(new NFmiGrid(
          areaPtr.get(), m_->getSize().getXS(), m_->getSize().getYS()));
      ...
      if (exact_level) {
        ...
        if (!(same_proj && same_gs)) {
          //			NFmiGrid wantedGrid(areaPtr.get(),
          //m_->getSize().getXS(),
          // m_->getSize().getYS());
        ...
        if (!(fi.PressureValueAvailable() // Data has pressure parameter (useful
                                          // for pressure level calculation)
              ||
              fi.PressureLevelDataAvailable() // Data has pressure levels as
                                              // such (can be interpolated from)
              )) {
          throw E_LOG_BUG0("Data does not support .PressureLevelValue()");
        }

        // No need to route data via 'NFmiDataMatrix'. This also solves the
        // crashing issue with '::PressureValues()'.
        //
        ...
        for (fi.ResetLocation(); fi.NextLocation();) {
          ...
          // It seems '.PressureLevelValue()' does not work with interpolated
          // times. It gives us NAN whenever 'mt' is not an exact time in the
          // data.

          // Note: Seems the 'fi.Time()' setting above does not cause time
          // interpolation for
          // '::PressureLevelValue()'.
          //      There's another API that takes a time argument.
          //
          float v = exact_time ? fi.PressureLevelValue(lv)
                               : fi.PressureLevelValue(lv, mt);
          ...
          // There's a bug in Newbase '::PressureLevelValue()'. If it does not
          // find the right pressure level, it changes the current parameter to
          // P (pressure). If we got NAN, we must reset the param to what we
          // wanted. (No way for us to know if the NAN was because of the "give
          // up" condition or because of genuinly missing data only on that
          // particular position).  --AKa 21-Dec-2010
          //
...
/*
 * 26-Sep-2011 PKi: Returns JDay as gm time_t or 0 if conversion is not
 * possible.
 */
...
/*
 * 23-Sep-2011 PKi: Calculates time_t difference of 2 JDays. Returns false if
 * the times are not valid as time_t values or the difference is not what
 * expected (times are taken as subsequent elements of a constant step time
 * serie)
 */

static bool timeDiff(JDay const &j1, JDay const &j2, time_t &tDiff, time_t &t1_,
                     unsigned int tStep) {
  // Times should be nonzero and in rising order. Use t1_ (the converted value
  // of j2 in previous call) if it is available
  ...
  return (tDiff && ((!tStep) || (!(tDiff % tStep))) &&
          ((!tD) || (tDiff == tD)));
}

/*
 * 23-Sep-2011 PKi: Fetch cross using newbase. Uses native projection and
 * gridsize
 */
...
/*virtual*/ CONST_IF_SERVER Matrix *
SQD_Data::push_NativeCross(lua_State *L, const std::vector<JDay> &vtVec,
  ...
  try {
    ...
    // 18-Oct-2011 PKi:
    //
    //   Special case: if looking for parameter 'Z' (with no id) look first at
    //   ':2' (kFmiGeopHeight) and then
    //   ':3' (kFmiGeomHeight) transparently to the user. Newbase does similar.
    //
    //   Note: Parameters ':2' and ':3' actually have different units ("gpm" and
    //   "m"). This further complicates
    ...
    bool setOk = fi.Param(
        e = mapParameter(
            p, true, &mapped)); // Try p or the primary parameter if available
    if (!setOk) {
      if (mapped)
        setOk =
            fi.Param(e = mapParameter(p, false)); // Try the secondary parameter
    ...
    //
    // When one location and multiple times with constant step, or multiple
    // locations and one time, TimeCrossSectionValues or CrossSectionValues are
    // used respectively instead of RouteCrossSectionValues

    NFmiDataMatrix<float> nm;

    std::vector<float> pressures_or_heights;
    std::vector<float> &pressures = pressures_or_heights,
                       &heights = pressures_or_heights;
    std::vector<NFmiLevel> hLevels;
    unsigned int nLevels = levelVec.size(), nLocs = locs.size(),
                 nTimes = vtVec.size(), i;
    time_t t1 = 0, tDiff = 0;
    bool timeCross =
        ((!flightRoute) && (nLocs == 1) && (nTimes > 1) &&
    ...
    bool ground =
        ((nLevels == 1) && (levelVec[0].getType() == NA_Level::GROUND_LEVEL));
    ...
    if (timeCross) {
      ...
      for (i = 2; (i < nTimes); i++)
        if (!timeDiff(vtVec[i - 1], vtVec[i], tDiff, t1,
                      NewBaseMinTimeStepInSecs)) {
          timeCross = false;
          break;
        }
    }
    ...
    if (timeCross) {
      ...
      //    	if (ground)
      //    		fi.TimeCrossSectionValuesGround( nm,
      //    NFmiPoint(locs[0].getLon(), locs[0].getLat()), validTimes );
      //    else
      if (hybrid)
        ...
        nm = fi.TimeCrossSectionValuesLogP(
            pressures, NFmiPoint(locs[0].getLon(), locs[0].getLat()),
            validTimes);
    } else {
      ...
      if (!flightRoute) {
        ...
        for (i = iLocs = 0; (i < nTimes); i++, iLocs += locIdxIncr) {
          ...
          locations.push_back(
              NFmiPoint(locs[iLocs].getLon(), locs[iLocs].getLat()));
        }

        if (cross)
          //        	if (ground)
          //            	fi.CrossSectionValuesGround( nm, validTimes[0],
          //            locations );
          //        	else
          if (hybrid)
        ...
        //      else if (ground)
        //      	fi.RouteCrossSectionValuesGround( nm, locations,
        //      validTimes );
        else if (hybrid)
        ...
        if (nm.NX() == 0)
          throw E_LOG_USAGE0("FlightRouteValues() returned no data; must have "
                             "equal number of locations, levels and "
                             "times");
...
/*virtual*/ void SQD_Data::getLocations(vector<LatLon> &locations,
  ...
  if (fi.Grid()) {
    ...
    NFmiAreaFactory::return_type area =
        NFmiAreaFactory::Create(proj.toString().c_str());
    ...
    NFmiAreaFactory::return_type area =
        target_proj ? NFmiAreaFactory::Create(target_proj->toString())
                    : nullptr;
...
/*virtual*/ void SQD_Data::getDataIds(std::vector<unsigned long> &dataIds,
  ...
  if (fi.Grid())
    throw runtime_error("Can't query data point id's from gridded data");

  if (target_proj) {
    NFmiAreaFactory::return_type area =
        NFmiAreaFactory::Create(target_proj->toString());
...
/*virtual*/ void SQD_Data::getDataNames(vector<string> &dataNames,
  ...
  if (fi.Grid())
    throw runtime_error("Can't query data point names from gridded data");
  ...
  if (target_proj) {
    ...
    for (fi.ResetLocation(); fi.NextLocation();) {
      ...
      if (area->IsInside(p))
        dataNames.push_back(string(reinterpret_cast<const char *>(
            fi.Location()->GetName().GetCharPtr())));
    }
  } else
    for (fi.ResetLocation(); fi.NextLocation();)
      dataNames.push_back(string(reinterpret_cast<const char *>(
 ...
 * (currently Z and W).
 *
 *                  Sets 'mapped' to true and returns the id of primary or
 * secondary parameter as requested if the input parameter 'p' is one of the
 * mapped parameters. Otherwise 'mapped' is set to false and returns the id of
 * 'p'.
 *
 * 31-Oct-2011 PKi: Mapping now through virtual_params[] instead of coding
 * parameterwise.
 */
...
FmiParameterName SQD_Data::mapParameter(const NA_Param &p, bool primary,
                                        bool *mapped) const {
  bool dummy;
  ...
  // 31-Oct-2011 PKi
  //
  virtualParameterName v =
      NA_Param::virtual_param_native_id(p.toString(false).c_str());
...
/*
 * 31-Oct-2011 PKi: Virtual parameter mapping for native parameters; collects
 * virtual parameters to be added to the parameter set.
 */

void SQD_Data::mapParameter(
    const FmiParameterName id,
    std::map<std::string, FmiParameterName> &virtualParams) {
  string s = NA_Param::native_id_virtual_param(id);
  if (s != "")
    virtualParams.insert(make_pair(s, id));
