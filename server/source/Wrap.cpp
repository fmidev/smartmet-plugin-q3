...
// Hide rest of 'debug.*' except for 'debug.getinfo' (used by some extension libraries
...
int LuaWrapper::my_traceback(lua_State *L)
{
  ...
  lua_call(L, 2 /*args*/, 1 /*retval*/);  // call upvalue (original 'debug.traceback')
  return 1;
}
...
*       998 for trying to return multiple (binary) results (which cannot be delivered by HTTP)
*       999 for internal bug situation
*/
int LuaWrapper::run(int args,
                    RequestResponse &rr,
                    double secs,
                    int decimals
                    ...
                    )
{
  ...
  //
  // 'run2()' must be called with 'lua_pcall()' to catch any runtime errors either in the
  // user-provided code or in the conversion of return values.
  //
  try
  {
...
/*
* 2nd stage of 'run', called via 'lua_pcall()' to collect all Lua errors with just one
* 'lua_pcall()' (use of which is more costly than 'lua_call()' so using just one is Good).
*
* void= run2( code_func [, ...] )
*
* Upvalues:
...
{
  RequestResponse &rr = *((RequestResponse *)lua_touserdata(L, lua_upvalueindex(1)));

  lua_call(L, lua_gettop(L) - 1 /*args*/, LUA_MULTRET);

  // We can only provide one MIME type for the returned data. If all is text, that's fine
  // (and we can join multiple return values by newline). If there's just one binary
  // return value, that's also fine.
  //
  // For multiple (some binary) return values we could i.e. create a ZIP file on the
  // server side. Is this useful, or required? Could be used i.e. to craft multiple pictures
  // all at one round.
  //
  ...
  for (unsigned i = 1; i <= retvals; i++)
  {
    ...
    {  // newline between multiple return values
      ss << "\n";
    }
  }
