...
bool Bz2_Tracker::update_from_cache(set<string> &seen_already) throw()
{
...

  {
    ...
    for (map<string, JDay>::const_iterator it = cache->mapping.begin(); it != cache->mapping.end();
         ++it)
    {
      ...
      {
        // We can write 'available_data' without locking, since the object has not been
        // declared initialized, yet.
        //
        // Note: This only initializes a 'TrackedData' entry. It DOES NOT open the file
...
#error "Not implemented for Win32"
  ...
  */
  ...
  while ((fn_abs = glob_fn(tmp.c_str(), gbuf, gbuf_i)) != NULL)
  {
    ...
    {
      continue;  // Origin Time not found; seems like bad data (log has been done)
    }
      ...
      if (it != available_data.end())
      {
        ...
        */
        ...
        else
        {
          ...
          {
            LOG_ERROR("Cannot override %s (in use) with later file %s", fn_abs2, fn_abs);
