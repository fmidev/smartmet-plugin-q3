...
{
 public:
  Bz2_Tracker(const std::string &fn_abs_mask,
              uint_t refresh_ms,
              unsigned wiping_[],
              bool relative_uv_,
 ...
 private:
  ...
  // data members
  /*const*/ std::string
      base_path;  // i.e. "/smartmet/archive/pal/querydata/pal/skandinavia/maanpinta/"
  /*const*/ std::string file_mask;  // i.e. "*.sqd.bz2"

  const string_or_null
      tmp_pattern;  // control when to extract to disk (and memory map) or open into memory
