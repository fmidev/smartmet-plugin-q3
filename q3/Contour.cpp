/*
 * CONTOUR.CPP                             Copyright (c) 2010, Ilmatieteen
 * laitos
 *
...
// Closed areas are automatically re-expanded after smoothening, to compensate
// for areal loss (which is calculated). If the need for expansion is below this
// limit, no expansion will be done.
//
...
/*
 * [edgepoint_ud]= __index( contour_ud, int )      -- 1..N (otherwise returning
 * nil)
 *
 * We use 1..N Lua indices to make the contour look like a Lua array. However,
 * 'offset' has been set such that the first point seen by Lua has some
 * guarantees to it (to make Lua coding simpler). See 'set_offset()' and
 * 'INVARIANT()'.
 */
int ContourBind::__index(lua_State *L) {
  ...
  if (lua_type(L, 2) == LUA_TSTRING) {
    ...
    if (k > 0) {
      ...
      assert(my.offset !=
             ((unsigned)-1)); // 'set_offset()' must have been called

      if ((unsigned)k <= n_) {
        new (L) EdgePoint(my.points[(my.offset + (k - 1)) %
                                    n_]); // copy constructor to Lua userdata
        return 1;
      } else {
...
int EdgePointBind::__tostring(lua_State *L) {
  ...
  lua_pushfstring(L, "{ x=%f, y=%f, edge=%s }", my.x, my.y,
                  my.edge ? "true" : "false");
  return 1;
}
 ...
 *
 * Note: This function is run once for each part to be labelled, and results
 * used for considering how good the particular position is (how horizontal it
 * is). This happens even if all labels are to be plotted horizontal.
 *
 * Note: If the part is not closed, we won't be needing the first and last
 * point's slants. However, for shortness of code we calculate them anyways.
 */
int Contour::calc_slants(lua_State *L) {
  ...
  while (lua_next(L, table_idx)) {
    ...
    // 04-Nov-2011 PKi: Check the value to detect end of part's contours; other
    // keys/values follow.
    //
  ...
  // On each iteration, we handle two vectors (one from previous point to us and
  // one from us to the next point).
  //
...
/*
 * [smoothenedmatrix,] tronhints, [contour_ud [, ...]]= contour( matrix, lo_val,
 * hi_val [, smooth_length, smooth_degree, smoothenedmatrix, tronhints ] )
 *
 * Calculate contours of constant value.
 */
int Contour::contour(lua_State *L) {
  proto(L, "Matrix, number, [ number ], [ number ], [ number ], [ Matrix ], [ "
           "TronHints ]");
   ...
   *   - Higher values are to the right of the curves (they travel around
   * 'hills' clockwise)
   *   - At edges (material edge or near missing data) curves are marked as
   * "edge"
   *
   * This also guarantees any curves of a certain level value will always be
   * alike (that contours and fills have exactly the same paths).
   */

  // TBD: We could have a 'ContourCollector' object here functioning as a a
  // callback.
  //      Would reduce the time used in copying stuff - pushing a contour
  //      directly to Lua stack and letting the adapter fill stuff to it.
  //
  //      The adapter could also encapsulate 'my' into it, keeping access to the
  //      matrix here (only accessed via 'ContourCollector').
  //
  // 29-May-2012 PKi: If smoothening has already been done, the latter input
  // matrix is just used and returned;
  //					otherwise the source/former matrix is pushed and it
  //is smoothened if requested.
  //
  //					Note: if old style call (not using lua
  //side contourcollector) from user script,
  // the source matrix is pushed (it's used to get the contours) but not
  // returned.
  //
  // 05-Jun-2012 PKi: Using tron hints if the 'TronHints' arg is passed in. If
  // it is nil, TronHints
  //					object is pushed onto stack; otherwise it is just
  //used
  // and returned. If the 'TronHints' 					arg is omitted, nil is
  // pushed and returned to keep the number of return values preceeding
  // the contours constant
  //
  unsigned argc =
      lua_gettop(L); // 5 or 6 args expected, 2 args if old style call
  unsigned coffs =
      ((argc > 3) ? 2 : 0); // Max number of pushed values in front of contours
  unsigned tos =
      argc + ((argc > 3) ? 0 : 1); // Top of stack; first return value

  float lo_val = lua_tonumber(L, 2),
        hi_val = ((argc >= 3) ? lua_tonumber(L, 3) : NAN);
  ...
  // 13-Dec-2011 PKi: Smoothing length and degree for tron. No smoothing by
  // default.
  //                  Default degree is 2
  //
  // 29-May-2012 PKi: Use zero smoothing length if the data has been smoothened
  // already
  //
  ...
  //was passed in - top of stack will be adjusted down by 1 by the
  // call if the object is not nil (nothing pushed, the hints are just used and
  // returned); if hints are not to be used, push
  // nil onto the stack to keep the number of return values preceeding the
  // contours constant
  //
  if ((argc > 3) && (argc < 7))
    lua_pushnil(L);
  ...
  //
  // 29-May-2012 PKi: Set offsets only when old style call (not used by lua side
  // contourcollector)
  //
  // 02-Mar-2015 PKi: Offsets must always be set for proper egde handling (must
  // start stroking from the start of a strokable section; offset is needed by
  // draw_path())
  //
...
int Contour::contour_smoothen_one(lua_State *L) {
  ...
  if (q3smoothening) {
    ...
    for (vector<EdgePoint>::iterator it = c2.points.begin();
         it != c2.points.end(); ++it, ++i) {

      // If 'half_window-1' points left and round of '*it' are not edge points,
      // smoothen. If there are edge points, try with a shortened range.
      //
      for (int hf = half_window; hf >= 1; hf--) {
        ...
        for (int j = -hf; j <= hf; j++) {
          ...
          // First or last point are allowed to be at edge but not others (there
          // must be no edge sections within our window, or we won't do the
          // adjust).
          //
          if (p.edge && (abs(j) < half_window)) {
            try_smaller_window = true;
            break; // do not move this point
          } else {
        ...
        if (!try_smaller_window) {
          ...
          p +=
              Vectors::Vector(p, avg / (hf * 2 + 1)) * factor; // move the point
    ...
    //      is possibly little less than what would have been best (our code
    //      expects an even expansion, but the edge points won't be expanded)
    //
    double area_was = old.area();
    double area_now = c2.area();

    double factor_1d =
        sqrt(area_was) / sqrt(area_now); // How much to expand (>1.0) to make
                                         // areas (almost) like they were
    ...
    for (vector<EdgePoint>::iterator it = c2.points.begin();
         ((it != c2.points.end()) && (spit != sp.end())); it++, spit++)
...
/*
 * Stretch a contour by 'factor' (>1.0) from its center point. The factor is a
 * one-dimensional factor; each non-edge point is moved that much further away
 * from the center.
 */
void Contour::stretch_out(double factor) {
  ...
  //
  Point cp;
  for (vector<EdgePoint>::const_iterator it = points.begin();
       it != points.end(); ++it) {
    cp += *it;
  }
  cp /= n;
...
double Contour::area() const {
  ...
  // "Multiply the x coordinate of each vertex by the y coordinate of the next
  // vertex. Add these." "Multiply the y coordinate of each vertex by the x
  // coordinate of the next vertex. Add these."
  //
  ...
  // Subtract the sums, divide by two and ignore the sign (indicates whether the
  // contour went clockwise or counter-clockwise).
  //
  return abs((sum1 - sum2) / 2.0);
}

/*
 * Set the 'offset' field of particular contour so it can be easily indexed from
 * Lua, with added assurance such that:
 *
 *   - if the contour is closed (no edges), any offset will do (==0)
 *
 *   - if there is at least one edge, '.edge' of the 'offset' point is 'true'
 * and the next point's 'edge' is 'false'. In other words, the 'offset' point is
 * the first point of a strokable non-edge section.
 */
void Contour::set_offset() {
  ...
  // Check all points until we find a suitable position (or figure that it's a
  // closed contour)
  //
  for (unsigned i = 0; i < n; i++) {
  ...
  // Note: For some cases we get contours that are all-edge. This may even be
  // beneficial, i.e.
  //      to fill areas from "down to level X" or "level X to up". So let them
  //      pass.
  //
#if 1
 ...
 *   rotation handled)
 *
 * Caller needs to use 'cairo_save/restore()' to restore the original
 * conditions.
 */
static void label_pos(cairo_t *cr, const Vectors::Point &p, const string &s,
                      double tilt_rad,
 ...
 * Label printing
 */
static void label(cairo_t *cr, const ContourInfo_Line::LabelParams &label_specs,
  ...
  cairo_save(cr);
  {
    ...
    //
    float w = te.width + 2 * LABEL_MARGIN_X; // give some extra (a bigger
                                             // bounding box than just the text;
    float h = te.height + 2 * LABEL_MARGIN_Y; // only matters if the font
                                              // filling is non-transparent)
 ...
 * intact).
 */
static void label_clip(cairo_t *cr, const Labelizer::TiltedRect &tr) {
...
public:
  MyDrawer(cairo_t *cr_, unsigned w_, unsigned h_)
      : Labelizer::Drawer(), cr(cr_), w(w_), h(h_) {}
  ~MyDrawer() {}
...
// DEBUG: show exactly where the label is meant to be
   ...
   */
  static void set_color_by_name(cairo_t *cr, const char *str,
                                double alpha = 0.2) {
 ...
 */
static Vectors::Vector label_size(cairo_t *cr, float font_size,
                                  const string &label) {
  ...
  if (h < 0)
    h *= -1.0; // compensate for negative Y mapping (SVG, PNG)
...
void Contour::draw_path(cairo_t *cr, bool line) const {
  ...
  // 02-Mar-2015 PKi: Offset must be used for proper egde handling (must start
  // stroking from the start of a strokable section)
  //
  unsigned n = points.size();
  assert((offset != ((unsigned)-1)) &&
         (offset < n)); // 'set_offset()' must have been called
  vector<EdgePoint>::const_iterator eit = points.begin();
  advance(eit, offset);
  vector<EdgePoint>::const_iterator last = eit;

  for (; (n > 0); eit++, n--) {
    if (eit == points.end()) // Wrap around to the first point (__index();
                             // points[ (my.offset+(k-1)) %n_ ] );
      eit = points.begin();  //
 ...
 *
 * drawcontours( cairocontext, gridsize_ud, { labelcfg }, { contourdefs }, {
 * contourvalues }, { contour_ud } )
 */
int Contour::drawcontours(lua_State *L) {
  try {
    proto(L, "CairoContext, MatrixPos, { } , { string }, { [ number ] }, { { "
             "Contour } }");

    cairo_t *cr = nullptr;
    MatrixPos *gs = nullptr;

    const int cr_idx = 1, gs_idx = 2, lcfg_idx = 3, cdef_idx = 4, cval_idx = 5,
              contour_idx = 6;

    if ((lua_gettop(L) == contour_idx) &&
        (lua_type(L, lcfg_idx) == LUA_TTABLE) &&
        (lua_type(L, cdef_idx) == LUA_TTABLE) &&
        (lua_type(L, cval_idx) == LUA_TTABLE) &&
        (lua_type(L, contour_idx) == LUA_TTABLE)) {
      Context *cx = Context::instance(L, cr_idx);

      // 09-Feb-2012 PKi: Context::instance() fails if not statically linked
      // with newcairo;
      //                  just casting ...
      if (!cx) {
      ...
      luaL_error(L, "Invalid args; usage: drawcontours( cr, gridsize_ud, { "
                    "contourdefs }, { contourvalues }, { contour_ud } )");

    if ((!cr) || (!gs))
      luaL_error(L,
                 "Invalid %s; usage: drawcontours( cr, gridsize_ud, { "
                 "contourdefs }, { contourvalues }, { contour_ud } )",
    ...
    while (lua_next(L, cdef_idx)) {
      ...
      if (!lua_isnumber(L, -2))
        luaL_error(L,
                   "arg %d: numeric contour descriptor index expected, got %s",
                   cdef_idx, lua_typename(L, lua_type(L, -2)));

      idx = (unsigned int)lua_tonumber(L, -2);

      if (lua_isnil(L, -1) || (!lua_isstring(L, -1)))
        luaL_error(L, "arg %d: contour descriptor expected, got %s", cdef_idx,
    ...
    while (lua_next(L, cval_idx)) {
      ...
      if (!lua_isnumber(L, -1))
        luaL_error(L, "arg %d: contour value expected, got %s", cval_idx,
                   lua_typename(L, lua_type(L, -1)));
      else {
        v = lua_tonumber(L, -1);
    ...
    // Configure Labelizer (strategy for positioning labels, label color and
    // size etc.)
    //
    MyDrawer my_drawer(cr, 0, 0);
    Labelizer::Config lCfg;
    Labelizer lzer(Labelizer::loadUserCfg(L, lcfg_idx, lCfg), my_drawer,
                   gs->getX() - 1, gs->getY() - 1);
    ...
    while (lua_next(L, contour_idx)) {
      ...
      if (!lua_isnumber(L, -2))
        luaL_error(L, "arg %d: numeric contour index expected, got %s",
                   contour_idx, lua_typename(L, lua_type(L, -2)));

      nc = (unsigned int)lua_tonumber(L, -2);

      if (lua_type(L, -1) != LUA_TTABLE)
        luaL_error(L, "arg %d: table of contours expected, got %s", contour_idx,
                   lua_typename(L, lua_type(L, -1)));
      ...
      if (labelize) {
        if (nc <= labelval.size()) {
          ...
          // 11-Jan-2012 PKi: If nan just do not labelize; go on to handle the
          // contour
          //
          //					if (isnan(v))
          //					{
      ...
      //
      // If contour index >= max. contour descriptor index, use the last
      // descriptor without searching

      cdefmap::const_iterator cit = contourdefs.end();
      cit--;

      if (nc < cit->first) {
        // If contour index >= previous index, start searching from prev.
        // iterator

        pnc++;

        for (cdefmap::const_iterator it =
                 (cc && (nc >= pnc)) ? pit : contourdefs.begin();
             it != contourdefs.end(); it++)
          if (it->first > nc)
            break;
          else
            cit = it;
      ...
      if ((!cc) || (cit != pit)) {
        ...
        //
        // 1 					step
        // zero_level(not used) 	lo 				hi 2
        // Nlevels					level1
        // level2 levelN stroke_color 		fill_color stroke_width
        // smooth_factor label_strategy label_font_height
        // label_text_color label_box_fill_color label_box_stroke_color
      ...
      if (cc->line_type()) {
        ...
        Labelizer::FontSpec fs(label, label_size(cr, font_size, label),
                               &ci.label);
        ...
        while (lua_next(L, -2)) {
          ...
          if (!c)
            luaL_error(L, "arg %d: contour expected, got %s", contour_idx,
                       lua_typename(L, lua_type(L, -1)));

          if (labelizethis) {
            bool fst = true;

            for (vector<EdgePoint>::iterator it = c->points.begin();
                 (it != c->points.end()); it++) {
              Vectors::Point p(it->x, it->y);
        ...
        if (!pc) {
          const ContourInfo_Fill &ci =
              static_cast<const ContourInfo_Fill &>(*cc);
          ...
          cairo_matrix_init_scale(&matrix, ci.patternSize() * (0.125 / ux),
                                  ci.patternSize() * (0.125 / uy));
    ...
    if (labelize) {
      vector<Labelizer::TiltedRect> label_trs =
          lzer.done(pixel_to_data_dist(cr, 100) / 100);
      ...
      if (clip_labels) {
        ...
        //
        for (vector<Labelizer::TiltedRect>::const_iterator it =
                 label_trs.begin();
             it != label_trs.end(); ++it) {
          label_clip(cr, *it);
        }
