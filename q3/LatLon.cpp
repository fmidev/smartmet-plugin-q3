...
//
static inline double isLeft(const LatLon &P0, const LatLon &P1,
                            const LatLon &P2) {
    ...
    for( vector<LatLon>::const_iterator it= V.begin();
       ...
       if (((prev.lat <= P.lat) && (it->lat > P.lat))    // an upward crossing
        || ((prev.lat > P.lat) && (it->lat <= P.lat))) { // a downward crossing
            ...
            if (P.lon < prev.lon + vt * (it->lon - prev.lon)) // P.lon <
intersect
...
//
static int wn_PnPoly(const LatLon &P, const vector<LatLon> &V) {
  ...
  // loop through all edges of the polygon
  //
  for (vector<LatLon>::const_iterator it = V.begin(); it != V.end();
       ++it) {                         // edge from 'prev' to '*it'
 ...
 *
 * Errors i.e. if an entry seems like a valid latlon string or table, but
 * there's some problem with it (i.e. latlon values out of range). In completely
 * unfamiliar cases, returns 'false'.
 *
 * Returns: true if entry was valid, false if not.
 */
bool LatLon::parse(lua_State *L, int index, bool errors_always) {
  ...
  // See if we can parse the string as number-starting latlon syntax ("yyyN
  // xxxE").
  //
  if (s && isdigit(*s)) {
    ...
    if (rc) {
      ...
      throw E_LOG_BUG("Failed to run LATLON parsing: %d %s", rc,
                      lua_tostring(L, -1));
    }
    // [-1]: lon number (or nil)
    // [-2]: lat number (or nil)
...
/*---=== LatLonList ===---*
 *
 * Note: Using Newbase 'NFmiAreaMask' (rather 'NFmiMultiPolygonAreaMask' API was
 * turned down, mostly because of its interface complexity but also because it
 * seems incapable of detecting when a point is exactly at the rim.    -- AKa
 * 12-Oct-2009
 *
 * Note: Rims spanning across the -180/180 longitude are NOT supported. Upper
 * levels could check for them and split the rims into two.
 */
 ...
 */
enum LatLonList::e_state LatLonList::init_from_ud(lua_State *L, int index,
                                                  bool checkClosing) {
  index = L_ABS(index);
  ...
  // Update bounding box
  //
  for (vector<LatLon>::const_iterator it = this->begin(); it != this->end();
       ++it) {
...
/*
 * Is a certain coordinate within the area defined by 'LatLonList' (inclusing
 * closure from end point to start).
 *
 * Returns:
 *   LatLonList::INSIDE
 *   LatLonList::AT_EDGE
...
enum LatLonList::e_within LatLonList::within(const LatLon &ll) const {
  ...
  //
  if ((ll.lon < bb_min.lon) || (ll.lon > bb_max.lon) || (ll.lat < bb_min.lat) ||
      (ll.lat > bb_max.lat)) {
    return OUTSIDE; // obviously outside
  }

  // 'wn_PnPoly' places edges inside/outside, depending on their facing
...
/*
 * m_mask= areamask( projection_str, gridsize_pos, value_num, invert_bool, {
 * latlon, ... } [, ...] )
 *
 * Returns:
...
int LatLonList::_areamask(lua_State *L) {
  ...
  // Matrix to be returned; we begin by counting the number of
  // inside/outside/rim hits as integers, but eventually use this matrix also as
  // the return value.
  //
  MemMatrix &m = *new (L) MemMatrix(
      gs, 0.0f, NA_Param::UNIT_UNKNOWN_ /*no interpolation*/, proj);
  ...
  for (unsigned i = first_latlon; i <= args; i++) {
    ...
    for (MatrixIter it(gs); it.within(); ++it) {
      LatLon ll =
          pr.latlon(it.getX() / x_top, it.getY() / y_top); // range (0,0)..(1.1)
...
* Note:
<<
In fact, when Sinnott published the Haversine formula, computational precision
was limited. Nowadays, JavaScript (and most modern computers & languages) use
IEEE 754 64-bit floating-point numbers, which provide 15 significant figures of
precision. With this precision, the simple spherical law of cosines formula
gives well-conditioned results down to distances as small as around 1 metre. In
view of this it is probably worth, in most situations, using either the simpler
law of cosines or the more accurate ellipsoidal Vincenty formula in preference
to Haversine!
<<
*/
...
double LatLon::haversine_great_circle_distance_km(const LatLon &o) const {
  ...
  double o_lat_rad = DEG2RAD(o.lat);
  double lon_rad = DEG2RAD(lon);
  double o_lon_rad = DEG2RAD(o.lon);
...
double LatLon::spherical_great_circle_distance_km(const LatLon &o) const {
  ...
  return acos(sin(lat_rad) * sin(o_lat_rad) +
              cos(lat_rad) * cos(o_lat_rad) * cos(o_lon_rad - lon_rad)) *
         R_km;
}
