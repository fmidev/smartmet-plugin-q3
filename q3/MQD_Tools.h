...
/*
 * Note: Boost has memory mapping API, but it's a rather thin API around the
 * Win32/Posix features. We're not needing Boost otherwise (Newbase does,
 * though) so let's not start here.
 *       -- AKa 24-Aug-2009
 */
class MemoryMap {
public:
...
private:
...
#ifdef UNIX
  void *mmap_ptr; // pointer received from 'mmap()' (page aligned); needed by
                  // 'munmap()'
  ...
  void _INVARIANT(const char *file, unsigned line) const {
    ...
    assert_invariant(length !=
                     (size_t)(-1)); // constructor has placed real length there
