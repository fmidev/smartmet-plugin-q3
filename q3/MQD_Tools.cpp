...
/*
 * Start a (read-only) memory mapping. The caller wants a pointer to the file
 * contents at 'start' (but the memory mapping will be made to start earlier,
 * due to page size issues). The mapping will be valid for at least 'length'
 * bytes (and maybe some more, again due to page sizes).
 *
...
/**
    // Open a memory mapping handle for Win32 (actual mappings are called
'views')
    //
...
**/
  ...
  /*
   * Windows and Linux memory mapping APIs require the mapping to begin at an
   * even VM page size (OS X allows any offset).
   */
  const size_t page_size = getPageSize();

  size_t o_skip1 =
      (start / page_size) * page_size; // relying on integer division here!
  ...
  //
  mmap_ptr =
      mmap(0, // address hint (don't use)
           o_skip2 + length, PROT_READ,
  ...
  if (mmap_ptr == MAP_FAILED) { // -1
    ...
    //          or: PROT_WRITE and MAP_SHARED are specified .. and file
    ...
    //          or: offset is not a multiple of the page size, as returned by
    //          sysconf(3)
    ...
    // [EOVERFLOW 75] Addresses in the specified range exceed the maximum offset
    // set for fd
  ...
  //
  void *mmap_ptr =
      MapViewOfFile(handle, FILE_MAP_READ, (DWORD)(o_aligned >> 32),
                    (DWORD)(o_aligned), gap + bytes());
