 ...
 *
 * If 'USE_NEWBASE' is defined, the native names are interpreted as
 * '[xxx][:nnn]' with optional name part and optional numeric trailer. Either
 * name, id or both can cause a match.
 */
static bool native_match(const string &name_in_data,
                         const string &name_to_match) {
  ...
  if (!SQD_Tools::cut_at_colon(name_in_data.c_str(), a_name_part, a_id)) {
    throw E_LOG_USAGE("Bad parameter name (expecting 'xxx' or 'xxx:nnn'): '%s'",
                      name_in_data.c_str());
  }
  ...
  if (!SQD_Tools::cut_at_colon(name_to_match.c_str(), b_name_part, b_id)) {
    // 17-Oct-2011 PKi: Invalid name; do not throw (not catched), just return
    // 'nonmatch'
    //
    // throw E_LOG_USAGE( "Bad parameter name (expecting 'xxx' or 'xxx:nnn'):
    // '%s'", name_to_match.c_str() );
...
/*
 * Returns the param within 'vec' that covers 'this', or empty 'NA_Param' if
 * none.
 *
 * The function can be called either as a test or for getting a native param
 * index to accessing the data.
 */
NA_Param ApiScalarParam::covered_by(const vector<NA_Param> &vec) const {
  ...
  if (name == "")
    return NONE; // empty param not covered - ever

  // Matching logic goes:
  //  1. if standard names match, it's a match
  //  2. if native name and/or id matches (both must match if given) it's a
  //  match
  //
  for (vector<NA_Param>::const_iterator it = vec.begin(); it != vec.end();
       ++it) {
...
/*
 * Given a set of native parameters, return the API level (including vector)
 * params that can be provided out of these.
 */
vector<string> ApiParam::convert_to_api(const std::vector<NA_Param> &vec,
                                        bool prefer_standard_names) {
  vector<string> ret;
...
/*
 * Given a set of 'ApiParam' parameters (including vectors), return the native
 * params that are needed to provide these.
 */
// 25-Oct-2011 PKi: Now needed by server too
//#ifdef METQU
vector<NA_Param> ApiParam::convert_to_native(const std::vector<ApiParam> &vec,
                                             const NA_Data *data) {
  ...
  for (vector<ApiParam>::const_iterator it = vec.begin(); it != vec.end();
       ++it) {
    assert(it->a);

    // 02-Sep-2011 PKi: Check for duplicate parameters. When creating a raw
    // using template this
    //					avoids "expanding" vectors UV (when parameters contain U and V)
    //and 					WIND (when parameters contain WS and WD) if the component scalars are
    //				    already included.
    //
    //					Check is made for all parameters (scalars too) to handle
    //possible duplicate 				    parameter definitions in scripts too
    //
    // 25-Oct-2011 PKi: Ignore virtual parameters

    if (data) {
      bool mapped;
      data->mapParameter(NA_Param((it->a).getName().c_str(),
                                  NA_Param::UNIT_UNKNOWN_INTERPOLATABLE),
                         true, &mapped);
...
/*
 * 25-Oct-2011 PKi: Returns data's parameters with virtual parameters stripped
 * off
 */
 ...
 */
bool ApiParam::covered_by(const vector<NA_Param> &vec, NA_Param &na,
                          NA_Param &nb, bool &polar_) const {
