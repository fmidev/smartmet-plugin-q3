...
public:
  ...
  JDay(unsigned year_, unsigned month_, unsigned day_, unsigned hour_,
       unsigned min_, unsigned sec_)
  ...
  void set(unsigned year_, unsigned month_, unsigned day_, unsigned hour_,
           unsigned min_, unsigned sec_) {
    jd_plus_half = calc(year_, month_, day_, hour_, min_, sec_);
  }
  ...
  // Application is supposed to have checked for validity before doing
  // comparisons (keep things fast).
  //
  bool operator==(const JDay &o) const {
    return jd_plus_half == o.jd_plus_half;
  }
  ...
  //
  JDay add_secs(long secs) const {
    JDay ret(*this);
    ret.jd_plus_half += secs;
    return ret;
  }
  ...
  void gregorian(unsigned *year_ptr, unsigned *month_ptr,
...
private:
  ...
  static jd_T calc(unsigned yyyy, unsigned mm, unsigned dd) {
    //
    // These secs are from midnight, essentially adding 43200 to the right
    // 'Julian day' value (which is measured from noon).
    //
    return calc_jdn(yyyy, mm, dd) * ((jd_T)86400);
  }

  static jd_T calc(unsigned yyyy, unsigned mm, unsigned dd, unsigned hour,
  ...
  //
  // Julian day from MIDNIGHT (full days since 0:00 UTC Jan 1st 4713 BC);
  // INVALID_VALUE for empty
  //
