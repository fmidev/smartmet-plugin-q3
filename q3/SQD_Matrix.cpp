/*
 * SQD_Matrix.CPP                          Copyright (c) 2009-10, Ilmatieteen
 * laitos
 *
 * Revised:  22-Oct-2010
 */
 ...
 */
static NFmiFastQueryInfo *new_iter(NFmiQueryData *qd, const JDay &vt,
                                   const NA_Level &lev, FmiParameterName e) {
  assert(qd);
 ...
 *
 * This is rather difficult for us and the idea of tying two parameters together
 * like this causes all kinds of complications (i.e. setting 'WS' to 0 should
 * implicitly also set 'WD' in those points).
 *
 * What we do is keep the stored range but forget about the synchronization
 * between WD and WS. So, writing 0 (North) to an SQD file will have the value
 * stored as 360. This should provide enough backwards compatibility, while
 * retaining the complication issues sealed within SQD format handling (and not
 * spread to rest of q3).
 *
 * Note: Even with 'SQD_WD_WS_TRACKING' enabled, making full synchronization is
 * a shitty job. One should track not only changes to WD, but also to WS since
 * setting WS to NAN/non-NAN would have implications on the WD side of things.
...
#endif
      ,
      proj(data->getProjection()), get_f(0), set_f(0)
#ifdef SQD_WD_WS_TRACKING
      ,
      my_e(e), fi_WD(0), fi_WS(0)
#endif
...
#ifdef SQD_WD_WS_TRACKING
  if (e == kFmiWindVectorMS) {
    fi_WD = new_iter(const_cast<SQD_Data *>(data)->getQD(), vt, lev,
                     kFmiWindDirection);
  }
  if ((e == kFmiWindDirection) || (e == kFmiWindVectorMS)) {
    fi_WS = new_iter(const_cast<SQD_Data *>(data)->getQD(), vt, lev,
                     kFmiWindSpeedMS);
  }
#endif
...
#ifdef SQD_WD_WS_TRACKING
  if (fi_WD) {
    fi_WD->LocationIndex(n);
  }
  if (fi_WS) {
    fi_WS->LocationIndex(n);
  }
  ...
  if (my_e == kFmiWindVectorMS) {
    ...
    if (v == 32700.0f) {
      ...
      if ((v_WD == 32700.0f) && (v_int % 100 != 0)) {
        ...
        //
        bad = ((v_int % 100) * 10 != v_WD) ||
              ((v_int / 100) != floorf(v_WS + 0.5));
      }
    }

    if (bad) {
      // TBD: There's too many of these to keep the logging enabled. Newbase
      // seems to be at fault.
      /*
       * WARNING ... Inconsistent WVEC: 1300.000000 (WS 13.400000, WD
       * 360.000000) WARNING ... Inconsistent WVEC: 1400.000000 (WS 13.900000,
       * WD 360.000000) WARNING ... Inconsistent WVEC: 1200.000000
       * (WS 11.800000, WD 360.000000) WARNING ... Inconsistent WVEC:
       * 1200.000000 (WS 12.300000, WD 360.000000) WARNING ... Inconsistent
       * WVEC: 1300.000000 (WS 12.800000, WD 360.000000) WARNING ...
       * Inconsistent WVEC: 1300.000000 (WS 13.400000, WD 360.000000) WARNING
       * ... Inconsistent WVEC: 1100.000000 (WS 11.300000, WD 360.000000)
       * WARNING ... Inconsistent WVEC: 1200.000000 (WS 11.900000, WD
       * 360.000000)
       * ...
       */
