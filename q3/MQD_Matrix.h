/*
 * MQD_MATRIX.H                          Copyright (c) 2009-10, Ilmatieteen
 * laitos
 *
 ...
 *
 * Note: The 'data' given to 'MQD_Matrix' in constructor is expected to remain
 * valid throughout the lifespan of this object (this is on the caller to
 * guarantee!).
 */
class MQD_Matrix : public Matrix {
public:
#ifdef METQU
  MQD_Matrix(MQD_Data *data_, const JDay &time, const NA_Level &level,
             const NA_Param &p);
#endif
  MQD_Matrix(const MQD_Data *data_, const JDay &time, const NA_Level &level,
             const NA_Param &p);
