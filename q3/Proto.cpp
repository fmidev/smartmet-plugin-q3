 ...
 *
 * This sets up the "proto.*" table on the Lua side, ties it to
 * 'PROTO_REGISTRY_KEY' and allows custom addon types to be introduced by:
 *
...
ProtoProxy proto_init(lua_State *L) {
  ...
  if (lua_isnil(L, -1)) {
    ...
    //
    int st = luaL_loadbuffer(L, (char *)proto_chunk, sizeof(proto_chunk),
                             nullptr /*from precompiled*/);
    if (st) {
      luaL_error(L,
                 lua_tostring(
                     L, -1)); // can only be LUA_ERRMEM (script is precompiled)
    }
