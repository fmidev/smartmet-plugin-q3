/*
 * SQD_PROJECTION.H                        Copyright (c) 2010, Ilmatieteen
 * laitos
 *
...
public:
  SQD_Projection(const char *proj_);
  SQD_Projection(const NFmiArea *area_);
  ...
  std::string creationPrefix() const {
    return (strcmp(ap->ClassName(), "kNFmiGdalArea") ? "" : "FMI|");
  }
