...
NFmiLevel SQD_Tools::newbase_level(const NA_Level &lev) /*throw(E_BUG)*/ {
  ...
  return NFmiLevel(lt,
                   (long)lev.getValue()); // Newbase has 'long' as level value
}
...
NFmiMetTime SQD_Tools::jd2mt(const JDay &a) {
  ...
  //
  return NFmiMetTime(yyyy, mm, dd, a.hour(), a.min(), a.sec(),
                     1 /*timestep in minutes*/);
}
...
JDay SQD_Tools::mt2jd(const NFmiMetTime &mt) {
  ...
  // DAMMIT!!!  Newbase const-ness sucks.  ('NFmiStaticTime::IsMissing' should
  // be const!)
  //
#if 1
...
#endif
  {
    ...
    return JDay(mt.GetYear(), mt.GetMonth(), mt.GetDay(), mt.GetHour(),
                mt.GetMin(), mt.GetSec());
  }
}
 ...
 *
 * Returns 'true' if 'proj' was a Newbase projection (whether point is within it
 * or not). 'false' if the projection string was unknown to Newbase.
 */
#if 0
...
/*
 * NOTE: We need to define the values here (not use the 'NA_Param::' globals) so
 * that they are guaranteed to be initialized prior to 'known_params' table.
 */
static const NA_Param::Unit UNIT_FLOAT_NEAREST(NA_Param::DATATYPE_FLOAT,
                                               NA_Param::INTERPOLATE_NEAREST);
static const NA_Param::Unit UNIT_UINT16_NEAREST(NA_Param::DATATYPE_UINT16,
                                                NA_Param::INTERPOLATE_NEAREST);
static const NA_Param::Unit UNIT_MAX14_ENUM(NA_Param::DATATYPE_HALFBYTE,
                                            NA_Param::INTERPOLATE_NEAREST);
static const NA_Param::Unit UNIT_MAX254_ENUM(NA_Param::DATATYPE_BYTE,
                                             NA_Param::INTERPOLATE_NEAREST);
static const NA_Param::Unit UNIT_BOOL(NA_Param::DATATYPE_BOOL,
                                      NA_Param::INTERPOLATE_NEAREST);
static const NA_Param::Unit UNIT_DEG(NA_Param::DATATYPE_FLOAT,
                                     NA_Param::INTERPOLATE_LINEAR_DEG, "deg");
static const NA_Param::Unit UNIT_PRC(NA_Param::DATATYPE_FLOAT,
                                     NA_Param::INTERPOLATE_LINEAR, "%");
static const NA_Param::Unit UNIT_10PRC(NA_Param::DATATYPE_FLOAT,
                                       NA_Param::INTERPOLATE_LINEAR,
                                       "10%"); // 0..10 (with fractions)
static const NA_Param::Unit UNIT_100PRC(NA_Param::DATATYPE_FLOAT,
                                        NA_Param::INTERPOLATE_LINEAR,
                                        "100%"); // 0..1 (with fractions)
static const NA_Param::Unit UNIT_1(NA_Param::DATATYPE_FLOAT,
                                   NA_Param::INTERPOLATE_LINEAR,
                                   "1"); // anything (but "just numbers")

static const NA_Param::Unit UNIT_UNKNOWN(NA_Param::DATATYPE_FLOAT,
                                         NA_Param::INTERPOLATE_UNKNOWN);

#define U(s)                                                                   \
  NA_Param::Unit(NA_Param::DATATYPE_FLOAT, NA_Param::INTERPOLATE_LINEAR, (s))

//---
// Note: UNIT_HOUR and UNIT_DAY are to be used for parameters where the value
// just
//       keeps going (i.e. past 24 hours, or 365 days). For parameters dealing
//       with limited upper range (calendar time) use 'UNIT_NEAREST'.
//
static const NA_Param::Unit UNIT_HOUR(NA_Param::DATATYPE_FLOAT,
                                      NA_Param::INTERPOLATE_LINEAR, "h");
static const NA_Param::Unit UNIT_DAY(NA_Param::DATATYPE_FLOAT,
                                     NA_Param::INTERPOLATE_LINEAR, "h24");
//
// data type: float (fractions allowed)

#define PARAM(id, unit)                                                        \
  { id, unit }
...
static struct {
  ...
  const NA_Param::Unit unit;
    ...
    //---
    // Standard named scalar parameters. Using i.e. "T" points to any param with
    // id 4 (unless there is a param
    // named "T" in the file).
    //
    PARAM(/*P*/ kFmiPressure, U("hPa")),
    PARAM(/*Z*/ kFmiGeopHeight, U("gpm")), // Primary Z param
    PARAM(kFmiGeomHeight,
          U("m")), // Secondary Z param (used if 'kFmiGeopHeight' is not there)
    PARAM(/*T*/ kFmiTemperature, U("°C")),
    PARAM(/*THETAW*/ kFmiPseudoAdiabaticPotentialTemperature, U("K")),
    PARAM(/*DP*/ kFmiDewPoint, U("°C")), PARAM(/*RH*/ kFmiHumidity, UNIT_PRC),
    PARAM(/*W*/ kFmiVerticalVelocityMMS, U("mm/s")),
    PARAM(/*RRCON*/ kFmiPrecipitationConv,
          U("mm")), // or "kg/m²" (same values, since 1l of water weighs a kg)
    PARAM(/*RRLAR*/ kFmiPrecipitationLarge, U("mm")),
    PARAM(/*CAPE*/ kFmiCAPE, U("J/kg")), PARAM(/*KIND*/ kFmiKIndex, UNIT_1),
    PARAM(/*POP*/ kFmiPoP, UNIT_PRC),
    PARAM(/*TKE*/ kFmiTurbulentKineticEnergy, U("J/kg")),
    PARAM(/*PSEUDOSATEL*/ kFmiRadiationNetTopAtmLW, U("W/m²")),
    PARAM(/*LRAD*/ kFmiRadiationLW,
          U("W/m²")), // "pitkäaaltoinen säteily" / long wave radiation
    PARAM(/*SRAD*/ kFmiRadiationGlobal,
          U("W/m²")), // "auringon säteily" / sun radiation
    ...
    // NOTE: GUST parameter is broken on SQD / Newbase. (TBD: check it out in
    // detail,
    //      at least report how exactly it's broken)
    //
    PARAM(/*GUST*/ kFmiHourlyMaximumGust,
          U("m/s")), // 0..409.5 (fixed one decimal)

    PARAM(/*U*/ kFmiWindUMS, U("m/s")), PARAM(/*V*/ kFmiWindVMS, U("m/s")),

    // WVEC: Old remains param that could be discontinued; the same can be
    // acquired
    //       by simple calculation (kept alive for sake of compatibility, and
    //       for
    //       telling how to interpolate them; or rather NOT to interpolate).
    //
    PARAM(/*WVEC*/ kFmiWindVectorMS,
          UNIT_UINT16_NEAREST), // round(WS)*100 + WD/10  -> range 0..40960
                                // (theoretically)
                                //
...
//
    ...
    PARAM(kFmiLandSeaMask,
          UNIT_100PRC), // value: 0..1 (fraction of land surface underneath)

    PARAM(kFmiTopoSlope, UNIT_DEG), PARAM(kFmiTopoAzimuth, UNIT_DEG),
    ...
    PARAM(
        kFmiForecastPeriod,
        UNIT_FLOAT_NEAREST), // TBD: unit unsure; maybe minutes ?
                             // <http://tih.fmi.fi/www_sivut/tuotanto_ajot/ParamIdents.html>
    PARAM(kFmiDeltaTime,
          UNIT_BYTE_NEAREST), // 0..23 time step used in modelling
#endif
    ...
    PARAM(kFmiStabilityIndex, U("°C")), PARAM(kFmiSaturationDeficit, U("hPa")),
    ...
    PARAM(kFmiWindShearMS, U("m/s")), PARAM(kFmiWindShearKT, U("kt")),
    PARAM(kFmiLapseRate, U("°C/m")), PARAM(kFmiPrecipitableWater, U("kg/m²")),
    PARAM(kFmiPrecipitationRate, U("kg/m²")),
    PARAM(kFmiPrecipitationAmount, U("mm")), PARAM(kFmiSnowDepth, U("cm")),
    PARAM(kFmiRadiationOutLW, U("W/m²")), PARAM(kFmiRadiationOutSW, U("W/m²")),
    PARAM(kFmiRadiationInSW, U("W/m²")),
    PARAM(kFmiGolfIndex, UNIT_FLOAT_NEAREST), // TBD: range; integer only?
    PARAM(kFmiSeaLevel, U("cm")), PARAM(kFmiTemperatureSea, U("°C")),
    PARAM(kFmiSalinity, UNIT_FLOAT_NEAREST), // TBD: range; integer only?
    PARAM(kFmiDensity, UNIT_FLOAT_NEAREST),  // TBD: range; integer only?
    PARAM(kFmiSeaLevelMinimum, U("cm")), PARAM(kFmiSeaLevelMaximum, U("cm")),
    ...
    PARAM(kFmiCAPPIHeight, U("m")), PARAM(kFmiRadarRadius, U("km")),
    PARAM(kFmiRadarLayer, U("km")), PARAM(kFmiAzimuthalLayer, UNIT_DEG),
    PARAM(kFmiCrossSectionDirection, UNIT_DEG),
    PARAM(kFmiAccumulationTime, UNIT_HOUR), PARAM(kFmiMindBZ, U("dBZ")),
    PARAM(kFmiMinRange, U("km")), PARAM(kFmiMaxRange, U("km")),
    PARAM(kFmiHorizontalResolution, U("m")), // =~ GSIZE.x
    PARAM(kFmiZResolution, U("m")), PARAM(kFmiCorrectedReflectivity, U("dBZ")),
    PARAM(kFmiEchoTop, U("km")), PARAM(kFmiRainfallDepth, U("mm")),
    ...
    PARAM(kFmiCurrentSpeed, U("m/s")), PARAM(kFmiCurrentDirection, UNIT_DEG),
    PARAM(kFmiSigWaveHeight, U("cm")), PARAM(kFmiSigWavePeriod, U("1/s")),
    PARAM(kFmiWaveDirection, UNIT_DEG), PARAM(kFmiWaveSpread, UNIT_DEG),
    PARAM(kFmiMaxWaveHeight, U("cm")), PARAM(kFmiMaxWavePeriod, U("s")),
    PARAM(kFmiWavePeriod, U("s")), PARAM(kFmiSigWaveHeightBandB, U("cm")),
    ...
    PARAM(kFmiSigWaveLength, U("m")), PARAM(kFmiMixedLayerDepth, U("m")),
    ...
    PARAM(kFmiPressureTendency,
          UNIT_FLOAT_NEAREST), // TBD: unit? range? integer only?
    ...
    PARAM(kFmiWindShearU, U("m/s")), PARAM(kFmiWindShearV, U("m/s")),
    PARAM(kFmiVapourPressure, U("hPa")), PARAM(kFmiEvaporation, U("mm")),
    PARAM(kFmiFlashAccuracy, U("km")),
    PARAM(kFmiFlashMultiplicity, UNIT_FLOAT_NEAREST), // TBD: unit?
    PARAM(kFmiFlashStrength, U("kA")), PARAM(kFmiSnowfallRate, U("mm/h")),
    ...
    PARAM(kFmiConvectiveSnow, U("mm")), PARAM(kFmiLargeScaleSnow, U("mm")),
    ...
    PARAM(kFmiSoilTemperature, U("°C")), PARAM(kFmiSoilMoistureContent, U("m")),
    ...
    PARAM(kFmiRadiationSW, U("W/m²")), PARAM(kFmiRadiationReflected, U("W/m²")),
    ...
    PARAM(kFmiWindSpeedGeostr, U("m/s")), PARAM(kFmiWindSpeed10M, U("m/s")),
    PARAM(kFmiWindDirection10M, UNIT_DEG), PARAM(kFmiTemperature2M, U("°C")),
    PARAM(kFmiDewPoint2M, U("°C")), PARAM(kFmiHumidity2M, UNIT_PRC),
    PARAM(kFmiWindUMS10M, U("m/s")), PARAM(kFmiWindVMS10M, U("m/s")),
    PARAM(kFmiShipDirection, UNIT_DEG), PARAM(kFmiShipSpeed, U("m/s")),
    PARAM(kFmiChillFactor, U("°C")),
    PARAM(kFmiDegreeDays, UNIT_FLOAT_NEAREST), // TBD: unit?
    PARAM(kFmiPrecipitation3h, U("mm")), PARAM(kFmiPrecipitation6h, U("mm")),
    PARAM(kFmiPrecipitation12h, U("mm")), PARAM(kFmiPrecipitation24h, U("mm")),
    ...
    PARAM(kFmiIceThickness, UNIT_FLOAT_NEAREST), // TBD: unit?  // NB: has been
                                                 // 'kFmiWindInstantaneous'
    ...
    PARAM(kFmiForecasterCode,
          UNIT_1), // just a number (TBD: is it interpolatable or enum?)
    ...
    PARAM(kFmiCloudHeight, U("m")), PARAM(kFmiPrecipitationPeriod, UNIT_HOUR),
    PARAM(kFmiMaximumWind, U("m/s")), PARAM(kFmiWindGust, U("m/s")),
    PARAM(kFmiOldVisibility, U("m")), PARAM(kFmiVisibilityChange, U("m")),
    ...
    PARAM(kFmiSnowDepth06, U("cm")), PARAM(kFmiSnowDepth18, U("cm")),
    ...
    PARAM(kFmiTotalOzone, U("DU")), PARAM(kFmiUVCumulated, U("MED")),
    PARAM(kFmiUVMaximum, U("MED/h")),
    PARAM(kFmiTimeOfUVMaximum,
          UNIT_FLOAT_NEAREST), // hhmm  (UNIT_xxx_NEAREST forces 'nearest point'
                               // interpolation; no custom interpolation for
                               // these values - if it were in minutes we'd be
                               // able to do linear)
    ...
    PARAM(kFmiHourlyPressure, U("hPa")), PARAM(kFmiHourlyTemperature, U("°C")),
    ...
    PARAM(kFmiPrecipitation06, U("mm")), PARAM(kFmiPrecipitation18, U("mm")),
    ...
    PARAM(kFmiBaseOfTurbulence, U("m")), PARAM(kFmiTopOfTurbulence, U("m")),
    ...
    // PARAM( kFmiWindCode, UNIT_FLOAT_NEAREST ),         // not known to
    // Newbase
    // PARAM( kFmiWindVoltage, UNIT_FLOAT_NEAREST ),      // not known to
    // Newbase
    // PARAM( kFmiAverageWind, UNIT_FLOAT_NEAREST ),      // not known to
    // Newbase
    // PARAM( kFmiWindDeviation, UNIT_FLOAT_NEAREST ),    // not known to
    // Newbase
 ...
 * Init the maps
 *
 * Note: We *must* do this as a function since there is no guarantee we'd be
 * initialized before 'UNIT_DEG' and others referred to here.
 */
static struct JustOnce_SQD_Tools { // note: must have unique name (otherwise
  ...
  JustOnce_SQD_Tools() {
    for (unsigned i = 0; known_params[i].id; i++) {
      ...
      // Make sure all the entries in the table have a known interpolation
      // method.
      //
#ifndef NDEBUG
      NA_Param::e_Interpolation method = known_params[i].unit.getMethod();
      if (method == NA_Param::INTERPOLATE_UNKNOWN) {
        throw E_LOG_BUG("Built-in table has unknown interpolation: %d #%d",
                        (int)e, i);
      }
#endif
...
NA_Param::Unit SQD_Tools::unit_by_id(FmiParameterName p) {
  ...
  map<FmiParameterName, NA_Param::Unit>::const_iterator it =
      unit_by_id_.find(p);
  if (it != unit_by_id_.end()) {
    ...
    // For unknown params id's (can be i.e. custom id in a file) default to
    // linear interpolation, float range.
    //
...
/*
 * Convert parameter names to Newbase id, base on the local parameters in
 * 'info'.
 *
 ...
 */
bool SQD_Tools::cut_at_colon(const char *s, string &name_part,
                             FmiParameterName &id) {
