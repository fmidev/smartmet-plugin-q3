 ...
 *       0 pointing NORTH and growing clockwise. This is for simplicity of
 * 'getX()' and 'getY()'.
 *
...
public:
  ...
  Vector() throw() {
  } // leave uninitialized (called when 'VectorMatrix' constructor does 'new')
  /*virtual*/ ~Vector() {}

  // Do operators here, since we need to manage polar/cartesian in a unified
  // manner.
  //
  ...
  // Note: Because our rads point NORTH (0,1) at 0 and C math lib points to
  // (1,0)
  //       the use of 'cos' and 'sin' is essentially swapped (this is a 90
  //       degree phase shift).
  //
  ...
  // A cartesian vector is NAN if either of its parts is. However, we allow
  // polar vectors to only carry strength or direction (and not be NAN as a
  // vector).
  //
  bool isnan() const {
    return polar ? (isnanf(v1) && isnanf(v2)) : (isnanf(v1) || isnanf(v2));
  }
...
private:
  ...
  void _INVARIANT(const char *file, unsigned line) const {
    ...
    // Cartesian: either 'v1' or 'v2' being NAN makes the whole vector
    // (obviously) NAN.
    //
    // Polar:     'v1' and 'v2' be NAN independent of each other (i.e. we can
    // have wind
