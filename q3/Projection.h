...
/*
 * Programming level interface to projections (either SQD or Proj4 kind, via
 * 'Projection_provider')
 */
class Projection {
public:
  ...
  // Used when the particular type of the underlying projection is known for
  // sure (i.e. 'SQD_Data' reading in data)
  //
#ifdef USE_NEWBASE
  Projection(const char *s, const Projection_provider &pb)
      : proj(pb.clone_()), creation_str(s ? s : "") {
    INVARIANT();
  }
#endif
  ...
  std::string creationPrefix() const {
    return (proj ? proj->creationPrefix() : "");
  }
