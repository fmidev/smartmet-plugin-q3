...
/*
 */
void ZmqLogger::log(enum category cat, const char *file, unsigned line,
                    const Logger::unique_t id, const char *s, size_t s_len) {
#ifdef USE_ZMQ
  if (!msg)
    return; // just in case
  ...
  // NOTE: Once this code (the log format) has stabilized, we can gain some
  // extra speed
  //      by writing directly to the ZMQ buffer.

  // NOTE: DO NOT use 'string_fmt' with 's' - the message may be long (i.e. if
  // coming from a script DUMP)
