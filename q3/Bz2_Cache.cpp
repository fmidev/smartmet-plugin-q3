...
/*
 * Read the cache file and remove any entries from it where the file is
 * currently non-existing. Helps to keep the cache small(ish).
 *
...
Bz2_Cache::Bz2_Cache(const char *fn) : mapping_m(), mapping(), f(0) {
  ...
  if (begins_with(fn, "~/")) {
    ...
    throw E_LOG_FATAL("Cache filename needs to start with '/' or '~/' (%s)",
                      fn);
  }
  ...
  while (fgets(buf, sizeof(buf), f)) { // key=val\n
    char *p = strchr(buf, '=');
    if (!p) {
...
//      check validity here.
    ...
    if (!t) {
      ...
      mapping[buf] = t; // overwrite if existing (same key may exist multiple
                        // times, last one applies)
    }
  }
  ...
  if (changed) {
    ...
    for (map<string, JDay>::const_iterator it = mapping.begin();
         it != mapping.end(); ++it) {
...
void Bz2_Cache::set(const string &key, JDay ot) {
  {
    ...
    if (it != mapping.end()) {
      ...
      LOG_WARNING("Two origintimes for same file: %s != %s (%s)", tmp1.c_str(),
                  tmp2.c_str(), key.c_str());
