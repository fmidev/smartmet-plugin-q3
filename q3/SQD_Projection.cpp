/*
* SQD_PROJECTION.CPP                          Copyright (c) 2010, Ilmatieteen
laitos
*
* For more info on Newbase projection syntax, see
'newbase/source/NFmiAreaFactory.cpp'
*
<<
...
/*virtual*/ SQD_Projection::SQD_Projection(const SQD_Projection &o)
    : Projection_provider(),
      ap(o.ap.get()->Clone()) { // using 'NFmiArea::Clone'
 ...
 * point 'll'.
 *
 * Returns 'true' if 'dx' and 'dy' were written (whether they're inside
 * projection or not) 'false' if 'dx' and/or 'dy' were not written (outside
 * projection)
 *
 * Note: Possibility of returning 'false' is an optimization for projection
 * implementations where it would be fast to see if something's outside. We can
 * always return 'true'
 *       - the caller will still check if 'dx' and 'dy' are within 0..1 range.
 */
/*virtual*/ bool SQD_Projection::at(const LatLon &ll, double &dx,
                                    double &dy) const {

  // TBD: Is this the way to go - should we generate such a grid at constructor
  // or do we
