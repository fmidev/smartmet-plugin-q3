/*
 * CONTOUR_TRON.CPP                          Copyright (c) 2010, Ilmatieteen
 * laitos
 *
...
// Must be included before the rest
...
public:
  ...
  size_type width() const { return cm.getXS(); }

  size_type height() const { return cm.getYS(); }
  ...
  bool valid(size_t i, size_t j) const { return true; }
...
private:
  typedef Tron::Contourer<MyGrid, Tron::FmiBuilder, MyTraits,
...
public:
  Tron_EdgeAdapter(/*const 08-Dec-2011 PKi*/ ContourMatrix &cm)
    ...
    MyContourer::fill(builder, mg, NAN,
                      NAN); // limits of the data (holes or grid edge)
...
public:
  ...
  MyHints &hints(MyGrid &mg) {
    if (!itsHints)
      itsHints = new MyHints(mg);
    return *itsHints;
  }
...
/*---=== ContourAdapter ===---
 *
 * Class for tying into the Tron for gaining N contours of matrix 'm' value
 * 'val'.
 */
...
public:
  Tron_ContourAdapter(ContourCollector &cc_,
                      /*const 08-Dec-2011 PKi*/ ContourMatrix &cm, float lo_val,
                      float hi_val, unsigned int smooth_length,
                      unsigned int smooth_degree, TronHints *th)
      : cc(cc_), mg(cm), current_contour(0)
#if TRON_MODE == 2
        ,
        edges(nullptr), x_max(cm.getXS() - 1), y_max(cm.getYS() - 1),
        itsGeomFactory(geos::geom::GeometryFactory::create())
#endif
  {
#if TRON_MODE == 3
    // Here Tron gives edge information to 'moveto()' and 'lineto()' calls (and
    // all paths are closed)
    //
    MyContourer::fill(
        *this, mg, val, NAN,
        false); // iso curves at value 'val' (right hand side is "uphill")
    ...
    if (!isnanf(lo_val)) {
      ...
      //
      // 01-Mar-2012 PKi: Smoothening length must be smaller than matrix
      // dimensions; adjust the
      //				    length if necessary
      //
      if (smooth_length > 0) {
        if ((int)smooth_length > x_max)
          smooth_length = x_max;
        if ((int)smooth_length > y_max)
          smooth_length = y_max;
      ...
      // 'this->lineto' will have access to edges and see which points are at
      // edge
      //
      ...
      if (th)
        ...
        MyContourer::fill(builder, mg, (lo_val == 32700) ? NAN : lo_val,
                          hi_val);
...
#error "TRON mode 1 removed"
  ...
  void moveto(double x, double y, bool edge) {
    current_contour = cc.new_contour(); // pushed on the caller side's (to us
                                        // hidden) Lua stack
  ...
  void lineto(const geos::geom::Coordinate &coordinate) {
    ...
    if (edges) {
      ...
      //
      double eps[] = {coordinate.x, coordinate.y, coordinate.x - x_max,
                      coordinate.y - y_max};
...
private:
  ContourCollector &cc;
  MyGrid mg;
  Contour *current_contour; // nullptr initially, copy of a pointer maintained
                            // by the caller (not to be released)

#if TRON_MODE == 2
  const Tron_EdgeAdapter
      *edges; // giving 'lineto()' visibility to edges we live in
  MatrixPos::xy_t x_max, y_max;
  geos::geom::GeometryFactory::Ptr itsGeomFactory;
#endif
};
...
void tron_contour(ContourCollector &cc,
  ...
  // 05-Jun-2012 PKi: If tron hints are to be used (L is not null) and the
  // 'TronHints' arg is nil, TronHints object is pushed onto stack;
  //					otherwise the top of stack is adjusted down by
  //1 (nothing was pushed) and the hints are just used and returned
  //
  // 10-Jan-2014 PKi: Even if tron hints were passed in (contour.lua was
  // modified not to do so if resmoothing is to be done),
  //					push new object if smooth factor is given; the data
  //will be (re)smoothed
  ...
  Tron_ContourAdapter curves(cc, cm, lo_val, hi_val, smooth_length,
                             smooth_degree, th);
}
