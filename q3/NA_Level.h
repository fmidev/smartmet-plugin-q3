...
public:
  enum Type {
    ...
    HYBRID_LEVEL, // "hybrid[:nnn]"   Specific model levels (no interpolation);
                  // 'lv' is integer
    ...
    PRESSUREORHYBRID_LEVEL // Used only to select latest origintime from
                           // pressure or hybrid data
  };

#ifdef MQD_ENABLED
  NA_Level(const char *s);
#endif
  // NA_Level( const char *lt, double lv );
  ...
  bool operator==(const NA_Level &o) const {
    return (lt == o.lt) &&
           ((lt == GROUND_LEVEL) || (lt == SOUNDING_LEVEL) || (lv == o.lv));
  }
...
private:
  static bool levels_range(enum Type lt_, const std::vector<NA_Level> &vec,
                           double &lv_min, double &lv_max);
  static bool has_at_least_n_of(enum Type lt_, const std::vector<NA_Level> &vec,
                                unsigned n);
