 ...
 */
static void parse_rgba(lua_State *L, int index, double &r, double &g, double &b,
                       double *a_ptr) {

  if (lua_type(L, 1) == LUA_TSTRING) {
    const char *s = lua_tostring(L, index++);
    if (*s == '#')
      ++s; // beginning hash is optional
    char *endp;
...
/*
 */
Context::Context(cairo_surface_t *cs) : cr(cairo_create(cs)) {
 ...
 *
 * TBD: Instead of doing it like this, we could tie 'cr' to its target in Lua,
 * via registry or something. Might be easier - handling memory with shared
 * resources ties up into problems rather fast.   AKa 15-Dec-2010
 */
#if 0
...
/*
 * cr= cr.select_font_face( family_str [,"normal"|"italic"|"oblique"]
 * [,"normal"|"bold"] )
 *
    ...
    if (!lua_isnil(L, i)) {
      if (!conv_font_slant.just_try(L, i, slant)) {
        if (!conv_font_weight.just_try(L, i, weight)) {
          luaL_error(L, "Unexpected slant or weight: '%s'",
                     L_string_or_typename(i));
        }
      }
    }
  }
 ...
 *               [subpixel_order= "default"|"rgb"|"bgr"|"vrgb"|"vbgr",]  -- not
 * implemented (not needed) [hint_style=
 * "default"|"none"|"slight"|"medium"|"full",] [hint_metrics=
 * "default"|"off"|"on",] } )
 */
int Context::set_font_options(lua_State *L) {
  proto(L, "{ antialias=[string], subpixel_order=[string], "
           "hint_style=[string], hint_metrics=[string] }");
  Context &my = *Context::instance(L, lua_upvalueindex(1));
  ...
  cairo_font_options_t *opt = cairo_font_options_create();
  {
    ...
    while (lua_next(L, t_idx)) {
      ...
      if (!key)
        continue; // skip non-string keys (should not really be any)
 ...
 *   antialias= "default"|"none"|"gray"|"subpixel",
 *   [subpixel_order= "default"|"rgb"|"bgr"|"vrgb"|"vbgr",]      -- not
 * implemented hint_style= "default"|"none"|"slight"|"medium"|"full",
...
int Context::get_font_options(lua_State *L) {
  ...
  cairo_font_options_t *opt = cairo_font_options_create();
  {
    ...
    lua_pushstring(L,
                   conv_hint_metrics(cairo_font_options_get_hint_metrics(opt)));
    lua_settable(L, -3);
  }
  cairo_font_options_destroy(opt);
  status_check_ok(L, cairo_status(my));
...
int Context::get_font_face(lua_State *L) {
  ...
  //
  // "This object is owned by cairo. To keep a reference to it, you must call
  // 'cairo_font_face_reference()'. This function never returns nullptr."
