/*
 * PROJECTION.CPP                          Copyright (c) 2010, Ilmatieteen
 * laitos
 */
...
Projection::Projection(const char *proj_) throw(E_USAGE)
...
#ifdef USE_NEWBASE
  try {
    proj = new SQD_Projection(
        proj_); // throws 'runtime_error' if string not recognized
    goto CHECK_INVARIANT;
  ...
  //
  try {
    proj = new Proj4_Projection(
        proj_); // throws 'E_USAGE' if string not recognized
    goto CHECK_INVARIANT;
  } catch (const E_USAGE &e) {
    // We can choose here to keep the Newbase error (already in 'reason') or
    // overwrite with ours.
    //
    reason = e.what();
...
Projection::Projection(const Projection &o)
    : proj(o.proj ? o.proj->clone_() : nullptr), creation_str(o.creation_str) {
...
/*
 */
Projection::~Projection() { delete proj; }
