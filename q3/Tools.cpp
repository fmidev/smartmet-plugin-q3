...
//
// 24-Aug-2011 PKi: If USE_NEWBASE_FL is defined, using newbase flightlevel
// calculation
...
const char *pthread_rc2str(int rc) {
  ...
  return (rc == EBUSY) ? "EBUSY" : (rc == EINVAL) ? "EINVAL" : buf;
}
...
void Sleep_ms(unsigned ms) {
  struct timespec ts_sleep = {(time_t)(ms / 1000),
                              (long)((ms % 1000)) * 1000000L /*ns*/};
...
const char *glob_fn(const char *fn_mask, glob_t &gbuf, int &gbuf_i,
  ...
  if (gbuf_i < 0) { /* first call */
    ...
    // GLOB_NOMATCH:    pattern did not match a pathname and GLOB_NOCHECK was
    // not set
  ...
  while ((size_t)gbuf_i < gbuf.gl_pathc) {
    ...
    if (stat(fn, &st) != 0) {
      ...
      // system ELOOP    Too many symbolic links are encountered in translating
      // the pathname.
...
void remove_file(const char *fn) {
  ...
  if (rc != 0) {
    ...
    // ENAMET:  Component of a pathname exceeds {NAME_MAX} characters, or an
    // entire path name exceeds {PATH_MAX} ENOENT:  file does not exist ENOTDI:
    // Component of the path prefix is not a directory EPERM:   The named file
    // is a directory and no sudo rights
    //          The directory containing the file is marked sticky, and neither
    //          the containing directory nor the file to be removed are owned by
...
/*
 * Returns: -1 if 'c' is not available in Unicode (or rather: not defined in
 * Latin1).
 */
...
static int utf8_to_latin1(unsigned utf8) {
  ...
  if (d) {
    ...
    if (((c & 0xf0) == 0xe0) && ((b & 0xc0) == 0x80) &&
        ((a & 0xc0) == 0x80)) { // U+0800..U+FFFF
      unsigned z = c & 0x0f;
      unsigned y = b & 0x3f;
      unsigned x = a & 0x3f;
      uc = (z << 12) | (y << 6) | x;
 ...
 *
 * Needed for reading of SQD data and conversion of 'FmiNames' location names
 * (s.a. 'Hämeenlinna'); Newbase and FmiNames use Latin1 internally, even in
 * Linux.
 *
...
string_or_null utf8_to_latin1(const char *utf8) {
  ...
  for (const unsigned char *p = (const unsigned char *)utf8; *p;) {
    ...
    // Note: For multibyte encoding we check the tail bytes actually are
    // nonzero.
    //      This protects from memory access errors with malformed strings (=
    //      we're tolerant to anything thrown at us, as long as it's zero
    //      terminated).
    //
    if (*p <= 0x7f) {
      u = *p++;
    ...
    BAD_UTF8:
      throw E_LOG_USAGE("Bad UTF8 data (unable to convert to Latin-1): %s",
                        utf8);
    }
...
NA_Level one_level(lua_State *L, int idx, const char *lt_name) throw(E_USAGE) {
  ...
  bool true_only = (strcmp(lt_name, "ground") == 0 ||
                    strcmp(lt_name, "sounding") == 0); // approve 'true' only
...
#ifdef CONFIG_FLIGHT_LEVELS_API
  NA_Level::Type lt =
      is_flight ? NA_Level::PRESSURE_LEVEL : NA_Level::lt_enum(lt_name);
#else
  ...
  case LUA_TBOOLEAN:
    if (lua_toboolean(L, idx)) {
      // 'flight=true' will cause a 'hpa=true' (matters only if
      // CONFIG_FLIGHT_LEVELS_API defined)
      //
      return NA_Level(lt);
    }
    break;
...
void one_or_many_levels(lua_State *L, int idx, const char *lt_name,
                        vector<NA_Level> &vec) throw(E_USAGE) {
  ...
  if (!lua_istable(L, idx) || true_only) {
    ...
    for (unsigned i = 1; i <= n; i++) {
      ...
      if (lua_isnumber(L, -1)) {
        ...
        throw E_LOG_USAGE("Bad '%s' value: %s", lt_name,
                          L_string_or_typename(-1));
      }
    }
  }
}
