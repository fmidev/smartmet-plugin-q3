...
/*
 * Note: Don't use these in initialization of other globals; the order of
 * initialization is undefined, and these will only be 'there' once 'main()'
 * starts.
 *
 *       'SQD_Tools.cpp' has a similar set for its own needs (for the above
 * reason). AKa 15-Nov-10
 */
const NA_Param::Unit
    NA_Param::UNIT_FLOAT_NEAREST(NA_Param::DATATYPE_FLOAT,
                                 NA_Param::INTERPOLATE_NEAREST);
const NA_Param::Unit
    NA_Param::UNIT_UINT16_NEAREST(NA_Param::DATATYPE_UINT16,
                                  NA_Param::INTERPOLATE_NEAREST);

const NA_Param::Unit NA_Param::UNIT_MAX14_ENUM(NA_Param::DATATYPE_HALFBYTE,
                                               NA_Param::INTERPOLATE_NEAREST);
const NA_Param::Unit NA_Param::UNIT_MAX254_ENUM(NA_Param::DATATYPE_BYTE,
                                                NA_Param::INTERPOLATE_NEAREST);
const NA_Param::Unit NA_Param::UNIT_BOOL(NA_Param::DATATYPE_BOOL,
                                         NA_Param::INTERPOLATE_NEAREST);

const NA_Param::Unit NA_Param::UNIT_DEG(NA_Param::DATATYPE_FLOAT,
                                        NA_Param::INTERPOLATE_LINEAR_DEG,
                                        "deg");

const NA_Param::Unit NA_Param::UNIT_LON(NA_Param::DATATYPE_FLOAT,
                                        NA_Param::INTERPOLATE_LINEAR_LON,
                                        "deg"); // -179.99 .. 180.0
const NA_Param::Unit NA_Param::UNIT_LAT(NA_Param::DATATYPE_FLOAT,
                                        NA_Param::INTERPOLATE_LINEAR,
                                        "deg"); // -90..90 (no wrap-around)

const NA_Param::Unit NA_Param::UNIT_PRC(NA_Param::DATATYPE_FLOAT,
                                        NA_Param::INTERPOLATE_LINEAR, "%");
const NA_Param::Unit NA_Param::UNIT_10PRC(NA_Param::DATATYPE_FLOAT,
                                          NA_Param::INTERPOLATE_LINEAR,
                                          "10%"); // 0..10 (with fractions)
const NA_Param::Unit NA_Param::UNIT_100PRC(NA_Param::DATATYPE_FLOAT,
                                           NA_Param::INTERPOLATE_LINEAR,
                                           "100%"); // 0..1 (with fractions)
const NA_Param::Unit NA_Param::UNIT_1(NA_Param::DATATYPE_FLOAT,
                                      NA_Param::INTERPOLATE_LINEAR,
 ...
 */
const NA_Param::Unit NA_Param::UNIT_UNKNOWN_(NA_Param::DATATYPE_FLOAT,
 ...
 */
const NA_Param::Unit NA_Param::UNIT_UNKNOWN_INTERPOLATABLE(
    NA_Param::DATATYPE_FLOAT, NA_Param::INTERPOLATE_LINEAR, nullptr);
...
static struct {
  ...
  const FmiParameterName id; // native id in SQD file
    ...
    // WVEC is essentially useless on q3/metqu, but we support it to cover all
    // combo params (derivatives of ':19' and ':326').
    //
...
static struct {
  ...
  virtualParameterName v;    // primary/secondary parameter id:s
    ...
    {virtual_Z.toString(true), virtual_Z.getNativeName_(),
     virtualParameterName((FmiParameterName)SQD_PRIMARY_Z,
                          (FmiParameterName)SQD_SECONDARY_Z)},
#endif
#ifdef SQD_CASCADE_W_ENABLED
    {virtual_W.toString(true), virtual_W.getNativeName_(),
     virtualParameterName((FmiParameterName)SQD_PRIMARY_W,
                          (FmiParameterName)SQD_SECONDARY_W)},
#endif
    {"", "", virtualParameterName((FmiParameterName)0, (FmiParameterName)0)}};

static struct JustOnce_NA_Param { // note: must have unique name (otherwise
  ...
  JustOnce_NA_Param() {
    ...
    for (unsigned i = 0; virtual_params[i].v.primaryId; i++) {
      id_by_virtualname.insert(
          make_pair(virtual_params[i].nativeName, virtual_params[i].v));
      virtualname_by_id.insert(make_pair(virtual_params[i].v.primaryId,
                                         virtual_params[i].standardName));
      virtualname_by_id.insert(make_pair(virtual_params[i].v.secondaryId,
                                         virtual_params[i].standardName));
      virtualname_by_id.insert(make_pair(virtual_params[i].v.primaryId,
                                         virtual_params[i].nativeName));
      virtualname_by_id.insert(make_pair(virtual_params[i].v.secondaryId,
                                         virtual_params[i].nativeName));
    }
  }
 ...
 *
 * Returns "" (empty string) if 'id' is not known, or does not have a standard
 * name.
 */
...
/*
 * 31-Oct-2011 PKi: Parameter mapping for virtual parameters; returns
 * primary/secondary parameter id:s for virtual parameter
 */
virtualParameterName NA_Param::virtual_param_native_id(const char *s) {

  if (s) {
    map<string, virtualParameterName>::const_iterator it =
        id_by_virtualname.find(s);

    if (it != id_by_virtualname.end())
      return it->second; // the id:s
  }

  return virtualParameterName((FmiParameterName)0,
                              (FmiParameterName)0); // not a virtual param
}

/*
 * 31-Oct-2011 PKi: Virtual parameter mapping for native parameters; returns
 * virtual parameter name for native parameter id
 *
 *                  Returns "" (empty string) if 'id' is not known, or does not
 * have a virtual name.
 */
 ...
 */
//#ifdef USE_TESTRAW
NA_Param::NA_Param(const char *s, const Unit &unit_, bool nonStd)
    : std_name(), native_name(), unit(unit_) {
#ifdef USE_NEWBASE
  if (strchr(s, ':')) {
    native_name = s;

  } else {
    // Note: 'USE_NEWBASE' makes our invariant require a native name, and id. As
    // long as we don't
    //       save these params to a file, the id need not be unique (in
    //       practise, of course it should).
    //
    // 20-Oct-2011 PKi: Currently used for Z and W; although not saving them
    // into a file, embed name's
    //                  first character into the id to keep them unique.
    //
    //                  Native name is used to detect virtual parameters when
    //                  needed.
    int pId = s[0];
...
/*
 * Constructor used when defining a set of native parameters to be used in
 * create-from-the-scratch Raw file.
 *
 * 's' can be either:
 *       "XXX:NNN" where both the native name and id are provided
 *       "XXX" where a *standard* parameter name is provided
 *
 *       If 'USE_NEWBASE' is defined, one is not allowed to give unknown names
 * (i.e. all params must carry a nonzero id).
 *
 *       If 'USE_NEWBASE' is not defined we can deal with any names as such (no
 * need for id's).
 */
// 25-Oct-2011 PKi: Now needed by server too
//#ifdef METQU
NA_Param::NA_Param(const char *s)
    : std_name(), native_name(), unit(UNIT_UNKNOWN_) {
  assert(s);
  ...
  if (strchr(s, ':')) { // native name (i.e. "Lämpötila:4")
    ...
    if (!SQD_Tools::cut_at_colon(s, ignored, id)) {
      throw E_LOG_USAGE(
          "Bad parameter name (expecting 'xxx' or 'xxx:nnn'): '%s'", s);
    }
    (void)ignored;
    ...
    // If the name is "WS", "WD", "T" or some other standard name, act
    // accordingly (we even know how to set the unit for those).
    //
    FmiParameterName e = standard_param_native_id(s);

    if (e) {
      // Keep the provided standard name as native name, with the id (i.e.
      // "N:79" instead of just ":79"). This is to give users of SQD data (i.e.
      // via an editor application) a hint on what the parameters carry.
      //
      ...
      throw E_LOG_USAGE(
          "Bad parameter name (not a standard name, no ':nnn' id tail): '%s'",
          s);
    }
  }
#else
  ...
  //
  // TBD: We should have a list of standard names. If 's' is some of those (i.e.
  // "WS", "WD", "T") we
  //      could store it as 'std_name' instead of (or in addition to)
  //      'native_name'.
  //
  native_name = s;
#endif
 ...
 *
 * Note: SQD file always has a non-zero id on its parameters. They may or may
 * not have a name part.
 */
#ifdef USE_NEWBASE
NA_Param::NA_Param(FmiParameterName e, const std::string &sqd_name_,
    ...
    : std_name(standard_param_name(e)), // i.e. "T" for id 4
      native_name(string_fmt("%s:%d", sqd_name_.c_str(), (int)e)), unit(unit_),
      interpolation_name(interpolation_name_), precision(precision_) {
 ...
 */
#ifdef USE_NEWBASE
NA_Param::NA_Param(FmiParameterName e, const std::string &interpolation_name_,
                   const std::string &precision_)
    : std_name(standard_param_name(e)), // i.e. "T" for id 4
      native_name(string_fmt(":%d", (int)e)), unit(SQD_Tools::unit_by_id(e)),
      interpolation_name(interpolation_name_), precision(precision_) {
...
/*
 * What is presented of a parameter to the user, in i.e. 'r.params' (and
 * 'r.native_params') list.
 */
