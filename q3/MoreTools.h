 ...
 *
 * Tools that were not wanted to be placed in 'Tools.h' because of possible
 * cyclic header reads etc. Anyways, they are here.
 *
...
struct T_String {
  ...
  static const std::string &toString(const std::string &s) throw() { return s; }
};
...
struct T_Level {
  ...
  static std::string toString(const NA_Level &p) throw() {
    return p.toString();
  }
};
...
struct T_NativeParam {
  ...
  static std::string toString(const NA_Param &p) throw() {
...
struct T_JDay {
  ...
  //
  static std::string toString(const JDay &t) throw() { return t.toString(); }
};
...
vector_of_(lua_State *L, int idx,
  ...
  try {
    ...
    // Note: don't make Lua error here, since upper levels may have other ideas
    // (i.e. return nil+err_str).
    //
...
pair_of_(lua_State *L, int idx) throw(E_USAGE) {
  ...
  try {
    for (unsigned i = 1; i <= 2; i++) {
      ...
      if (i == 1)
        ret.first = T::create(L, -1); // can throw E_USAGE
      else
        ret.second = T::create(L, -1); // -''-
...
/*
 * Make a string output of vector of times, levels or params (for MQD output;
 * see 'MQD_Data.cpp').
 */
#ifdef MQD_ENABLED
template <typename T>
void output_vector(const std::vector<typename T::ValueT> &vec,
                   std::ostream &os) throw() {

  os << "{ ";

  bool first = true;
  for (typename std::vector<typename T::ValueT>::const_iterator it =
           vec.begin();
       it != vec.end(); ++it) {
    if (first)
      first = false;
...
inline std::ostream &operator<<(std::ostream &os,
                                const std::vector<JDay> &vec) throw() {
  output_vector<T_JDay>(vec, os);
  return os;
}

inline std::ostream &operator<<(std::ostream &os,
                                const std::vector<NA_Level> &vec) throw() {
...
#ifdef METQU
inline std::ostream &operator<<(std::ostream &os,
                                const std::vector<NA_Param> &vec) throw() {
...
#ifdef METQU
inline std::ostream &operator<<(std::ostream &os,
                                const std::vector<std::string> &vec) throw() {
