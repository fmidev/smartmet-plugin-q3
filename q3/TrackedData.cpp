...
const NA_Info &TrackedData::getInfo() const {
  ...
  if (!info_) {
    ...
    if (!info_) {
      ...
      //
#ifdef MQD_ENABLED
      // Try with MQD first, we're able to tell from the first line if it's an
      // MQD file.
      //
...
void TrackedData::Wipe_LOCKED() /*throw(E_FATAL)*/ {
  ...
  */
  ...
  const char *fn_f = strrchr(source.c_str(), '/');
  if (fn_f && *(fn_f + 1))
    fn_f++;
  else
    fn_f = source.c_str();
 ...
 *
 * Note: The 'tmp_threshold' value defines how much free space is required to
 * exist BEFORE the extraction of a data, for it to be extracted on disk. It
 * does NOT guarantee keeping that much of free space - apply a wide safety
 * margin!
 *
 * Throws: E_BAD_FILE if we don't have access rights to open the file (we may
...
NA_Data *TrackedData::Acquire(bool metaQuery) throw(E_BAD_FILE, E_BUG) {
...
#ifndef METQU
  {
    ...
    if (!qd) {
      ...
      // Check if we're wished to extract a .BZ2 file on disk, instead of the
      // default (opening it in memory via Newbase).
      //
#if 1
      // TBD: we need to remake the whole archive/extraction code (use MQD with
      // compression built-in to the format)
#else
    ...
    if (tp) {
      ...
      // Note: 'mkstemp()' both modifies 'buf' AND opens a file (in read-write
      // mode)
      //      so we don't have to worry about two threads getting the same
      //      filename.
      //      (( Do NOT use the regular 'mktemp()' here. ))
      //
      int fd = mkstemp(buf);
      if (fd == -1) {
        // Just proceed and load the file via Newbase (as if 'tmp_pattern' was
        // nullptr)
        //
        ...
        if (av > tmp_threshold) {
          ...
          if (bytes < 0) {
            LOG_WARNING(
                "Unable to extract %s -> %s (we'll load to memory instead)",
                source.c_str(), buf);
          } else {
 ...
 * Wipe data
 *
 * If the data has not been acquired since 'wiping' and 'metawiping' seconds,
 * and it's currently not in use, close it (releasing the memory mapping).
 *
  ...
  if ((refcount == 0) &&
      ((wiping == 0) ||
       (((time(nullptr) - last_acquired) > (int)wiping) &&
        ((meta_acquired == 0) ||
         ((time(nullptr) - meta_acquired) > (int)metawiping))))) {
    Wipe_LOCKED();
    meta_acquired = 0;
    assert(qd == 0);
    return true;
  }
  // printf("*** %s NOT wiped: refcnt=%ld, wip=%u, ldff=%ld, mwip=%u,
  // mdff=%ld***\n",source.c_str(),refcount,wiping,time(nullptr) -
  // last_acquired,metawiping,time(nullptr) - meta_acquired);
  return false;
}
#endif
