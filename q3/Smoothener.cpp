 ...
 *   - SMOOTH_COPY whether to smoothen the curve as a copy or in place
 * (default). If smoothing in place, smoothed points affect smoothing of
 * succeeding points.
 *
...
void SmoothPath::done(float factor, bool closed_) {
  ...
  if (factor <= 0.0)
    return; // nothing to do (use as is)
  ...
  for (vector<PointAndEdge>::const_iterator it = rim.begin(); it != rim.end();
       ++it) {
  ...
  double total_sum =
      0.0; // sum of all (moved) points from the center (before they were moved)
#endif
...
#endif
        mir[i] +=
            Vectors::Vector(rim[i], avg / WINDOW) * factor; // move the point
