...
public:
  ...
  enum OriginTimeQuery { Current, Archived, All, NA };

  TrackedData *getData_must_release(const JDay &origin_time, bool archivedData,
                                    bool metaQuery) throw();
  std::vector<JDay> getOriginTimes_sort() const throw();
  void addOriginTimes_unsorted(std::vector<JDay> &vec,
                               OriginTimeQuery originTimeQuery = Current) const
      throw();
  void addDatas_must_release(const JDay &ot,
                             std::vector<TrackedData *> &vec) const throw();
...
protected:
  TrackerBase(unsigned refresh_secs_, unsigned wiping_[], bool relative_uv_,
...
protected:
  ...
  unsigned wiping[4]; // wiping cycle for "normal", archived and metadata
                      // access, plus an extra slot for filemask specific
                      // setting ("normal" or archived)
  bool relative_uv; // Set if U and V are relative to the grid

private:
  const unsigned
      refresh_secs; // Look for new archives every N secs (0=no refresh)
