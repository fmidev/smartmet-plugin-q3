/*
 * VECTORMATRIX.H                        Copyright (c) 2009-10, Ilmatieteen
 * laitos
 *
...
public:
  ...
  const MatrixSize &getSize() const {
    assert(
        m1); // detect if 'getSize()' is being used in constructor (not allowed)
    return m1->getSize();
  }

  MatrixPos getGridSize() const {
    assert(m1); // detect if 'getGridSize()' is being used in constructor (not
                // allowed)
  ...
  // Return direct pointers to the submatrices (or nullptr if needs reading via
  // Vector iteration and '.getX()', '.getY()', '.getAbs()' or '.getDeg()')
  //
  ...
  void push_cartesian_x(lua_State *L, int b_index) const {
    push_cartesian(L, b_index, true /*x*/);
  }
  void push_cartesian_y(lua_State *L, int b_index) const {
    push_cartesian(L, b_index, false /*y*/);
  }
...
private:
  ...
  Matrix
      *m2; // (no deletion on these, they are handled by Lua GC automatically!)
