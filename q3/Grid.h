...
public:
  ...
  static const char *env_mode() {
    return "v";
  } // instance environment table, weak values
...
/*
 * Holds certain time, level and projection/gridsize combination and allows
 * reading all parameters from that.
 */
class Grid : public LuaNew<GridBind> {
public:
#ifdef METQU
  Grid(Raw_interface &r_, const JDay &vt, const NA_Level &lev,
       const Projection &projection,
       const MatrixPos &gs = MatrixPos::ZERO
       //
  ...
  CONST_IF_SERVER ApiMatrix *push_Matrix(lua_State *L,
                                         const ApiParam &p) CONST_IF_SERVER;

  float at(const LatLon &ll,
           const NA_Param &p) const; // single value at given location

  const MatrixPos &getGridSize() const {
    return gridsize;
  } // 'MatrixPos::ZERO' for native gridsize
  ...
  CONST_IF_SERVER Matrix *push_ScalarMatrix(lua_State *L,
...
private:
  ...
  const MatrixPos gridsize; // 'MatrixPos::ZERO' for native grid size, otherwise
                            // non-native (do scaling)

  unsigned rkey; // magic number used by 'push_alive()' to get a Lua instance of
                 // 'r' needed for connecting also '[SQD|MQD]_Matrix' objects to
                 // the same instance's lifecycle. (0 immediately after
                 // construction, but soon set thereafter)

  const std::vector<JDay>
      vtVec; // 22-Sep-2011 PKi: For fetching cross using newbase
  const std::vector<NA_Level> levelVec; //
  const LatLonList locs;                //
  bool flightRoute;   // 05-Mar-2015 PKi: Flag for flightroute query (one value
                      // per loc/level/time)
  DataIdList dataIds; // 07-Apr-2015 PKi: Data point (e.g. observation station)
                      // id's for point data query
  ...
  void _INVARIANT(const char *file, unsigned line) const {
    ...
    // 'gridsize' is allowed to be empty (marking "native grid size for each
    // param tile")
  }
#endif
};
