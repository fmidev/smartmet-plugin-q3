 ...
 * mapped.
 *
 * 'last_acquired' and 'meta_acquired' fields are set when the data is acquired;
 * they are used to find data to wipe out.
 */
class TrackedData {
public:
  TrackedData(const char *fn, const JDay &ot_given_,
              const string_or_null &tmp_pattern_, size_t tmp_threshold,
              bool relative_uv) throw(E_BAD_FILE, E_USAGE);
  TrackedData(const char *fn, const NA_Info &info_given, bool relative_uv_,
              const string_or_null &tmp_pattern_ = 0,
              size_t tmp_threshold_ = 0) throw(E_USAGE);
  ~TrackedData();

  NA_Data *Acquire(bool metaQuery = false) throw(
      E_BAD_FILE, E_BUG); // get data with increasing reference count
  ...
  JDay getOriginTime() const {
    return ot_given ? ot_given : getInfo().getOriginTime();
  }
...
private:
  ...
  string_or_null tmp_fn; // if BZ2 which is extracted to disk, this holds the
                         // extraction filename (for removal)
