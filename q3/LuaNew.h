 ...
 *
 * WARNING:
 *   Do NOT use the Lua stack in constructor of an object derived from
 * 'LuaObject'. Doing so will trigger a "pure virtual" function call (via
 * 'LuaObject::__gc') if a garbage collection cycle is initiated by accessing
 * the Lua stack.
 *   -- AKa 23-Nov-2009
 */
#ifndef LUANEW_H
 ...
 *   given on Lua stack. If the value is not userdata, or not of the right
 *class, NULL is returned (just as 'lua_toxxx()' does).
 */
...
protected:
  ...
  virtual ~LuaNew_base() = 0; // gateway from Lua GC -> C++ destructor of
                              // derived class

protected:
  typedef void (*f_setup)(lua_State *L);
  static void create_mt(lua_State *L, f_setup, const LuaNew_ID &_id,
                        const char *name);

  static LuaNew_base *instance(lua_State *L, int index, const LuaNew_ID &_id);

  static void *push(lua_State *L, size_t size, const LuaNew_ID &_id,
...
public:
  ...
  static unsigned /*key*/ keep_alive(lua_State *L, int index_this,
                                     int index_ref);
 ...
 *
 * 06-Feb-2012 PKi: References to BIND::ID replaced by call to BIND_T::id(),
 *thus avoiding libfmi-q3.so build dependency to newcairo library (was made
 *01-Feb-2012) which in turn avoids rpm build problem (does not handle the
...
public:
  ...
  static void *operator new(size_t size, lua_State *L) {
    return LuaNew_base::push(L, size, BIND_T::id(), BIND_T::name(),
                             BIND_T::env_mode());
  }
  ...
  static CAST_REF instance_notnull(lua_State *L, int index) {
    CAST_PTR p =
        (CAST_PTR)(void *)LuaNew_base::instance(L, index, BIND_T::id());
    if (!p) {
      luaL_error(L, "Expecting '%s', got %s", BIND_T::name(),
                 L_typename(index));
    }
    return *p;
  }
  ...
  static const char *name() { return BIND_T::name(); }
};
...
#define L_GROW(n)                                                              \
  {                                                                            \
    if (!lua_checkstack(L, n)) {                                               \
      throw std::runtime_error("Out of Lua stack");                            \
    }                                                                          \
  }
...
#define L_MID(change)                                                          \
  {                                                                            \
    ...
    if (__tos_now != __tos + change) {                                         \
      ...
      luaL_error(L,                                                            \
                 "Stack assert failed at %s:%d: has %d items instead of %d",   \
