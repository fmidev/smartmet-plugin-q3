 ...
 */
LuaNew_base *LuaNew_base::instance(lua_State *L, int index,
                                   const LuaNew_ID &ID) {
  void *ud = lua_touserdata(L, index);
  if (!ud)
    return 0; // was not a userdata
 ...
 */
void *LuaNew_base::push(lua_State *L, size_t bytes, const LuaNew_ID &ID,
                        const char *name, const char *env_mode) {
  static const runtime_error nomem_exc(
      "out of memory"); // so throwing it won't cause allocs :)
  L_GROW(2);
...
/*
 * Keep 'index_ref' Lua object alive (out of GC) until 'index_this' (a
 * LuaNew-derived userdata) has been destroyed.
 *
 * This functionality is provided using the Lua userdata environment table 1..N
 * indices. The application can use empty indices, or indices other than 1..N,
 * but NOT TOUCH the ones we've added.
 *
 * Returns:
 *   the index used for keeping 'index_ref' alive. This can be used for
 * reclaiming a copy of that value via 'push_alive()'.
 */
unsigned /*key*/ LuaNew_base::keep_alive(lua_State *L, int index_this,
                                         int index_ref) {
 ...
 */
void LuaNew_base::create_mt(lua_State *L, f_setup setup_f, const LuaNew_ID &_id,
                            const char *name) {
  assert(name);
...
/*
 * Like 'lua_typename(L,lua_type(L,index))' but digs out the userdata subtypes
 * of LuaNew-derived userdata objects.
 */
const char *LuaNew_base::typename_(lua_State *L, int index) {
  ...
  // Check 'metatable.__type' first. This allows i.e. tables to have a subtype
  // (not only userdata)
  //
  if (lua_getmetatable(L, index_abs)) {
    ...
    case LUA_TSTRING: {
      const char *s =
          lua_pushfstring(L, "%s:%s", normal_typename, lua_tostring(L, -1));

      // Leaving the type name on TOS would confuse the caller. Removing it
      // might make the C pointer invalid (not immediately, but in later GC). We
      // need to give a second reference somewhere.
      //
...
void LuaNew_base::dump(lua_State *L, const char *file, int line) {
  ...
  for (i = 1; i <= top; i++) {
    // 'L_typename()' gives "userdata:xxx" kind of names (= more info than Lua
    // itself)
    //
...
void _L_nilerr_fmt(lua_State *L, const char *fmt, ...) {
  ...
  /*
   * By using 'lua_pushvfstring()' we don't need to make a buffer of arbitrary
   * size (and won't ever get truncations on the report).
   */
  va_list vl;
  va_start(vl, fmt);
