...
void TrackerBase::init() {
  PTHREAD_CALL(pthread_create, &thread_h, nullptr, polling_thread,
               (void *)this);
}
...
TrackerBase::~TrackerBase() {
   ...
   */
  for (map<JDay, TrackedData *>::iterator it = available_data.begin();
       it != available_data.end(); ++it) {
 ...
 * Polling thread.
 *
 * Find out periodically whether new data is available, and update
 * 'available_data'.
 *
 * Note:
  ...
  while (true) {
     ...
     */
    {
      ...
      for (map<JDay, TrackedData *>::iterator it = my->available_data.begin();
           it != my->available_data.end(); ++it) {
        TrackedData *d = it->second;
        if (!d)
          continue; // BZ2 entry based on cache information (skip)

        // Release memory mapping if no users and last acquire was old enough.
        // Use global or track specific, or filemask specific wiping cycle (the
        // last array slot) if given
        d->Wipe(my->wiping[my->wiping[3] ? 3 : (my->archived() ? 1 : 0)],
                my->wiping[2]);
      }
    }
...
/*
 * Get the available set of origintimes, sorted to descending order ([0] is
 * latest).
 */
 ...
 */
void TrackerBase::addOriginTimes_unsorted(
    vector<JDay> &vec, const OriginTimeQuery originTimeQuery) const throw() {
...
void TrackerBase::addDatas_must_release(const JDay &ot,
                                        vector<TrackedData *> &vec) const
    throw() {
  wait_until_initialized();
...
/*
 * Returns the most recent origintime available, or 'empty' (test with 'operator
 * bool') for no data.
 *
 * This function is FAST and does not cause locks to be required.
 */
JDay TrackerBase::getLastOriginTime_fast(NA_Level::Type &leveltype) const
    throw() {
  wait_until_initialized();
...
/*
 */
JDay TrackerBase::getLastOriginTime_LOCKED(NA_Level::Type &leveltype) const
    throw() {
  JDay last_ot;
  ...
  if (!archived())
    for (map<JDay, TrackedData *>::const_iterator it = available_data.begin();
      ...
      // For PRESSUREORHYBRID_LEVEL use pressure (primary) or hybrid data
      // whichever is newer

      if (((leveltype == NA_Level::NO_LEVEL) ||
           (leveltype1 == it->second->getLevelType()) ||
           (leveltype2 == it->second->getLevelType())) &&
          ((!last_ot) || (ot > last_ot) ||
           ((ot == last_ot) &&
...
/*
 * Returns the oldest origintime available, or 'empty' (test with 'operator
 * bool') for no data.
 *
 * TBD: Getting this faster (not needing the lock) might be welcome for upper
 * layers.
 */
