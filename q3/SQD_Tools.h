...
struct SQD_Tools { // just as a namespace
  ...
  // static FmiParameterName newbase_id( const char *name, const NA_Info *info=
  // 0 );

  static std::string standard_param_name(FmiParameterName e);
  static FmiParameterName standard_param_id(const char *s);

  static bool latlon_dx_dy(const char *proj, const LatLon &ll, double &dx,
                           double &dy);

  static LatLon convert(const NFmiPoint &p) { return LatLon(p.X(), p.Y()); }

  static bool cut_at_colon(const char *s, std::string &name_part,
                           FmiParameterName &e);
