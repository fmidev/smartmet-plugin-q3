 ...
 */
const ContourInfo_common *
ContourInfo_common::create(const std::string &conf,
                           const ContourMatrix<float> &data) {
  switch (conf.c_str()[0]) {
  case '1':
  case '2':
...
static ContourInfo_common::uint_ARGB SetColor(const string &s,
                                              vector<unsigned> &dash) {
  string s_no_dash = StripDash(s, dash);

  // Pre-known special values
  //
  if (s_no_dash == "none")
    return 0xff000000; // completely transparent
  if (s_no_dash == "red")
    return ContourInfo_common::RED;
  if (s_no_dash == "green")
    return ContourInfo_common::GREEN;
  if (s_no_dash == "blue")
    return ContourInfo_common::BLUE;
  if (s_no_dash == "yellow")
    return ContourInfo_common::YELLOW;
  if (s_no_dash == "white")
    return ContourInfo_common::WHITE;
  if (s_no_dash == "black")
    return ContourInfo_common::BLACK;
  ...
  for (unsigned n = 0; n < 4; n++) {
    ...
    if (k == string::npos) {
      ...
      if (n < 3)
        ...
        color |= (255 - vi) << 24; // save A as transparency (255-opaqueness)
    }
  }
  return color;
}
 ...
 */
static ContourInfo_common::uint_ARGB SetColorAndHatch(const string &s_,
                                                      int &hatch_index) {
  string s_rgb;
 ...
 */
ContourInfo_common::StrokeParams::StrokeParams(const string &color_s,
                                               const string &width_s,
                                               const string &smooth_s) {

  color = (color_s == "def" || color_s == "") ? BLACK : SetColor(color_s, dash);
  width = (width_s == "def" || width_s == "")
              ? 1
              : NFmiStringTools::Convert<float>(width_s.c_str());
  smooth = (smooth_s == "def" || smooth_s == "")
               ? 0.0F
               : NFmiStringTools::Convert<float>(smooth_s.c_str());
  if (smooth < 0.0F)
    smooth = 0.0F;
}
 ...
 */
bool ContourInfo_common::StrokeParams::operator!=(
    const ContourInfo_common::StrokeParams &o) const {
 ...
 * Label params
 */
ContourInfo_Line::LabelParams::LabelParams(const string &strategy_s,
  ...
  strategy = ((strategy_s == "def" || strategy_s == "")
                  ? 0
                  : NFmiStringTools::Convert<unsigned>(strategy_s.c_str()));
  font_size_ = (font_size_s == "def" || font_size_s == "" || strategy_s == "")
                   ? 0
                   : NFmiStringTools::Convert<float>(font_size_s.c_str());
  font_color = (font_color_s == "def" || font_color_s == "" || strategy_s == "")
                   ? BLACK
                   : SetColor(font_color_s);
  box_fill_color =
      (box_fill_color_s == "def" || box_fill_color_s == "" || strategy_s == "")
          ? YELLOW
          : SetColor(box_fill_color_s);
  box_stroke_color = (box_stroke_color_s == "def" || box_stroke_color_s == "" ||
                      strategy_s == "")
                         ? BLUE
                         : SetColor(box_stroke_color_s);
  box_stroke_width =
      (box_stroke_width_s == "def" || box_stroke_width_s == "" ||
       strategy_s == "")
          ? 1
          : NFmiStringTools::Convert<float>(box_stroke_width_s.c_str());
}
...
ContourInfo_Fill::FillParams::FillParams(const string &fill_color_s)
    : color(WHITE), hatch_width(0) // no hatch pattern
      ,
      hatch_offset(false), hatch_type((enum e_hatch_type)0) {
  if (fill_color_s != "def") {
    int hatch_index;
...
static void find_limits(const ContourMatrix<float> &data, float step,
  ...
  if (lo_ref) {
    ...
    *lo_ref = (mod == 0.0) ? min : (min > 0.0) ? min + (step - mod) : min + mod;
  }
  if (hi_ref) {
    // Round 'max' downwards to next 'step'
    //
    float mod = fmodf(fabsf(max), step);
    *hi_ref = (mod == 0.0) ? max : (max > 0.0) ? max - mod : max - (step - mod);
  }
}
 ...
 *
 * i.e. "1 5.0 0.0 900.0 1050.0 rgba(40,40,40,100) none 2  2.0 2 20 def none
 * none 1"
 *       ^-- function type
 *         ^-- ...type specific
 *                              ^-- color & opacity
 ...
 */
const ContourInfo_Line *
ContourInfo_Line::create(const string &conf, const ContourMatrix<float> &data) {
  vector<float> levels;
  ...
  if (type == 1) {
    ...
    if ((lo == 32700) || (hi == 32700)) {
      find_limits(data, step, (lo == 32700) ? &lo : nullptr,
                  (hi == 32700) ? &hi : nullptr);
    }
    ...
    throw runtime_error("Bad contour type " +
                        boost::lexical_cast<string>(type) + " (expected 1,2)");
  }
  ...
  string stroke_color, fill_color, stroke_width, smooth_factor, label_strategy,
      label_font_height, label_text_color, label_box_fill_color,
      label_box_stroke_color, label_box_stroke_width;
  ...
  return new ContourInfo_Line(
      levels, StrokeParams(stroke_color, stroke_width, smooth_factor),
 ...
 */
cairo_pattern_t *
ContourInfo_Fill::FillParams::cairo_pattern_create(size_t size) const {
  ...
  case HATCH_HORIZONTAL_VERTICAL:
  case HATCH_HORIZONTAL:
    cairo_move_to(cr, -((int)size), mid_y);
    cairo_line_to(cr, size, mid_y);
    if (hatch_offset) {
      cairo_move_to(cr, -((int)size), mid_y - size);
      cairo_line_to(cr, size, mid_y - size);
    }
    if (hatch_type == HATCH_HORIZONTAL)
      break;
    /* fall through */

  case HATCH_VERTICAL:
    cairo_move_to(cr, mid_x, -((int)size));
    cairo_line_to(cr, mid_x, size);
    if (hatch_offset) {
      cairo_move_to(cr, mid_x - size, -((int)size));
      cairo_line_to(cr, mid_x - size, size);
    }
    break;

  case HATCH_DOWN_UP:
  case HATCH_DOWN:
    cairo_move_to(cr, -((int)size), (int)size);
    cairo_line_to(cr, size, -((int)size));
    if (hatch_type == HATCH_DOWN)
      break;
    /* fall through */
 ...
 */
const ContourInfo_Fill *
ContourInfo_Fill::create(const string &conf, const ContourMatrix<float> &data) {
  ...
  if (type == 3) {
    ...
    *
    if ((lo==32700) || (hi==32700)) {
        find_limits( data, step, lo==32700 ? &lo:nullptr, hi==32700 ?
    &hi:nullptr );
    }
    */
    ...
    throw runtime_error("Bad contour type " +
                        boost::lexical_cast<string>(type) + " (expected 3,4)");
  }
  ...
  return new ContourInfo_Fill(
      ranges, StrokeParams(stroke_color, stroke_width, smooth_factor),
