/*
 * SQD_DATA.H                            Copyright (c) 2009-10, Ilmatieteen
 * laitos
 *
...
public:
  ...
  /*virtual*/ CONST_IF_SERVER Matrix *push_NativeMatrix(
      lua_State *L, const JDay &vt, const NA_Level &lev,
      const NA_Param &p
      //
      // 22-Aug-2011 PKi: For fetching data using newbase interpolation,
  ...
  */
  static void
  mapParameter(const FmiParameterName id,
...
private:
  ...
  // Note: Must have different name from 'push_NativeMatrix' (this is not
  // virtual)
  //
  CONST_IF_SERVER Matrix *push_NativeMatrix_e(
      lua_State *L, const JDay &vt, const NA_Level &lev,
      FmiParameterName e
      //
      // 22-Aug-2011 PKi: For fetching data using newbase interpolation,
  ...
  */
  /*virtual*/ FmiParameterName mapParameter(const NA_Param &p,
                                            bool primary = true,
  ...
  // data members
  //
  // Note: 'NFmiQueryData' is const in server mode, but since Newbase
  // 'NFmiFastQueryInfo' constructor
  //       does not allow a const 'NFmiQueryData', we cannot declare it as const
  //       here. Bohooo... :(
  //
#ifdef METQU
