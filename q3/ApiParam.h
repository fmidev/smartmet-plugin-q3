...
public:
  ApiScalarParam(const char *name_ = 0) : name(name_ ? name_ : "") {
    INVARIANT();
  }
...
public:
  ...
  ApiParam(const ApiScalarParam &o)
      : name(o.getName()), a(o), b(), polar(false) {
    INVARIANT();
  }
  ...
  bool covered_by(const std::vector<NA_Param> &vec, NA_Param &na, NA_Param &nb,
                  bool &polar_) const;
  ...
  convert_to_api(const std::vector<NA_Param> &vec, bool prefer_standard_names);
  // 25-Oct-2011 PKi: Now needed by server too
  //#ifdef METQU
  static std::vector<NA_Param>
  convert_to_native(const std::vector<ApiParam> &vec,
                    const NA_Data *data = nullptr);
  //#endif
...
private:
  // Note: Use of 'vector<ApiParam>' requires an assignment operator. The
  // automatic one is okay, but
  //       we cannot have any 'const' members.

  // data members
  //
  /*const*/ std::string
      name; // name of param as seen in the script (i.e. "T", "WS", "WIND", ..)

  /*const*/ ApiScalarParam
      a; // scalar param | first vector component (x or abs)
