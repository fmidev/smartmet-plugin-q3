...
public:
  ...
  typedef unsigned long
      offset_t; // even 32-bit allows for 16GB data blocks (enough)
  ...
  MatrixPos operator+(const MatrixPos &o) const {
    return MatrixPos(x + o.x, y + o.y);
  }
  MatrixPos operator-(const MatrixPos &o) const {
    return MatrixPos(x - o.x, y - o.y);
  }
  ...
  //
  bool operator<(const MatrixPos &o) const {
