 ...
 *
 * Bugs:
 *   Ubuntu 32-bit (9.04) is seen to have problems when SSE is enabled. Unsure,
 * why.
 *       --AKa 27-Aug-2009
 */
#include "SSE.h"
 ...
 * To be run once.
 */
static struct JustOnce_SSE { JustOnce_SSE(); } just_once;
...
JustOnce_SSE::JustOnce_SSE() {
  ...
  v = std::min<float>(nan, nan);
  always_assert(isnanf(v));
  v = std::min<float>(X, nan);
  always_assert(v == X);
  // v= std::min<float>( nan, X ); always_assert( v==X );   // THIS FAILS

  v = std::max<float>(nan, nan);
  always_assert(isnanf(v));
  v = std::max<float>(X, nan);
  always_assert(v == X);
  // v= std::max<float>( nan, X ); always_assert( v==X );   // THIS FAILS

  v = std::min<float>(INF_F, nan);
  always_assert(v == INF_F);
  v = std::min<float>(INF_F, X);
  always_assert(v == X);

  v = std::max<float>(-INF_F, nan);
  always_assert(v == -INF_F);
  v = std::max<float>(-INF_F, X);
  always_assert(v == X);
...
void SSE_abs(MemMatrix &c, const Matrix &a, MatrixPos::offset_t &i,
  ...
  if (a_data) {
    ...
    while (i + (16 - 1) < n) { // bulk handled here
      *(vc++) = __builtin_ia32_sqrtps(__builtin_ia32_mulps(*va, *va));
      ++va;
      *(vc++) = __builtin_ia32_sqrtps(__builtin_ia32_mulps(*va, *va));
      ++va;
      *(vc++) = __builtin_ia32_sqrtps(__builtin_ia32_mulps(*va, *va));
