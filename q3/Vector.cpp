 ...
 *
 * Note: This method is not really needed, but it may be expected by some (to be
 * able to compare vectors by value). Without this, Lua compares userdata
 * references, not their values (only the object itself is equal to itself).
 *
 ...
 *
 * Note: Two vectors with same absolute strength but different angles are deemed
 * not-less-than to each other. Still, they are not equal either. In other
 * words, this function cannot be used for sorting.
 *
 * Note: How exactly vectors are placed in order (and whether an order is
 * required, at all) is arguable. This feature is here mainly for MAXZ et.al. to
 * be able to find strongest values also in vector fields.
 */
 ...
 * vector_ud= __div( (any), vector_ud )      -- this is illegal, but will take
 * us here
 */
 ...
 *
 * Note: 'RegTools' decimals and binary setting only affect matrix output, not
 * us (we output in full accuracy, always).
 */
int VectorBind::tostring(lua_State *L) {
  ...
  //
  // Note: We do lose values in the case polar coordinates would have either
...
/*
 * For polar coordinates, degs are converted to rads and normalized to certain
 * range.
 *
 * Values are adjusted to be within the [0,inf) and [0,2*M_PI) ranges. Doing it
 * here makes sure all calculations will automatically get their values rightly
 * adjusted (i.e. multiplication by negatives will turn the direction around).
 */
...
/*
 * Return the direction of the vector, in [0,360) range (0=North, grows
 * clockwise).
 *
 * Note: Instead of doing special if/else for the NAN case, we make sure it
 * flows through the regular calculations, producing NAN at the end (this is
 * faster).
 */
double Vector::getDeg() const {
  ...
  if (polar) {
    ...
    // 2. reducing from pi turns 0 to North and swaps to clockwise growth.
    // [-3/2*pi, pi/2)
...
Vector Vector::operator*(double d) const {
  if (!polar) {
    ...
    return Vector(
        v1 * d, v2,
        true); // constructor will turn the direction if 'v' is negative
  }
}
