 ...
 */
const unsigned MINIMUM_POINTS_IN_CHUNK =
    10; // 2..n (too small or too big is slow)
    ...
    : public PointAndVector // also have the N-1..N vector (cached)
{
  ...
  static const double
      SCORE_REJECTED; // point considered and found to be no-good
  static const double
      SCORE_TOO_CLOSE; // point too close to existing label (don't consider)
...
public:
  ChainChunk(double area_limit_) : curve(), area_limit(area_limit_), bb() {}
  ...
  unsigned size() const { return curve.size(); }

  bool intersects(const TiltedRect &tr, Labelizer::Drawer &debug_drawer) const;
  bool intersects(const PointAndVector &tr,
                  Labelizer::Drawer &debug_drawer) const;

  PointScore &operator[](unsigned i) { return curve[i]; }
};
...
public:
  ...
  bool valid() const { return chunk_n >= 0; }
  ...
  bool is_last() const {
    return (!is_closed) && ((unsigned)chunk_n == chunks.size() - 1) &&
           ((unsigned)inner_n == chunks[chunk_n].size() - 1);
  }
  ...
  ChainIterator &operator-=(int step) { return *this += (-step); }
  ...
  PointScore *operator->() { return &(operator*()); }
  const PointScore *operator->() const { return &(operator*()); }
  ...
  bool operator==(const ChainIterator &o) {
    return valid() && o.valid() && (chunk_n == o.chunk_n) &&
           (inner_n == o.inner_n);
  }

  bool operator!=(const ChainIterator &o) { return !(*this == o); }
};
...
public:
  Chain(const FontSpec &fs_, double chunk_area_limit_, Drawer &debug_drawer_)
      : fs(fs_), chunks(), bb(), st(ST_BUSY), total_points(0),
        chunk_area_limit(chunk_area_limit_), prev_p(),
...
Labelizer::~Labelizer() {
  ...
  for (vector<Chain *>::iterator it = chains.begin(); it != chains.end();
       ++it) {
    delete *it;
  }
  ...
  */
  ...
  LOG_TIMER("Profiling: Sort %ld, "
            ...
            SORT_PROF.ms(), CANDIDATE_PROF.ms(), LIMIT_ALONG_LINE_PROF.ms(),
            LIMIT_DIRECT_PROF.ms(), LIMIT_CROSSING_LINES_PROF.ms(),
...
void Labelizer::Chain::add_point(const Point &xy) {
  ...
  if (empty) {
    ...
    chunks.back().add_point(
        xy, Vector()); // null vector since we don't have a prior one
  } else {
    Vector v(prev_p, xy); // stored in 'PointScore' class for optimization
                          // purposes (direction from point N-1 to N)
...
void Labelizer::Chain::done(const Config &config) {
  ...
  if (config.dampen_corners > 1.0)
    LOG_WARNING(".dampen_corners factor is supposed to be <= 1.0 (%lf)",
                config.dampen_corners);

  if (config.boost_horizontal < 1.0)
    LOG_WARNING(".boost_horizontal factor is supposed to be >= 1.0 (%lf)",
                config.boost_horizontal);

  assert(total_points > 0);

  const double DAMPEN_CORNERS_LIMIT_COS =
      cos(config.dampen_corners_limit_deg * (180.0 / M_PI));
  const double BOOST_HORIZONTAL_LIMIT_COS =
      cos(config.boost_horizontal_limit_deg * (180.0 / M_PI));
  ...
  for (; it.valid(); ++it, ++it_next) {
    ...
    if (it == it_next) {
      ...
      //
      LOG_BUG("One-point chunks should not happen (skipping): %g %g", it->p.x,
              it->p.y);
...
void Labelizer::add_point(const Point &xy, const FontSpec &fs) {
  ...
  //
  double chunk_area_limit = (config.curve_optimization != 0.0)
                                ? bb.area() / config.curve_optimization
...
vector<Labelizer::TiltedRect> Labelizer::done(double scale) {
  ...
  for (vector<Chain *>::iterator it = chains.begin(); it != chains.end();
    ...
    while (true) {
      ...
      if (!LimitAlongTheLine(chosen, config.limit_along_line * scale,
                             is_closed)) {
        ...
        if (!bb.inside_or_at_edge(tr.get_bounding_box())) {
        ...
        } else if ((!config.allow_overlapping_lines) &&
                   (!LimitCrossingLines(tr, chains, *it))) {
          // some (other than chain) line passed through the label (YELLOW)
          debug_drawer.mark_candidate(chosen_p, goodness, "yellow");
        } else if ((!config.allow_overlapping_labels) &&
                   (!LimitOverlappingLabels(tr, done))) {
          ...
          //
          chosenlabels.push_back(
              ChosenLabel(tr.get_center(), tilt_rad, (*it)->get_fs()));
          used = true;
       ...
       */
      chosen->score =
          used ? PointScore::SCORE_LABEL : PointScore::SCORE_REJECTED;
    }
  }
 ...
 *   true for go ahead
 */
bool Labelizer::LimitAlongTheLine(const ChainIterator &chosen, double limit,
                                  bool is_closed) {
  bool ret = false;
 ...
 *   true for go ahead
 */
bool Labelizer::LimitDirect(const Point &xy, const vector<TiltedRect> &done,
                            double limit) {
  bool ret = false;

  LIMIT_DIRECT_PROF.start();
  {
    for (vector<TiltedRect>::const_iterator it = done.begin(); it != done.end();
         ++it) {
      double dist = Vector(xy, it->get_center()).norm();
      // LOG_DEBUG( "Dist %lf,%lf -> %lf %lf = %lf (limit %lf)", xy.x, xy.y,
      // it->get_center().x, it->get_center().y, dist, limit );
 ...
 *   true for go ahead
 */
bool Labelizer::LimitCrossingLines(const TiltedRect &tr,
 ...
 *   true for go ahead
 */
bool Labelizer::LimitOverlappingLabels(const TiltedRect &tr,
                                       const vector<TiltedRect> &done) {
  bool ret = false;
 ...
 */
void Labelizer::RejectNearby(const ChainIterator &chosen,
                             double limit_along_line, bool is_closed) {
  ...
  for (unsigned round = 0; round < 2; round++) {
    ...
    ChainIterator it_was = chosen;
    for (ChainIterator it(chosen, is_closed, step); it.valid();
         it_was = it, it += step) {
 ...
 */
Labelizer::TiltedRect::TiltedRect(const Point &p, const Vector &size,
                                  double tilt_rad)
 ...
 * horizontal, rising counter-clockwise). The angle is limited to [-pi/2,pi/2)
 * range making it always upright to the user.
 *
 ...
 */
Labelizer::ChainIterator Labelizer::Chain::candidate(double *tilt_rad_ref,
                                                     double *goodness_ref) {
...
/*
 */
bool Labelizer::ChainChunk::intersects(const PointAndVector &v,
                                       Labelizer::Drawer &debug_drawer) const {
  ...
  for (vector<PointScore>::const_iterator it = curve.begin(); it != curve.end();
       ++it) {
    if (it->intersects(v)) {
...
/*
 */
bool Labelizer::ChainChunk::intersects(const TiltedRect &tr,
                                       Labelizer::Drawer &debug_drawer) const {
  ...
  for (vector<PointScore>::const_iterator it = curve.begin(); it != curve.end();
       ++it) {
    // cached vector is from the earlier point to us, so we cannot use it
    // directly
    //
...
// 01-Feb-2012 PKi: Load Labelizer's default configuration (taken from config
// file)
//
...
//
const Labelizer::Config &Labelizer::loadUserCfg(lua_State *L, int idx,
...
//
//                  uConfig is false when called for configuration loaded from
//                  config file (lCfg is reference to Labelizer::lblCfg and
//                  Labelizer::cfp field addresses can be used directly);
//                  otherwise (user given configuration) field offsets must be
//                  used
//
const Labelizer::Config &Labelizer::loadCfg(lua_State *L, int idx,
                                            Labelizer::Config &lCfg,
                                            bool uConfig) {
  ...
  while (lua_next(L, idx)) {
    ...
    if (!(cf->fld))
      ...
      luaL_error(L, "Labelizer config: invalid value type %s for %s",
...
const Labelizer::_configFlds *Labelizer::getConfigFlds() {
  static Labelizer::_configFlds labelCfgFlds[] = {
      {(const char *)"limitalongline", Labelizer::T_DOUBLE,
       &Labelizer::lblCfg.limit_along_line, nullptr, nullptr},
      {(const char *)"limitdirect", Labelizer::T_DOUBLE,
       &Labelizer::lblCfg.limit_direct, nullptr, nullptr},
      {(const char *)"allowoverlappinglines", Labelizer::T_BOOL, nullptr,
       &Labelizer::lblCfg.allow_overlapping_lines, nullptr},
      {(const char *)"allowoverlappinglabels", Labelizer::T_BOOL, nullptr,
       &Labelizer::lblCfg.allow_overlapping_labels, nullptr},
      {(const char *)"dampencornerslimitdeg", Labelizer::T_DOUBLE,
       &Labelizer::lblCfg.dampen_corners_limit_deg, nullptr, nullptr},
      {(const char *)"dampencorners", Labelizer::T_DOUBLE,
       &Labelizer::lblCfg.dampen_corners, nullptr, nullptr},
      {(const char *)"boosthorizontallimitdeg", Labelizer::T_DOUBLE,
       &Labelizer::lblCfg.boost_horizontal_limit_deg, nullptr, nullptr},
      {(const char *)"boosthorizontal", Labelizer::T_DOUBLE,
       &Labelizer::lblCfg.boost_horizontal, nullptr, nullptr},
      {(const char *)"curveoptimizationfactor", Labelizer::T_DOUBLE,
       &Labelizer::lblCfg.curve_optimization, nullptr, nullptr},
      {(const char *)"decimals", Labelizer::T_INT, nullptr, nullptr,
       &Labelizer::lblCfg.decimals},
