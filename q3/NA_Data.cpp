...
/*
 * Note: These strings are used only as runtime keys. Feel free to rename
 * without Dire Consequences. Anything unique (among themselves) will do.
 */
...
NA_Info::NA_Info(const time_t loadTime_, const time_t modificationTime_,
                 const JDay &ot_, const vector<JDay> &times_,
                 const string &nativeLevelType_,
                 const vector<NA_Level> &levels_,
                 const vector<NA_Param> &params_, const Projection &projection_,
                 const string &wkt_, const uint timeStep_, const double dx_,
                 const double dy_, const bool gridData_) throw()
    : loadTime(loadTime_), modificationTime(modificationTime_), ot(ot_),
      times(times_), nativeLevelType(nativeLevelType_), levels(levels_),
      params(params_), proj(projection_), wkt(wkt_), timeStep(timeStep_),
      dx(dx_), dy(dy_), gridData(gridData_),
      //
      extra(/*extra_*/) {
  INVARIANT();
}
...
int NA_Info::getExtra_int(const char *key, int def) const {
  ...
  if (*endp) {
    throw E_LOG_USAGE("Bad syntax in extra (expected an integer): '%s=%s'", key,
                      s.c_str());
  } else {
    return ret;
  }
 ...
 *
 * Calls:
 *   cancel_bool= func( progress_num 0..100, secs_num, total_secs_num, a_str,
 * b_str, c_str )
 */
#ifdef METQU
bool /*cancel*/ NA_Data::ProgressCallback::progress(unsigned long current,
                                                    unsigned long total,
                                                    const char *a,
                                                    const char *b,
                                                    const char *c) {
  ...
  if (!f_index)
    return false; // no callback enabled
  ...
  double eta_secs = ((!current) || (secs == 0.0))
                        ? 10.0 // no progress yet (cannot make an estimate)
  ...
  if (st != 0) {
    throw E_LOG_USAGE("Runtime error in progress callback: %s",
                      lua_tostring(L, -1));
  }
 ...
 *
 * This is the default projection function and can be overridden by certain data
 * provider modules (classes derived from 'NA_Data'). I.e. SQD format overrides
 * this to handle Newbase projection strings.
 */
/*virtual*/ const Matrix *
NA_Data::push_Matrix(lua_State *L, const JDay &vt, const NA_Level &lev,
                     const NA_Param &p, const Projection &target_proj_orig,
  ...
  const Matrix *m_native =
#ifdef METQU
      push_NativeMatrix_const(L, vt, lev,
                              p
#else
      push_NativeMatrix(L, vt, lev, p
                              ...
                              // 22-Aug-2011 PKi: Fetch SQD data using newbase
                              // interpolation, projection and scaling

                              ,
                              &target_proj_orig, &target_gs_orig, &target_ready,
                              dataIds
  ...
  if (method == NA_Param::INTERPOLATE_UNKNOWN) {
    ...
    throw E_LOG_BUG("Shouldn't be here with unknown interpolation (%s)",
                    name.c_str());
  }
#endif
  ...
  const Projection &target_proj =
      target_proj_orig ? target_proj_orig : native_proj;
