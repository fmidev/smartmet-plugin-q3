...
void contourFromLinearRing(const LinearRing *geom,
...
void contourFromLineString(const LineString *geom,
                           PathAdapterBase *pathAdapter) {
  if (geom == nullptr || geom->isEmpty())
    return;
...
void contoursFromMultiLineString(const MultiLineString *geom,
                                 PathAdapterBase *pathAdapter) {
  if (geom == nullptr || geom->isEmpty())
    return;

  for (size_t i = 0, n = geom->getNumGeometries(); i < n; ++i)
    contourFromLineString(
        dynamic_cast<const LineString *>(geom->getGeometryN(i)), pathAdapter);
}
...
void contoursFromMultiPolygon(const MultiPolygon *geom,
                              PathAdapterBase *pathAdapter) {
  if (geom == nullptr || geom->isEmpty())
    return;

  for (size_t i = 0, n = geom->getNumGeometries(); i < n; ++i)
    contoursFromPolygon(dynamic_cast<const Polygon *>(geom->getGeometryN(i)),
                        pathAdapter);
}
...
void contoursFromGeometryCollection(const GeometryCollection *geom,
                                    PathAdapterBase *pathAdapter) {
  if (geom == nullptr || geom->isEmpty())
    return;
...
void getContours(const Geometry *geom, PathAdapterBase *pathAdapter) {
  if (const LinearRing *lr = dynamic_cast<const LinearRing *>(geom))
  ...
  else if (const MultiLineString *ml =
               dynamic_cast<const MultiLineString *>(geom))
  ...
  else if (const GeometryCollection *g =
    ...
    throw std::runtime_error(
        "Encountered an unsupported GEOS geometry component");
}
} // namespace Q3GeosTools
} // namespace SmartMet
