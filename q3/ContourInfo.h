...
public:
  ...
  static double A(uint_ARGB argb) {
    return 1.0 - (((argb) >> 24) & 0xff) / 255.0;
  }
  ...
  /*const*/ struct StrokeParams {
    uint_ARGB color;
    std::vector<unsigned>
        dash; // empty vector for not being used
  ...
  protected:
    ...
    StrokeParams(const std::string &color_s, const std::string &width_s,
...
protected:
  ContourInfo_common(const StrokeParams &stroke_) : stroke(stroke_) {}
};
...
public:
  ...
  /*const*/ struct LabelParams {
    unsigned strategy; // 0: no labels, 1&2: horizontal, 3: tilted according to
                       // curvature
  ...
  private:
    friend class ::ContourInfo_Line;
    LabelParams(const std::string &strategy_s, const std::string &font_height_s,
  ...
  ContourInfo_Line(const std::vector<float> &levels_,
                   const StrokeParams &stroke_, const LabelParams &label_)
      : ContourInfo_common(stroke_), levels(levels_), label(label_) {}

  virtual bool line_type() const { return true; }

private:
  friend class ContourInfo_common;
  static const ContourInfo_Line *create(const std::string &conf,
                                        const ContourMatrix<float> &data);
};
...
public:
  ...
  ContourInfo_Fill(const std::vector<std::pair<float, float>> &ranges_,
                   const StrokeParams &stroke_, const FillParams &fill)
      : ContourInfo_common(stroke_), ranges(ranges_), pSize(10) {
...
private:
  friend class ContourInfo_common;
  static const ContourInfo_Fill *create(const std::string &conf,
                                        const ContourMatrix<float> &);
...
public:
   ...
   */
  double scale_x() const { return (x[1] - x[0]) / (xx[1] - xx[0]); }
  double scale_y() const { return (y[1] - y[0]) / (yy[1] - yy[0]); }

  long pan_x() const { return x[0] - (long)floor(xx[0] * scale_x() + 0.5); }
  long pan_y() const { return y[0] - (long)floor(yy[0] * scale_y() + 0.5); }
};
