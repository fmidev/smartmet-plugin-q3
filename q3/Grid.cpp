 ...
 *
 * Handles one time, level, projection and gridsize (all parameters) data
 * management.
 *
 ...
 *
 * The purpose of the caching is not so much the speedup, but preventing
 *situations where explicit optimizations are needed (i.e. local aliasing).
 *--------------------*/
...
int GridBind::__index(lua_State *L) {
  ...
  /*
   * Note: There is no '.gridsize' (or '.size') property since in our data model
   * the size may be different for different parameters. Instead, take a
   * parameter matrix and use its '.size'.
   */
  ...
  if (lua_isnil(L, -1)) {
    ...
    // Regardless the type of pushed matrix (MemMatrix or proxy) the grid needs
    // to be kept alive for the '.grid' feature to work.
    //
    // 'SQD_Matrix' and 'MQD_Matrix' would require such keep-alive simply to
    // remain usable.
    //
 ...
 *      What happens (should happen) if matrices with different projection
...
/*
 * 'gs' == 'MatrixPos::ZERO' marks "native gridsize" (for each param tile, which
 * can have different sizes)
 */
#ifdef METQU
Grid::Grid(Raw_interface &r, const JDay &vt_, const NA_Level &lev_,
    ...
    : data(r.getData_rw()), vt(vt_), level(lev_), proj(proj_), gridsize(gs_),
      ...
      locs(locs_ ? *locs_ : LatLonList()), flightRoute(flightRoute_),
  ...
  //  11-Nov-2011 PKi: Do not check cross times; just return missing values for
  //  times not in data
  //
  //  else
  //  {
...
Grid::Grid(const Raw_interface &r, const JDay &vt_, const NA_Level &lev_,
           const Projection &proj_,
           const MatrixPos &gs_
           //
           // 22-Sep-2011 PKi: For fetching cross using newbase
  ...
  // In METQU mode the data can be read/write. If we get a 'const' 'r', the data
  // better also be marked read-only.
  //
#ifdef METQU
 ...
 */
#ifdef METQU
void Grid::setMatrix(const NA_Param &p, const Matrix &m) throw(E_READONLY,
                                                               E_BAD_SIZE,
                                                               E_NO_MATCH) {
 ...
 *
 * If native projection and gridsize are used (as initialized in 'this' object),
 * we connect directly to the underlying data (allowing writes in command line
 * mode). Otherwise the returned matrix is a projected, scaled read-only
 * 'MemMatrix'.
 *
 * 'na' and (if a vector) 'nb' are parameters that have already been proven to
 * be in the data.
 */
CONST_IF_SERVER ApiMatrix *
Grid::push_Matrix(lua_State *L, const ApiParam &p) CONST_IF_SERVER {
  ...
  CONST_IF_SERVER Matrix *ma =
      push_ScalarMatrix(L, na); // scalar | first component
  assert(ma);
...
/*
 * Push a scalar matrix, scaled to 'gridsize' (native gridsize for the parameter
 * if 'gridsize'=='MatrixPos::ZERO').
 */
CONST_IF_SERVER Matrix *
Grid::push_ScalarMatrix(lua_State *L, const NA_Param &p) CONST_IF_SERVER {
  ...
  if ((vtVec.size() > 0) && (levelVec.size() > 0) && (locs.size() > 0)) {
    ...
    // 09-Apr-2015 PKi: Do not call push_NativeMatrix to take projection into
    // account
    //
    //	} else if (!gridsize) {
    //  	return data->push_NativeMatrix( L, vt, level, p, nullptr,
    //  nullptr, nullptr, &dataIds);
  } else {
    const Matrix *m = data->push_Matrix(L, vt, level, p, proj, gridsize,
                                        &dataIds); // project and scale
#ifdef METQU
    // We're returning a non-const pointer but essentially it's a read-only
    // object (since it's a projection or scale of the real thing, not 1:1
    // proxy).
    //
    if (m) {
      assert(m->isReadOnly());
...
ApiMatrix *Grid::push_NativeMatrix(lua_State *L, const ApiParam &p) {
  ...
  ApiScalarParam a, b;
  bool polar = false; // to please the compiler ('is_2d' does set it if 'p' is
                      // vector param)
  ...
  if (na) {
    Matrix *ma =
        data->push_NativeMatrix(L, vt, level, na); // scalar | first component
    assert(ma);
