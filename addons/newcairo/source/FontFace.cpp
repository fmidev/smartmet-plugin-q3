...
struct FontFaceMethodNames : public MethodNames {
    FontFaceMethodNames() {
        ...
        if (initialized++) throw runtime_error( "There should be only one FontFaceMethodNames" );
...
cairo_font_face_t *FontFace::load_ttf( const char *fn, unsigned face_index, FT_Library &ft_lib ) {
...
# endif
    ...
    //
    // Ref. http://www.cairographics.org/manual/cairo-FreeType-Fonts.html#cairo-ft-font-face-create-for-ft-face
    //
    static const cairo_user_data_key_t key={0};

    cairo_font_face_t *ff= cairo_ft_font_face_create_for_ft_face( ft_face, 0 /*options*/ );
    int st= cairo_font_face_set_user_data( ff, &key, ft_face, (cairo_destroy_func_t) FT_Done_Face );
    if (st) {
        cairo_font_face_destroy(ff);
