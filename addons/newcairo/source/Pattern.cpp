...
static unsigned get_grant( /*const*/ cairo_pattern_t *p ) {
    ...
    return (pt==CAIRO_PATTERN_TYPE_SOLID) ? GRANT_SOLID :
           ...
           (pt==CAIRO_PATTERN_TYPE_RADIAL) ? GRANT_RADIAL : 0 /*should not occur*/;
}
...
struct PatternMethodNames : public MethodNames {
    PatternMethodNames() {
        ...
        if (initialized++) throw runtime_error( "There should be only one PatternMethodNames" );

        // These functions only for LINEAR and RADIAL patterns:
        //
        map( "add_color_stop_rgb",		Pattern::add_color_stop_rgb, GRANT_LINEAR|GRANT_RADIAL );
        map( "add_color_stop_rgba",		Pattern::add_color_stop_rgba, GRANT_LINEAR|GRANT_RADIAL );
        map( "get_color_stop_rgba",		Pattern::get_color_stop_rgba, GRANT_LINEAR|GRANT_RADIAL );
...
int Pattern_bind::index( lua_State *L ) {
    ...
    if (s) {
        ...
        if (strcmp(s,"info")==0) {
            ...
            {
            double arr[4];
            cairo_status_t st= cairo_pattern_get_linear_points(my, arr,arr+1,arr+2,arr+3);
            if (st==0) {
            ...
            {
            double arr[6];
            cairo_status_t st= cairo_pattern_get_radial_circles(my, arr,arr+1,arr+2,arr+3,arr+4,arr+5);
            if (st==0) {
            ...
            // .matrix      (matrix)
            {
                ...
                cairo_pattern_get_matrix(my,*new(L) Matrix());  // push matrix, then fill it
                lua_settable(L,-3);
            }
...
/*
*/
Pattern::Pattern( cairo_pattern_t *cp_ ) 
    : cp(cp_) { INVARIANT(); }
...
int Pattern::add_color_stop_rgb( lua_State *L ) {
    proto( L, lua_type(L,2)==LUA_TNUMBER ? "number,number,number,number" : "number,string" );
    ...
    // "If the pattern is not a gradient pattern, (eg. a linear or radial pattern), 
...
int Pattern::add_color_stop_rgba( lua_State *L ) {
    proto( L, lua_type(L,2)==LUA_TNUMBER ? "number,number,number,number,number" : "number,string,number" );
    Pattern &my= *Pattern::instance( L, lua_upvalueindex(1) );
...
*       things differently (i.e. not needing 'cairo_pattern_get_color_stop_count()').
*/
int Pattern::get_color_stop_rgba( lua_State *L ) {
    ...
    cairo_status_t st= cairo_pattern_get_color_stop_rgba( my, index-1, &offset, &r,&g,&b,&a );
        //
