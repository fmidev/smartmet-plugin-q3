...
*
* Define:
*   SANDBOX to compile without direct access to local filesystem (usable i.e. on servers).
*
* Todo:
*   We could make error handling uniform, and maybe use 'cairo_status_to_string()' to get
*   the strings. Would there be any help from doing that?
*/
#include "LuaNew.h"
#include "Invariant.h"
#include "Proto.h"
...
# warning "Not guaranteed to work on < 1.6.4 - please upgrade (1.8.8 is the most recent)"
#endif
...
/*
* NOTE: Who and when should do the 'FT_Library' initialization is not clear. We need it for
*       loading TrueType fonts (from a file) and just one (at least *per thread*) such is
*       enough.
*
*       We should clean this up by 'FT_Done_FreeType()' but currently, we don't.
*       --AKa 8-Sep-10
*/
...
/*
* image_surf= image_surface( w_pixels_num, h_pixels_num, [format_string], [filename_str] )  (full)
* image_surf= image_surface( w_pixels_num, h_pixels_num, [format_string] )                  (SANDBOX)
*
* format:   "argb32" (default) | "rgb24" | "a8" | "a1"
*
* Note: There should not be any need to use 'format' other than ARGB32. Having the format
*       where it is does not really matter; this is just an internal API (to '.surface()').
*/
...
# endif
    ...
    cairo_surface_t *surf= cairo_image_surface_create( format, w,h );
        //
        // Always returns a valid pointer, but it will return a pointer to a "nil" surface
        // if an error such as out of memory occurs. Use 'cairo_surface_status()' to check
        // for this.
...
*/
#if (!defined SANDBOX) && \
    (defined CAIRO_HAS_IMAGE_SURFACE) && \
...
* pdf_surf= pdf_surface( w_points_num, h_points_num )                   -- SANDBOX
*/
...
/*
* svg_surf= svg_surface( w_points_num, h_points_num, [version_required_str], [filename_str] )   -- full
* svg_surf= svg_surface( w_points_num, h_points_num, [version_required_str] )                   -- SANDBOX
*/
#ifdef CAIRO_HAS_SVG_SURFACE
...
# endif
    ...
    cairo_svg_version_t v_e= conv_svg_version(L,3,CAIRO_SVG_VERSION_1_2 /*latest*/);
# ifndef SANDBOX
    const char *fn= lua_tostring(L,4);
# endif
...
static int pattern_rgba( lua_State *L ) {
    ...
    Pattern *pat= new(L) Pattern( cairo_pattern_create_rgba(col.r,col.g,col.b,col.a) );
    status_check( L, cairo_pattern_status(*pat) );
    return 1;
}
...
/*
* pattern_ud= pattern_radial( cx0_num, cy0_num, r0_num, cx1_num, cy1_num, r1_num )
*/
static int pattern_radial( lua_State *L ) {
    ...
    Pattern *pat= new(L) Pattern( cairo_pattern_create_radial(x0,y0,r0,x1,y1,r1) );
    status_check( L, cairo_pattern_status(*pat) );
    return 1;
}
...
extern "C" int WIN32_DLLEXPORT luaopen_newcairo( lua_State *L )
{
    ...
    //
    // Ref. http://www.freetype.org/freetype2/docs/reference/ft2-base_interface.html#FT_Init_FreeType
    //
...
L_START {
    // Set up classes
    //
    // Note: For derived classes, we need a slightly more elaborate syntax. It's still
...
# endif
    ...
    lua_pushliteral( L, "newcairo" );   // name for our module (matches the name of the .so and this function)
