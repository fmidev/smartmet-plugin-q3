/*
* SCALEDFONT.CPP                               Copyright 2010, Ilmatieteen laitos
*/
#include "LuaNew.h"
#include "Invariant.h"
#include "Proto.h"
...
struct ScaledFontMethodNames : public MethodNames {
    ScaledFontMethodNames() {
        ...
        if (initialized++) throw runtime_error( "There should be only one ScaledFontMethodNames" );
...
int ScaledFont::text_extents( lua_State *L ) {
    ...
    lua_pushliteral( L, "x_bearing" ); lua_pushnumber( L, ext.x_bearing );
    lua_pushliteral( L, "y_bearing" ); lua_pushnumber( L, ext.y_bearing );
    lua_pushliteral( L, "width" ); lua_pushnumber( L, ext.width );
    lua_pushliteral( L, "height" ); lua_pushnumber( L, ext.height );
    lua_pushliteral( L, "x_advance" ); lua_pushnumber( L, ext.x_advance );
    lua_pushliteral( L, "y_advance" ); lua_pushnumber( L, ext.y_advance );
