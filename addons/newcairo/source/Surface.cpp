...
static unsigned get_grant( /*const*/ cairo_surface_t *surf ) {
    ...
    switch(st) {
        case CAIRO_SURFACE_TYPE_IMAGE:  return GRANT_IMAGE;
        case CAIRO_SURFACE_TYPE_PDF:    return GRANT_PDF;
        case CAIRO_SURFACE_TYPE_SVG:    return GRANT_SVG;
        default: break;
    }
    ostringstream os;
...
/*---=== Surface ===---
*
* This is the base class for actual surfaces s.a. 'ImageSurface' (bitmaps), 'PdfSurface'
* and others.
*
* The surfaces don't really have common properties or methods, but by doing this we can
* allow for object to any of them to be used via 'Surface::instance' as a generic surface.
*/
...
struct SurfaceMethodNames : public MethodNames {
    SurfaceMethodNames() {
        ...
        if (initialized++) throw runtime_error( "There should be only one SurfaceMethodNames" );
    ...
    if (s) {
        ...
        if (cairo_surface_get_type(my) == CAIRO_SURFACE_TYPE_IMAGE) {
            ...
            if (strcmp(s,"format")==0) {
                lua_pushstring(L, conv_image_format( cairo_image_surface_get_format(my) ));
                return 1;
            }
        }
...
#endif
{
    if (surf_) {
        ...
        if (st) {
            ostringstream os;
            os << "Cairo surface at error in initialization: %d" << cairo_status_to_string(st);
            throw runtime_error( os.str().c_str() );
        }
    }
...
* void= surf.finish()
*
* Used for explicitly closing a surface. This will write contents on disk (if not SANDBOX
* and filename given) and mark any drawing contexts to give CAIRO_STATUS_SURFACE_FINISHED
* error if continued to be used.
*
* Use this for making sure a surface has reaced the disk (otherwise it will happen at
* GC, in an unknown time in the future - latest at Lua state cleanup).
*/
...
int Surface::contents( lua_State *L ) {
    ...
    if (!my.finished) {
        ...
        //
        //      For 'ImageSurface', just calling 'image_to_out()' would be sufficient.
        //
        cairo_surface_finish(my);
        my.finished= true;
    }
...
*/
cairo_status_t Surface::collect_f( void *v_this, const unsigned char *data, unsigned int len ) {
...
int ImageSurface::write_to_png( lua_State *L ) {
    ...
    if (st) {
        luaL_error( L, "Cairo error writing to '%s': %s", fn, cairo_status_to_string(st) );
    }
...
/*
* Note: We must have a valid 'io' stream right from the moment we create the 'cairo_surface_t'.
*       Otherwise its header data will be written into a non-initialized stream. In other words,
*       delay creation of the 'cairo_surface_t' until the body of the constructor.
*/
#ifndef SANDBOX
...
#endif
{
    // Now 'Surface' and its 'io' stream are initialized; we can provide 'this' (or just 'io',
    // does not matter) to the stream.
    //
    surf= cairo_pdf_surface_create_for_stream( collect_f, (void*)this, w_points, h_points );
...
/*---=== SvgSurface ===---
*
* NOTE: The implementation is almost 1:1 with 'PdfSurface' but we've still chosen
*       not to make these two from an intermediate common class. They are separate
*       classes; they are just very, very alike (but not quite).
*/
#ifdef CAIRO_HAS_SVG_SURFACE

/*
* 'ver_required' is CAIRO_SVG_VERSION_1_1 | CAIRO_SVG_VERSION_1_2 | (-1: default version)
*/
#ifndef SANDBOX
SvgSurface::SvgSurface( double w_points, double h_points, cairo_svg_version_t ver_required, const char *fn )
    : Surface( 0 /*coming soon*/ , fn )
#else
SvgSurface::SvgSurface( double w_points, double h_points, cairo_svg_version_t ver_required )
    : Surface( 0 /*coming soon*/ )
#endif
{ 
    surf= cairo_svg_surface_create_for_stream( collect_f, (void*)this, w_points, h_points );
