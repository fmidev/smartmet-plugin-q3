...
-- bool= dump( file_ud )
--
-- Returns 'true' if there is input and corresponding output has been created.
--         'false' if there is no input (i.e. the earlier stage failed in a pipe).
--              Some output may be created even in such case.
--
...
local function main( argv )
    ...
    if out_fn then
        ...
        assert( f, "Unable to write '"..out_fn.."': "..(err or "") )
    ...
    if argv[2] then
        USAGE()
    elseif argv[1] then
        local f,err= io.open( argv[1], "rb" )    -- must open as binary
        if not f then
            error( "bin2c: "..err )
        end
        ok= dump(f)
    ...
    if not ok then 
        ...
        os.exit(-1)     -- had no input; preceding stage of a pipe must have failed
    end
end
